...
- Distillation may propose a source-bound Custom Code clause and capability need, but authenticated review must separately select exact closed template parameters. Runtime derives and binds the template version, parameter/configuration digests, Evidence obligation, executor identity, and required guards.
- The initial `resource_usage_limit` template supports exact model-token, cost, latency, changed-file, and trace-byte limits over one Decision attempt, Planning attempt, Implementation Assignment, or Implementation attempt. It requires a matching exact Runtime capability snapshot and complete candidate-bound `resource_usage` Evidence.
- Hard resource clauses may produce Decision, Planning-feasibility, and Implementation-usage Custom Code Checks plus Runtime-derived preflight, meter, and cancellation guards from the same exact Check binding. Missing estimate, measurement, or enforcement capability fails closed or yields `indeterminate`.
- Accepted ordering preferences compile into exact protected Standard/source/passage-bound triage bindings. Runtime owns closed lexicographic precedence and comparator directions; models cannot author rank, weights, or opaque scores. The bindings affect only the deterministic Backlog Triage Projection, and lower order is not Check failure or Planning priority.
