...
Settings includes Assurance / Standards and Checks without creating a fifth primary destination. Its library groups Custom Checks by source User Standard and Check Type and keeps exact source freshness, passage, Default Check coverage, evaluator kind, applicability, lifecycle, definition digest, guard capability, latest Result, Evidence gaps, and estimated cost legible. Authoring uses progressive disclosure: paste text or select a source first; inspect distilled clauses and proposed Checks second; refine applicability, refs, repair guidance, and evaluator details third; review generated protected-config diff and policy consequences before acceptance. Draft/active/disabled are text-labeled states. Activation is an authenticated guarded mutation because every applicable active Custom Check becomes required. Model confidence never substitutes for Result or authority.
