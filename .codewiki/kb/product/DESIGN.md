...
**Context and history inspection** distinguishes refreshable producer Project Material Generations from frozen Candidate Gate Evaluation Packages. It exposes generation/package identity, source-linked local queries, coverage, unknowns, truncation, staleness, controlled refreshes, and retained exact model-visible ledger ranges. Compaction summaries remain visibly non-authoritative projections. Optional programmatic query runs show exact generation, code digest, limits, canonical JSON output, and receipt without presenting an opaque persistent heap.
