...
Settings includes Assurance / Custom Checks without creating a fifth primary destination. Its library groups Custom Checks by Check Type and keeps name, atomic requirement, applicability, Knowledge refs, revision, rollout, route, latest exact Result, Evidence gaps, and estimated cost legible. Authoring uses progressive disclosure: type and requirement first; applicability, refs, repair guidance, and route details second; generated protected-config diff and policy consequences before save or promotion. Draft/observe/warn/require are text-labeled states, and `require` promotion presents explicit authenticated approval. Model confidence never substitutes for Result or approval.
