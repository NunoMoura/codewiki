...
Settings includes Assurance / Custom Checks without creating a fifth primary destination. Its library groups Custom Checks by Check Type and keeps name, atomic requirement, applicability, Knowledge refs, lifecycle, definition digest, route, latest exact Result, Evidence gaps, and estimated cost legible. Authoring uses progressive disclosure: type and requirement first; applicability, refs, repair guidance, and route details second; generated protected-config diff and policy consequences before save or activation. Draft/active/disabled are text-labeled states. Activation is an authenticated guarded mutation because every applicable active Custom Check becomes required. Model confidence never substitutes for Result or authority.
