...
Assurance / Standards and Checks is the primary source-to-assurance surface. It accepts bounded inline User Standards or exact user-selected source bindings, shows Runtime-sanitized snapshots and atomic passages, identifies Default Check coverage, and proposes Custom Model or approved-template Custom Code Checks. It previews applicability, Evidence, evaluator/guard capability, agent feedback, and estimated cost; shows exact source/definition digests and per-Check Assessment/Result history; and guards `draft | active | disabled` lifecycle. Every applicable active Custom Check is required. The browser sends expected-digest proposals to Project Runtime and renders generated Git-backed diffs. It never fetches privileged sources directly, stores policy in dashboard state, treats Standard text as a system prompt, or lets a policy-changing Candidate disable protected-base Checks.
