...
- Public Pi command surface is bounded to `/wiki-state`, `/wiki-resume`,
...
- Repo-local dogfood is explicit through `.pi/settings.json` package path `..`;
  no `.pi/extensions/codewiki.ts` shim is present.
...
- Enabled controlled repo-local dogfood in `.pi/settings.json` using `..`
  because Pi resolves relative package paths against the settings file, and in
  `.codewiki/config.json`.
- Updated readiness/docs/drift checks from "dogfood disabled" to
  "dogfood allowed only through the project-local package path".
