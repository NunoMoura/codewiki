...
For the CodeWiki source checkout itself, fast edit feedback is never enough to make a change registered. The self-dogfood re-enable gate in the extension contract has passed and repo-local Pi-tool dogfood is enabled under supervision. Pi `wiki_*` tools may be used only under the same invariant: every repo source/test/README/KB mutation must be covered by a durable decision/planning/implementation trace record or an explicit direct-implementation decision record, with current content proof, expected-byte/sequence guards, and guarded append evidence.
...
Decision hard gates cover table readiness, understood intent, route safety,
