...
System diagrams are the navigation spine for system knowledge. Each system `.md` file, except `system/overview.md` and `system/diagrams/README.md`, should map to at least one diagram ref once the vNext diagram-ref migration is enabled. Valid refs may target components, adapters, flows, domain entities, lifecycles, policy boundaries, artifacts, actors, or external systems. Diagram nodes can set `requires_doc` when a detail doc is mandatory; not every diagram node needs a prose doc.
