...
Runtime compares synchronized Git head and tree with accepted state and persisted Candidate custody. An exact Manifest match preserves controlled provenance; otherwise Runtime classifies the observed state as external, pauses guarded effects, fingerprints repository, base, head, tree, paths, and diff, and captures immutable code material under a Runtime-owned ref or worktree without mutating the user branch.
