...
type: Concept
title: API
description: "`src/api/**` is the stable package/source facade. Root exports are reduced to the core `wiki_*` facades and stable types. `src/pi/**` contains the Pi-native tool/command adapter exposed by package metadata for external installs and repo-local dogfooding. `src/cli/index.ts` remains a temporary development/test harness, not the normal product surface."
tags:
  - codewiki
  - system
  - api
