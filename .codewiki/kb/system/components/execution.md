...
Managed Execution is CodeWiki's sole shipped fully controlled agent and model execution path. Neutral internal ports describe Candidate production, Worker execution, research, Code Checks, Model Checks, cancellation, usage, structured output, repository access, and isolation. The concrete model adapter uses a pinned Pi SDK; CodeWiki makes no initial multi-engine execution promise.
...
Every managed Worker session uses an explicit ResourceLoader, tool allowlist, isolated agent directory, Runtime-owned worktree, CodeWiki context envelope, bounded model route, and disabled ambient prompts, extensions, skills, settings, and project-agent discovery. Every Model Check uses a separate tool-free session and route policy, with no Worker memory or fallback. Every Code Check runs through an admitted sandbox and never falls back to host execution. Execution receipts bind route or runtime, input, tool manifest where applicable, source base, timing, cancellation, usage, output, and isolation identity.
