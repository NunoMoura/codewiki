...
Backend Execution turns Runtime work into bounded Agent and Check runs. Agent Run Supervisor owns each logical Run lifecycle: immutable dispatch, exact Runner authentication, event and cancellation ordering, quiescence, exit proof, and custody-scoped receipt. A narrower process launcher handles OS spawn, private pipes, exit, and forced termination. Backend storage retains qualified content-addressed Runner Bundles and exact artifact bytes. Qualification verifies bytes; expected-generation CAS selects one active bundle for new Runs. Older bundles remain for bound resume and explicit rollback. DSH owns Turn Loop, streaming, tool pairing, session events, compaction mechanics, provider transport, and child-process coordination. CodeWiki owns Stage Loops, their inputs and artifacts, Gates, transitions, and effects.
