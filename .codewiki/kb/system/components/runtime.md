...
A detached project daemon now owns this service independently of Pi session lifetime. Pi clients use leased authenticated connections for runtime inspection and candidate-only submission; the service selects the exact invariant and rejects loop mismatch or runtime-owned candidate fields. Dashboard runtimes register separate observer clients. Two packed Pi RPC processes plus one dashboard have been proven against one generation, with supervisor loss pausing new execution and explicit shutdown removing daemon state.

Durable WorkState event delivery, autonomous SDK-backed semantic dispatch, implementation-worker scheduling, dashboard-service consolidation, and generation-aware draining of active jobs remain open. One-shot host callers remain implementation drift rather than project-wide ownership.
