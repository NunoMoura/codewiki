...
Current source has a detached local coordinator, local generation fencing, authenticated exact-revision Decision attention selection, local Change Trace writes, isolated worker/Integration primitives, and guarded branch/publication operations. Selection transport and the canonical attempt-start runtime are executable when a host supplies trusted authority resolution, current context, and canonical append capabilities. `createNativeDecisionAttemptExecutor()` now reloads fresh synchronized Git state, verifies the authenticated active attempt and exact protected-source/config-bound Exit Runtime, invokes one versioned candidate producer, materializes Candidate `2.0.0`, runs Code/Model/Evidence/Report/Route reduction, commits the continuation under exact CAS, and recovers terminal canonical attempts without reinvocation. Pi daemon startup accepts an injected complete `decisionStart` bundle, but no default trusted user-authority resolver, Git context/appender assembly, or Pi candidate producer is installed. Current policy snapshot and lease behavior protects only local process coordination. Source still uses expected-byte/local-sequence mutation, local ownership events, and partial filesystem recovery. Separate clones cannot coordinate accepted ownership. The clean cut replaces those contracts after pure protocol and two-clone Git experiments pass.
