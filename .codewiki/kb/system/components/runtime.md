...
Exact execution of every selected reaction inside the elected service, durable WorkState event delivery, Pi/dashboard migration, and generation-aware draining remain open. Until those land, process-local Pi reactors and one-shot host callers remain implementation drift rather than project-wide ownership.
