...
Executable protocols `codewiki.backlog-triage-policy@1.0.0`, `codewiki.backlog-triage-projection@2.0.0`, and `codewiki.backlog-triage-query@2.0.0` validate exact accepted Standard passages, protected config, WorkState, graph, and projection bindings. Runtime compiles active source-bound dimensions into fixed lexicographic comparator directions and precedence, retains per-dimension support and assumptions, derives no opaque score, and caps shared user/agent results at 100. Policy compilation, projection, and query are pure and disposable; repeated readers require no ownership and write no triage state. Authenticated `/v1/runtime/decision-attention` bootstraps a bounded query by binding the current projection server-side, then accepts optional strict `codewiki.backlog-triage-query@2.0.0` requests only for that exact digest. Drift and unsupported fields fail closed, and absent Decision host capabilities return `decision_attention_projection_unavailable`.

This projection recommends Decision attention only. It is not canonical priority, cannot disposition a Change, and cannot schedule implementation. `codewiki.decision-attention-selection@2.0.0` accepts only an authenticated user's strict command for one exact eligible current Change revision. The command carries a principal-scoped idempotency key, Change ID, content-addressed revision ID, and current projection digest; that digest already commits WorkState, triage Candidates, graph, protected config, and compiled policy. Runtime asks an injected authority verifier to authorize the exact immutable request, reloads context after authorization, and fails closed on stale or tampered identity.

Successful admission appends canonical `loop.attempt_started` before execution. Its authority binding, authentication Evidence ref, revision, base snapshot, and private idempotency digest are the durable selection record; its operation ID is also the coordinator job key. No separate selection receipt, receipt store, or duplicate job identity exists. Same principal/key and revision replay that operation, while different semantic input conflicts. The coordinator exposes a dedicated authenticated selection client method and a native Decision lane keyed by `changeRevisionId`. Pi exposes read-only `wiki_attention` and `/wiki-attention`, while only explicit user `/wiki-select` submits the strict command; there is no agent selection tool. The legacy numeric approval revision and generic candidate/trigger APIs cannot substitute for selection. Pending Changes alone leave Decision execution quiescent and do not suppress approved Planning solely because of possible overlap. Revision-derived source, Knowledge, component, and Change refs serialize conflicting attempts, while canonical attempt state supports recovery. Selection grants neither disposition nor Planning priority. Rolling Planning alone orders execution across accepted Changes.
...
Current source has a detached local coordinator, local generation fencing, authenticated exact-revision Decision attention selection, local Change Trace writes, isolated worker/Integration primitives, and guarded branch/publication operations. Selection transport and the canonical attempt-start runtime are executable when a host supplies trusted authority resolution. `createDecisionGitAdmission()` now supplies fresh Git context loading, exact protected-config triage policy/graph projection, short-lived context reuse across authorization revalidation, expected-WorkState attempt append, no blind stale retry, and canonical post-push verification. `createNativeDecisionAttemptExecutor()` reloads fresh synchronized Git state, verifies the authenticated active attempt and exact protected-source/config-bound Exit Runtime, invokes one versioned candidate producer, materializes Candidate `2.0.0`, runs Code/Model/Evidence/Report/Route reduction, commits the continuation under exact CAS, and recovers terminal canonical attempts without reinvocation. `createPiSdkNativeDecisionCandidateProducer()` now validates the exact authority-free request, runs one isolated read-only Pi SDK session, accepts one strict proposal, and propagates cancellation through abort and disposal. `codewiki.pi-native-decision-host@2.0.0` now composes Git admission, native Pi production, protected-config Exit Runtime construction, bounded trusted-host research collection plus isolated claim-support transport, continuation commit, and recovery when the host supplies trusted repository identity, current project authority, replay policy, and Runtime continuation authority. Its default resolver accepts only approved project-local Pi coordinator connections, persists only hashed principal/authentication Evidence identity, and permits additional project authorization denial. Pi daemon `nativeDecision` installs this bundle and cannot coexist with a raw `decisionStart`. This is local coordinator authentication, not external user/provider identity. Other policy snapshot and lease behavior still protects only local process coordination. Source still uses expected-byte/local-sequence mutation, local ownership events, and partial filesystem recovery. Separate clones cannot coordinate accepted ownership. The clean cut replaces those contracts after pure protocol and two-clone Git experiments pass.
