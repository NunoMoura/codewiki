...
This projection recommends Decision attention only. It is not canonical priority, cannot disposition a Change, and cannot schedule implementation. `codewiki.decision-attention-selection@1.0.0` accepts only an authenticated user's strict command for one exact eligible current Change revision. The command binds its idempotency key to the current native WorkState digest, projection digest, protected project-config digest, triage-policy digest, triage Candidate digest, Change ID, and content-addressed revision ID. Runtime validates the projection against WorkState, asks an injected authority verifier to authorize the exact immutable request, reloads the complete context after authorization, and fails closed on stale or tampered identity.

The resulting content-addressed receipt retains authentication Evidence, authority, exact Git/source/Knowledge/config/policy/graph bindings, deterministic conflict refs, and one native revision-bound Decision job ID. Same-key/same-input calls replay one receipt; same-key/different-authority or semantic input conflicts. The coordinator exposes a dedicated authenticated selection client method and a native Decision lane keyed by `changeRevisionId`; the legacy numeric approval revision and generic candidate/trigger APIs cannot substitute for selection. Pending Changes alone now leave Decision execution quiescent and do not suppress approved Planning solely because of possible overlap. Once selected, the coordinator serializes overlapping source, Knowledge, component, or Change scopes, revalidates the exact current selection before execution, and supports durable job recovery. Selection grants neither disposition nor Planning priority. Rolling Planning alone orders execution across accepted Changes.
