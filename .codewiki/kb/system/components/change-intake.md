...
No opaque `overallScore` may hide safety, uncertainty, or effort tradeoffs. The view states why each item appears where it does. `codewiki.decision-attention-selection@2.0.0` lets an authenticated user choose any eligible current pending revision through one principal-scoped idempotency key, exact Change/revision identity, and current projection digest. The projection digest already commits the triage Candidates, native WorkState, graph, protected config, and compiled policy. Runtime revalidates context after authorization, appends canonical `loop.attempt_started`, and schedules that operation ID with revision-derived conflict refs. Selection starts one independent Decision attempt; it grants no Decision disposition or Planning priority. Pending Change presence and generic triggers do not select work.
