...
Candidate applicability uses Development stage and repository path overlap, derives language facts deterministically, and may use Change type or kind when known. Repository scopes are Git-relative exact files or directory prefixes. Absolute paths, parent traversal, symlink escape, regular expressions, and globs are rejected. Unknown or incomplete facts widen selection conservatively rather than omit a possibly applicable Check.
