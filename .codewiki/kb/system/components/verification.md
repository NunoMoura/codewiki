...
Both evaluator kinds return one bounded `pass`, `fail`, or `indeterminate` Observation with summary and findings or reason. They cannot create Results, claim authority, route Runtime, mutate state, or supply Runtime-owned identity. Runtime validates canonical Candidate, policy, Check, Invocation, bounds, and output binding before Verification records a boolean-measured Result retaining the Invocation digest. Invalid or unavailable output becomes redacted indeterminate; Runtime-owned drift fails admission. Runtime validates admissibility, not arbitrary evaluator truth.
