...
title: Clients
description: Owns software endpoints and deterministic User Interfaces without Runtime or Backend execution authority.
status: stable
...
Optional Pi integration exposes bounded reads and explicit User operations through the same Server and Runtime contracts. It does not own, import, or launch CodeWiki Backend, Server, Runtime, Agent Runner, or Check Runner lifecycle implementations, dynamically activate Stage Loop tools, or schedule semantic work from ambient chat, tool-result, startup, or resume events. Pi is one Client Integration rather than the product host or Backend Agent engine.
