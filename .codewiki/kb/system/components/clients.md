...
Check Packs have no dedicated CLI management surface. Users may edit `.codewiki/check-packs/**` directly, including through a user-controlled external agent that follows public documentation. The App exposes the same source files through stage and Pack navigation. A Model Check form captures requirement, pass, fail, feedback, input, measurement, threshold, model profile, and budget and writes `check.json` plus `CHECK.md`. A Code Check form captures common fields, accepts one `CHECK.mjs` upload, validates it, and may preview it in an admitted sandbox. Users may create, rename, edit, or delete any Pack, including every default.

The App marketplace follows Pi Package ergonomics: search npm packages tagged `codewiki-check-pack`, accept exact npm, Git, or local package sources, inspect declared resources, install selected Packs into `.codewiki/check-packs/**`, record resolved source integrity, and show exact update diffs. Installation and update are explicit User actions. CodeWiki never invokes a model to author or alter Check files autonomously.
