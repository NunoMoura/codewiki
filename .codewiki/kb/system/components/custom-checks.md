...
Policy acceptance keeps review and authority separate. `codewiki.custom-check-policy-review@2.0.0` binds one `pass | fail | indeterminate` review receipt to the exact mutation receipt, protected source/config base, complete proposed config digest, User Standard snapshots, Custom Check config digest, proposed definitions, authenticated reviewer, Evidence ids, and review time. Review `pass` is required but cannot authorize Git. `codewiki.custom-check-policy-acceptance@2.0.0` separately binds repository identity, configured remote/protected ref, exact reviewed proposal, authenticated acceptance authority, and Runtime authorization digest.
