...
Accepted User Standard snapshots, compiled triage preference bindings, and Custom Check configuration are project truth in protected Git-backed `.codewiki/config.json`, bound into Team WorkState, Backlog Triage policy where applicable, Runtime execution policy where applicable, and Resolved Exit Policy digests. Large/private source bytes remain external and content-addressed. No dashboard database, mutable Standard/Check registry, model memory, or provider object becomes canonical.
...
Policy acceptance keeps review and authority separate. `codewiki.custom-check-policy-review@5.0.0` binds one `pass | fail | indeterminate` review receipt to the exact mutation receipt, complete distillation receipt where applicable, selected proposal ids, approved Custom Code template bindings, activation capability snapshot where applicable, Standard and definition transitions, complete compiled triage preference bindings, protected source/config base, complete proposed config digest, authenticated reviewer, Evidence ids, and review time. Review `pass` is required but cannot authorize Git, activate draft Checks, or make pending triage behavior authoritative. `codewiki.custom-check-policy-acceptance@5.0.0` separately binds repository identity, configured remote/protected ref, exact reviewed proposal, authenticated acceptance authority, and Runtime authorization digest.
...
The initial executable template is `codewiki.custom-code-template@1.0.0` / `resource_usage_limit`. It accepts only an exact metric, scope, and positive maximum. Closed metrics are `model_tokens`, `cost_usd`, `latency_ms`, `changed_files`, and `trace_bytes`; closed scopes are `decision_attempt`, `planning_attempt`, `implementation_assignment`, and `implementation_attempt`. Runtime derives the unit, accounting window, required meter, full template/configuration identity, quantitative `resource_usage` Evidence obligation, and mandatory `preflight | meter | cancellation` enforcement. Capability Snapshot Protocol `codewiki.custom-code-capability-snapshot@1.0.0` admits only exact `codewiki.model-usage-meter`, `codewiki.git-change-meter`, or `codewiki.trace-size-meter` capabilities with matching metric, scope, configuration, environment, and enforcement support. Missing or mismatched capability blocks activation or Runtime route admission; missing, duplicate, stale, partial, wrong-candidate, or mismatched usage Evidence yields `indeterminate`.

Distillation may identify a source-bound Custom Code clause and describe template intent or capability needs, but it cannot author execution parameters. An authenticated bundle mutation must select the exact proposal and separately provide one approved closed template parameter binding. Runtime derives template version, parameter digest, binding digest, unit, accounting window, capability requirement, and guard requirements; review and mutation receipts retain the exact proposal-to-template binding.
...
→ show Default Check coverage, proposed Custom Checks, compiled triage dimensions, and unresolved clauses
→ review exact passages, evaluator kind, applicability, Evidence, ordering behavior, guards, and cost
...
A Standard preference such as “prioritize critical security regressions” compiles into `codewiki.backlog-triage-policy@1.0.0` rather than producing a passing/failing Check. The protected config binding retains the exact distillation receipt/clause, Standard/source/passage identities, and closed dimensions. Runtime owns one fixed lexicographic dimension order and comparator direction, merges duplicate dimension refs without adding weight, and applies the resulting criteria only after protected safety tiers. The model cannot author rank, weights, direction, or precedence. Lower order is not failure, and Planning priority remains separate.
...
User Standard schema `1.0.0` now materializes immutable bounded inline or HTTPS source snapshots with exact observed time, normalized content digest, atomic passage ids, stable Standard id/digest, credential/control/URI rejection, canonical byte ceilings, and tamper validation. Project config persists complete `userStandards[]`, protected `triagePreferences[]`, and `customChecks[]`. Custom Check schema `4.0.0` rejects every source-unbound proposal, requires exact accepted Standard id/digest and passage bindings, and cleanly distinguishes `model` definitions from approved-template `code` definitions; Standard or template changes therefore invalidate Check definition/config identity.

Protected Custom Check configuration schema `3.0.0` binds complete User Standard, triage preference, and Custom Check arrays through the exact project-config digest and protected snapshot. Check Catalog `10.0.0` emits active Standard-backed Custom Model or Custom Code Checks as required project-authority Checks with CodeWiki-owned execution, measurement, Evidence obligations, timeout, cost, dependencies, evaluator identity, and exact template binding where applicable. Resolved Exit Policy deterministically applies Loop, Change-kind, affected-layer, and path-scope filters and carries exact Standard refs and Custom Code template semantics into each binding. Decision Model Check Request Protocol `5.0.0` carries one exact `codewiki.custom-check-evaluator@1.0.0` binding into each independent request.
...
User Standard Distillation Protocol `2.0.0` (`codewiki.user-standard-distillation`) binds one retrieved source receipt, the exact kernel Default Check catalog, all closed Check Types, one configured route, and hard limits. Its request supplies the clean-cut triage vocabulary: severity, exposure, regression, urgency, risk of inaction, impact, strategic value, effort, confidence, freshness, and age fairness; source-unbound risk or priority fields are not executable ordering inputs. One fresh tool-free Pi session receives only that sanitized request. Runtime accepts exact source excerpts and preserves Default Check coverage, source-bound Custom Model proposals, inert Custom Code template intents and capability needs, non-Check triage preferences, fully specified quantitative Runtime guard proposals, and unresolved unsupported/ambiguous/contradictory/stale/partial/excluded/unavailable/negative/retracted/superseded clauses. Content-addressed receipts and deterministic review bundles bind every clause and generated proposal to the exact Standard passage. Distillation cannot activate policy, assign authority, produce Results, select ordering, install code, or mutate config.

The guarded `codewiki.custom-check-mutation@5.0.0` Runtime command supports strict Standard-backed Check create, update, activate, and disable actions plus `create_distilled_bundle`. Bundle creation accepts one exact completed distillation receipt, an authenticated bounded selection of generated proposal ids, and one separately approved closed template parameter binding for every selected Custom Code proposal, then atomically adds the immutable User Standard, every source-bound `triage_preference` binding, and selected draft Custom Checks through one complete-config CAS. Custom Code activation additionally requires one exact Runtime-observed capability snapshot. Authorization and content-addressed receipts bind the full distillation review bundle, selected ids, proposal-to-template bindings, activation capability snapshot digest where applicable, exact Standard/definition transitions, current/protected config, and protected source head. A Standard whose clauses map only to Default Checks, triage preferences, guards, or unresolved items may be accepted with zero generated Checks. The project adapter derives the complete canonical project-config digest, serializes cross-process writes with an exclusive lock, verifies persisted output, and can load Custom Check policy from an exact protected Git commit while working-tree changes prepare the next snapshot.

The executable review/acceptance boundary now uses `codewiki.custom-check-policy-review@5.0.0` and `codewiki.custom-check-policy-acceptance@5.0.0`. Review identity includes the complete distillation receipt, every selected proposal id, exact Standard/definition transitions, compiled triage preference bindings, and proposed complete config. Runtime authenticates the exact mutation and review receipts through separate verifiers, requires review `pass` plus separate authenticated acceptance authority, and rejects any receipt whose before-config digest differs from the exact protected-base config so stacked unreviewed working changes cannot hitchhike into acceptance. It binds repository/protected-ref/config identity, creates a deterministic config-only child commit, uses safe credential-free Git expected-head CAS, re-observes accepted bytes, rejects stale races without retry, and replays an already accepted exact commit. Generated Checks remain `draft`; a working-tree mutation, distillation, or passing review alone grants no protected or activation authority.

Resolved Exit Policy, the shared Loop-exit runtime, and native Decision runtime now accept only a validated `ProtectedCustomCheckConfigSnapshot`; the removed raw `customChecks` input is rejected. Active bindings carry the protected source head, complete project-config digest, Custom Check config digest, protected snapshot digest, Custom Check id, and `definitionDigest`. Decision Model Check Request Protocol `5.0.0` (`codewiki.decision.model-check-request`) carries those exact bindings plus accepted Standard snapshot identity and selected passage text, exact considered Evidence, prerequisite Result identities, evaluator configuration, and no-tool provider/model route. `codewiki.custom-check-evaluator@1.0.0` requires `fresh_no_shared_state` execution and one response that echoes the exact evaluator binding and prerequisite Results while bounding Evidence gaps, counterevidence, coverage, truncation, repair summary, and repair targets. Mismatched definition, route/configuration replay, stale Evidence, prerequisite drift, malformed fields, or oversized repair stays `indeterminate`. Focused model calls remain the execution baseline. Failing or indeterminate active Custom Checks block exit and remain available for bounded agent repair feedback.

Evidence schema `1.4.0` retains exact candidate-bound `resource_usage` records and Check-scoped approval plus residual-risk authority bindings, and requires exact request and Assessment digests for every model assessment. Custom Check assessments additionally persist evaluator/definition/Standard/prerequisite identity, Evidence gaps, counterevidence, coverage, truncation, and repair targets. The initial `resource_usage_limit` executor independently reduces one exact complete-window observation to satisfied, unsatisfied, or indeterminate, while Runtime exposes matching guard resolution, fail-closed preflight, live meter/cancellation decisions, and observed Evidence materialization. Shared and native Decision runtime composition installs these Custom Code executors; the shared runner rejects route admission when an applicable active resource guard lacks exact capability.

Standard replacement/redistillation remains pending; accepted source drift cannot silently rewrite existing Standards, triage bindings, or Checks. Dashboard/CLI/API source transport and policy-change review UI remain pending, as do production public-HTTPS and private source connectors, hosted-provider review publication adapters beyond the provider-neutral Git acceptance boundary, production meter collectors and full Loop scheduling of resource observations, sealed Check Type route calibration, Knowledge-content resolution, type-level batching/sharding, Planning/Implementation evaluator cuts, and production `wiki_decide` cutover. Generic dashboard config patching intentionally exposes neither User Standards nor Custom Checks as editable settings.
