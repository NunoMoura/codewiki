...
Knowledge exposes immutable bounded OKF bundle, concept, relationship, source-ownership, and test-ownership facts for snapshot-bound consumers. Backend Agent, delegated, and External Agent Client Stage Context plus the Check SDK query those same facts and traverse their Alignment projection; none creates a duplicate graph or context store. They cannot mutate Knowledge, infer missing desired state, or treat generated projections, conversation summaries, or execution scratch as accepted truth. Query results retain exact Knowledge digest, source references, deterministic ordering, coverage, unknowns, truncation, cursor position, query-engine identity, and staleness.
