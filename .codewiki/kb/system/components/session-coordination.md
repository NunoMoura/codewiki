...
A detached project daemon owns coordinator lifetime. Pi sessions register leased clients, use remote inspection for active-tool routing, submit semantic candidates through coordinator jobs, heartbeat while active, reconnect after generation loss, and disconnect on `session_shutdown`. Interactive and RPC Pi sessions are explicit approved supervisors; print/JSON sessions are observers. Dashboard runtimes register distinct observer clients. A disposable packed spike proves two real Pi RPC processes and one dashboard share one generation, and that execution pauses after both supervisors exit. Durable project event delivery, autonomous SDK session dispatch, and implementation-worker lifecycle remain open.
