...
The executable `codewiki.host-pairing@1.0.0` boundary sends an opaque transient proof only to a named trusted authentication adapter and accepts only the exact normalized assertion shape. The assertion must bind the requested Client kind and instance; it cannot supply an actor, role, authority, Runtime time, credential, or effect. Pairing issuance derives the actor exclusively from one active authenticated-identity mapping, rejects duplicate pairing, proof-reference, and active Client-instance bindings, and optionally derives a bounded expiry from a Host-owned clock and requested lifetime. Revocation requires fresh trusted authentication mapped to the owning actor and exact pairing/authentication-reference CAS. Both transitions bind the current registry generation, advance it once, preserve historical records, and leave canonical persistence to the registry writer. Raw proof material is never added to registry state.
