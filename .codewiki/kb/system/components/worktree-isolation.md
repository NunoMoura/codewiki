...
disabled, so runtime claim selection and implementation loops never mutate Git
automatically. Default worktree roots are project-local under
`.codewiki/runtime/tmp/<trace-id>/worktree/**`; custom hosts may still pass an
explicit `worktreeRoot` when their sandbox allows it.
