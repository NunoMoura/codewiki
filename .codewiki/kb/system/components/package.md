...
CodeWiki ships as one standalone local-first product whose durable service is the CodeWiki Backend. The Backend contains CodeWiki Server, one authoritative Project Runtime per governed project, semantic owners, Agent Run Supervisor, isolated DSH-based CodeWiki Agent Runners, Check Runners, Workbench management, and first-party App and CLI entrypoints. It may run quietly as a local daemon or later as a hosted service, but it runs behind the project rather than inside Claude Code, Codex, Pi, DSH's product shell, or another Agent product.
