...
codewiki_relationships:
  ...
  - type: realizes
    ...
    rationale: Package transports self-contained authored Checks and their optional Pack Skills without becoming an execution environment.
---
# Package
...
Package sources are transport rather than execution environments. Installation runs no package, Skill, or Check code, imports no credentials, and grants no lifecycle or effect authority. Skill scripts and setup instructions are ordinary vendored Agent Skill resources and may run later only when an admitted work-producing Agent invokes them through its existing Assignment capabilities. Marketplace Code Checks are prebundled and later run only in the admitted sandbox; Model Checks later run only through isolated configured routes. Users may edit or delete installed files immediately. Updates are explicit and never overwrite local changes silently. Default Packs ship as package resources and are materialized only once at project bootstrap; deleting them is supported and upgrades do not restore them.

Package installation cannot introduce credentials, telemetry, install hooks, canonical writes, unsandboxed execution, protected Check floors, or fixed lifecycle changes. Benchmarks, private state, and source-checkout dogfood machinery do not ship in the CodeWiki product package. Packed candidates are tested only in disposable external projects.
