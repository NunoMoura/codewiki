...
`codewiki.standard-evidence-check-executor@1.0.0` installs trusted report-bound capabilities into the native Loop Exit Runtime. One capability names an exact Loop, Catalog Check/version, every obligation owned by that Check, normalized selector, adapter receipt, and materialized bundle. The Resolved Exit Policy must contain the same selector under the protected `standardEvidenceCheckSelector` parameter. Runtime rechecks Candidate/source identity, produces immutable Evidence Records and all obligation resolutions, maps evaluator disposition into the Check's boolean measurement, and alone creates the Result and Exit Report. Report-bound execution is not generically cached. Duplicate Check capabilities, duplicate bundle assignment, partial obligation ownership, non-Code Checks, permissive obligations, selector drift, cancellation, and source mismatch fail closed. Check Catalog `7.0.0` requires artifacts for standard external-command Evidence obligations.
