...
The shared implementation is named **Verification**, not a fourth Loop and not a second copy of Decision, Planning, or Implementation:
...
Verification owns shared Check, Evidence-obligation, policy-binding, Result, Exit Report, runner, and cache mechanics. It cannot import Runtime or a Loop implementation. It receives generic execution ports and returns bounded execution facts; it cannot select a semantic job, construct a Loop Candidate, interpret Loop meaning, or choose Runtime Route.

Each semantic Loop owns its Candidate, Check, attempt, and route modules under `src/decision/**`, `src/planning/**`, or `src/implementation/**`. A Loop-local `exit/**` directory is allowed only for those Loop-specific bindings; it cannot reimplement shared Verification mechanics. No `src/runtime/loop-exit-runtime.ts` facade remains. Runtime supplies generic ports for identity, persistence, bounded execution, cancellation, final freshness/CAS, and effects; it does not own Check policy or semantic evaluation.

`src/verification/**` now owns shared Verification. The clean move carried every caller, test, ownership record, package script, and export from former `src/loop-exit/**` without an old-path re-export. `src/runtime/loop-exit-runtime.ts` remains migration debt until Loop composition and generic Runtime ports separate.
...
Executable `codewiki.verification-capability-matrix@2.0.0` projects every Loop-qualified Default or active Custom Check from exact Check Catalog/config identity. Each immutable row reports execution availability (`native | host_required | capability_required`), current Evidence obligations, trusted-producer/capability gaps, candidate format adapters, and its own digest. Native canonical Evidence admission is distinct from collection. All nine bounded core adapters now ingest SARIF 2.1, common JUnit XML, LCOV, Cobertura, CycloneDX 1.7 JSON, SPDX 2.3 JSON, Pact 4.0 JSON, OpenAPI 3.0/3.1 JSON or YAML, and authenticated provider Check receipts under exact Runtime-owned bindings. Playwright execution uses JUnit plus CodeWiki UI captures, while axe findings use SARIF; tool-native Playwright and axe schemas are not core interchange contracts. Every format has `grantsResult: false`; formats measure or transport Evidence only, while CodeWiki owns applicability, semantic reduction, Result identity, and exit authority.

`codewiki.evidence-adapter.materialization@1.0.0` verifies one accepted core-adapter receipt and converts its bounded command/source material into immutable Evidence Records under Runtime-fixed source subject, protocol producer identity, observed or verified authority ceiling, coverage, exact source freshness, project sensitivity, and protocol/receipt/binding provenance. A deterministic helper reduces records from exact Check-accepted protocols through normal Evidence obligations while retaining wrong-protocol records as neutral considered input. Complete observations become `ready` even when they contain a factual test/provider failure; partial, unavailable, drifted, tampered, wrong-protocol, or duplicate observations remain missing or `indeterminate`. Readiness is not a verdict, and neither the adapter nor bridge can create a Result.

`codewiki.standard-evidence-check-evaluation@1.0.0` consumes one exact ready adapter bundle under a closed protected selector. JUnit selectors enforce minimum tests, zero failures/errors, and maximum skips; LCOV/Cobertura selectors compare line, branch, or function hits to integer basis-point thresholds without floating-point ambiguity; SARIF selectors count explicitly blocked finding levels; provider selectors compare a completed authenticated conclusion against an explicit accepted set; and structured artifact selectors require a minimum identity count for CycloneDX, SPDX, Pact, or OpenAPI. A selector cannot consume another adapter family. Obligations must require complete exact-Candidate Evidence and a non-optional artifact. Evaluation emits a digest-bound disposition and factual counts only; Runtime still creates the Check Result. Structured identity satisfaction remains inventory/contract presence, never security, license, verification, compatibility, or conformance proof.

`codewiki.standard-evidence-check-executor@1.1.0` installs trusted report-bound capabilities into the native Loop Exit Runtime. One capability names an exact Loop, Catalog Check/version, every obligation owned by that Check, normalized selector, adapter receipt, and materialized bundle. The Resolved Exit Policy must contain the same selector under the protected `standardEvidenceCheckSelector` parameter. Runtime rechecks Candidate/source identity, binds receipt and bundle digests into execution identity, maps evaluator disposition into the Check's boolean measurement, and alone creates the Result and Exit Report. Multiple independent Checks in one Loop may consume one exact bundle. Loop Exit Runner `1.3.0` records each byte-identical record once, reuses exact replay input without recording it again, and rejects same-id content conflicts; each Check independently resolves its obligation and receives its own Result. One `onCheckMaterialized` callback now carries an executed Result and every newly produced Evidence Record together, replacing split callbacks so a trusted host can durably commit one atomic materialization unit. This run-level coordination adds no Evidence store. Report-bound execution is not generically cached. Duplicate Check capabilities, inconsistent shared-substrate bindings, partial obligation ownership, non-Code Checks, permissive obligations, selector drift, cancellation, and source mismatch fail closed. Check Catalog `10.0.0` retains required artifacts for standard external-command Evidence obligations.

`codewiki.production-security-collector@2.0.0` installs fixed Semgrep SARIF static-analysis, Gitleaks directory SARIF secret-detection, and offline Trivy filesystem/advisory SARIF adapters behind the existing closed scanner suite. Collector identity commits exact executable bytes/version, Semgrep configuration, Gitleaks rules plus ignore policy, or Trivy database, Candidate source snapshot/tree/environment, fixed arguments, and closed process environment. Gitleaks scans only the exact Candidate directory, emits SARIF to stdout, redacts secrets, disables archive and recursive decode scanning, and cannot consume ambient configuration. Runtime verifies bound files before and after execution, probes version, disables metrics/version/database updates, bounds output and time, and feeds accepted bytes through `codewiki.evidence-adapter.sarif@1.0.0`. Missing, drifted, cancelled, timed-out, oversized, unsuccessful, malformed, or source/advisory-mismatched collection remains unavailable or partial. No arbitrary command, shell, plugin, implicit network access, scanner authority, or Result enters the contract.

SARIF ingestion hashes at most 4 MiB of exact bytes, accepts at most 32 exact tool-bound runs, processes at most 8,192 findings, and retains at most 256 compact observations/refs. Raw messages become digests. Only project-relative locations enter Evidence; unsafe URI/absolute locations are excluded and force partial coverage. Tool mismatch, malformed context, duplicate JSON keys, excessive nesting/node counts, unsupported caller authority, and unsafe artifact refs fail closed. Output contains `command_execution` and `source_observation` material plus a canonical receipt, never Runtime subject/time/authority or a Result.
...
CycloneDX and SPDX ingestion share a 4 MiB, 64-depth, 100,000-node JSON boundary and bind exact source snapshot, inventory scope, 1..256 project-relative source paths, up to 512 required identities, tool/version, request/invocation/environment/configuration identity, and execution state. CycloneDX accepts exact 1.7 JSON and preserves bounded components, services, dependency edges, vulnerabilities, licenses, and composition completeness. SPDX accepts exact 2.3 JSON and preserves bounded packages, files, snippets/ranges, relationships, external references, file-analysis state, and license assertions. Names, locators, file names, vulnerability identifiers, and document content enter only identity digests or the external artifact. Truncation, missing identities, unresolved references, external SPDX documents, or declared incomplete composition remains partial. An SBOM is inventory Evidence; it does not independently prove vulnerability absence, license compliance, provenance, or requirement completeness.

Pact ingestion accepts exact V4 JSON consumer-provider contract files and preserves bounded interaction types, pending/plugin state, provider-state counts, and matching-rule counts through digest identities. It does not interpret a Pact file as provider verification success; that requires separate JUnit or authenticated provider-check Evidence. OpenAPI ingestion accepts 3.0.0 through 3.0.4 and 3.1.0 through 3.1.1 in duplicate-key-safe JSON or alias/tag-free YAML. It inventories bounded paths, operations, callbacks, webhooks, schemas, security schemes, servers, and internal/external refs without persisting names, paths, examples, URLs, or bodies. External or dangling refs remain partial. OpenAPI presence is contract content, not implementation conformance, compatibility, security, or runtime behavior proof. Both adapters emit collection `command_execution` plus contract `source_observation` material and never Results.
...
`codewiki.custom-check-evaluator@1.0.0` binds each focused Custom Model Check to one exact Candidate, Check/definition, protected configuration, accepted Standard snapshot and selected passage text, considered Evidence set, prerequisite Result set, no-tool route, and evaluator configuration. Decision Model Check Request Protocol `5.0.0` requires fresh no-shared-state execution and validates one response against that exact binding. Evidence schema `1.4.0` persists the request and Assessment digests, evaluator identity, Standard and prerequisite digests, Evidence gaps, counterevidence, coverage, truncation, and repair targets. Wrong identity, stale route/configuration replay, prerequisite drift, malformed output, or oversized repair remains `indeterminate`; evaluator output grants neither Result nor authority.
