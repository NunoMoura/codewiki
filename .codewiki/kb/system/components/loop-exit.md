...
Requirement origin, project/kernel authority, execution kind, Check Type, measurement shape, and enforcement are independent:
...
`observe` and `warn` Results remain visible but do not block exit. Kernel Checks are required according to CodeWiki's closed policy and cannot be disabled. Custom Checks progress from draft, to `observe`, to `warn`, to explicitly approved `require`. A Candidate changing Custom Check configuration is evaluated under the protected-base policy snapshot and cannot weaken its own assurance.
