...
Current `src/loop-exit/**` is a migration path. One clean move replaces it with `src/verification/**`; every caller, test, ownership record, and export moves in the same slice with no old-path re-export.
