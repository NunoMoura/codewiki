...
Current source still uses legacy Quality Standard, Assessment, gate, graph, judge, profile, pack, and review vocabulary. The unused native contracts, catalog, and resolver have moved directly to `src/loop-exit/**` without old-path exports and now depend on the persistence-independent `SemanticLoop` type. The catalog now owns kernel registration, assigns project authority to accepted project definitions, rejects every caller-declared authority field, and cannot be replaced through resolver input. Legacy Standard/Policy contract names remain until exact identity hardening. `src/loop-exit/identity.ts` now provides strict canonical JSON, lowercase SHA-256 digest validation, deeply frozen canonical values, and a Runtime-owned candidate envelope whose deterministic identity binds normalized content and exact observed-base digests/refs; existing policy identity uses the same primitive. Loop-specific content schemas and admission remain pending. Identity-only declarations live under each Loop's `exit/**` directory, and `src/runtime/loop-exit-runtime.ts` composes them into one frozen `LoopExitSuite` with the closed catalog. These declarations do not yet register role-specific candidates/Checks or run production exit. Production Decision, Planning, and Implementation still execute separate legacy paths; exact declarations, the native runner, and clean Loop cuts remain pending.
