...
Check Catalog `8.0.0` retains protected Code Check `security_scanners_valid` before `security_privacy_reviewed` and adds five protected atomic scanner Checks. The closed `codewiki.security-scanner-suite@2.0.0` selects static analysis as the baseline plus dependency advisory, secret detection, authorization testing, configuration, or migration testing from exact surfaces. Runtime sends strict bounded requests carrying exact Candidate, source snapshot/tree, environment, adapter version/configuration, source/Knowledge/ownership refs, and fresh advisory snapshots. It materializes command and source observations with `observed` authority and exact scanner-family, request, and outcome provenance. `codewiki.atomic-security-scanner-check@1.0.0` consumes only Evidence IDs from the exact `security_scanners_valid` dependency and separately reduces static-analysis, dependency-advisory, credential-exposure, authorization-control, and persistence-safety obligations. Each active Check receives an independent Runtime Result, so one scanner family may pass while another fails or remains `indeterminate`. Scanner findings cannot grant canonical severity or Result authority and are separately converted into sanitized `security_scanner` intake material. Missing scanners, execution errors, malformed observations, partial coverage, unavailable execution, and stale advisories are `indeterminate`. Scanner Results complete before the independent challenge, whose focused request receives exact produced Evidence and prerequisite Result. Persisted scanner Evidence replays only against the same request and source/environment/advisory identity; changed scan context forces execution rather than generic Result-cache reuse. `codewiki.production-security-collector@1.0.0` now supplies two closed real-tool profiles: fixed Semgrep SARIF static analysis and fixed offline Trivy filesystem/advisory SARIF. Each profile binds exact executable bytes and version, Semgrep configuration or Trivy database bytes, source snapshot/tree/environment, and fixed invocation identity; uses structured no-shell arguments and a credential-free environment; disables metrics, version checks, and database updates; bounds time/output; and rechecks bound files after execution. Accepted output passes through the bounded SARIF adapter before sanitized scanner intake. Absence, drift, mismatched advisory identity, cancellation, timeout, overflow, nonzero exit, malformed output, and post-run mutation remain unavailable or partial Evidence. No caller command, argument, plugin, implicit network update, authority, or Result is admitted.
