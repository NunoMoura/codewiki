...
Exact high/critical Decision risk now activates `security_independent_challenge_reviewed` and `security_residual_risk_authorized`. The independent challenge runs on a distinct no-tool provider/model route and emits separate asserted Candidate-bound Evidence. Evidence schema `1.3.0` scopes approvals to exact Check/version/purpose. A residual-risk receipt requires a qualified security/risk role, an authenticated identity different from general Candidate approval, supported primary and independent assessments from distinct route/configuration/producer identities, exact Candidate risk, prior approval identity, and digest-only rationale/finding bindings. Missing or disagreeing assessment, same-route/provider-model reuse, generic approval reuse, same authenticated identity, and wrong-Candidate Evidence remain `indeterminate`; no model or scanner can authorize risk.
