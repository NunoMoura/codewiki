...
Authenticated coordinator clients may bootstrap one bounded `codewiki.backlog-triage-query@2.0.0` result from the current projection, then issue only exact projection-bound follow-up queries. Pi provides read-only `wiki_attention` and `/wiki-attention`; only an explicit user `/wiki-select <change-id> --revision <revision-id> --projection <digest>` may submit selection. No model-callable selection tool exists, and unavailable native host capability fails before projection or selection.
...
Security challenge output has `asserted` authority. It cannot assign canonical CVSS, verify exploitability, accept residual risk, pass its own Check, or create canonical Change priority. High/critical policy requires independent model routes and separately authenticated residual-risk authority; deterministic reproduction and qualified research Evidence remain applicable where policy activates them. Candidate producer and security challenge Model Checks never share conversational state.

Exact high/critical Decision risk now activates `security_independent_challenge_reviewed` and `security_residual_risk_authorized`. The independent challenge runs on a distinct no-tool provider/model route and emits separate asserted Candidate-bound Evidence. Evidence schema `1.4.0` retains exact Check/version/purpose approval scoping and adds exact model request/Assessment identity. A residual-risk receipt requires a qualified security/risk role, an authenticated identity different from general Candidate approval, supported primary and independent assessments from distinct route/configuration/producer identities, exact Candidate risk, prior approval identity, and digest-only rationale/finding bindings. Missing or disagreeing assessment, same-route/provider-model reuse, generic approval reuse, same authenticated identity, and wrong-Candidate Evidence remain `indeterminate`; no model or scanner can authorize risk.
...
Check Catalog `10.0.0` retains protected Code Check `security_scanners_valid` before `security_privacy_reviewed` and adds six protected atomic scanner Checks. The closed `codewiki.security-scanner-suite@3.0.0` selects static analysis as the baseline plus dependency advisory, secret detection, infrastructure configuration, authorization testing, or migration testing from exact surfaces. Runtime sends strict bounded requests carrying exact Candidate, source snapshot/tree, environment, adapter version/configuration, source/Knowledge/ownership refs, and fresh advisory snapshots. It materializes command and source observations with `observed` authority and exact scanner-family, request, and outcome provenance. `codewiki.atomic-security-scanner-check@2.0.0` consumes only Evidence IDs from the exact `security_scanners_valid` dependency and separately reduces static-analysis, dependency-advisory, credential-exposure, infrastructure-configuration, authorization-control, and persistence-safety obligations. Each active Check receives an independent Runtime Result, so one scanner family may pass while another fails or remains `indeterminate`. Scanner findings cannot grant canonical severity or Result authority and are separately converted into sanitized `security_scanner` intake material. Missing scanners, execution errors, malformed observations, partial coverage, unavailable execution, and stale advisories are `indeterminate`. Scanner Results complete before the independent challenge, whose focused request receives exact produced Evidence and prerequisite Result. Persisted scanner Evidence replays only against the same request and source/environment/advisory identity; changed scan context forces execution rather than generic Result-cache reuse. Scanner vocabulary stops at six distinct measured families; parser, process, filesystem, cryptography, public-interface, and browser-trust concerns use applicable static-analysis, standard contract/test, or independent Model Checks. `codewiki.production-security-collector@2.0.0` supplies three closed real-tool profiles: fixed Semgrep SARIF static analysis, fixed Gitleaks directory SARIF secret detection, and fixed offline Trivy filesystem/advisory SARIF. Each profile binds exact executable bytes and version, Semgrep configuration, Gitleaks rules plus ignore-policy bytes, or Trivy database bytes, source snapshot/tree/environment, and fixed invocation identity; uses structured no-shell arguments and a credential-free environment; bounds time/output; and rechecks bound files before and after execution. Gitleaks redacts output, disables archive and recursive decode scanning, and cannot consume ambient configuration. Accepted output passes through the bounded SARIF adapter before sanitized scanner intake. Absence, drift, mismatched advisory identity, cancellation, timeout, overflow, nonzero exit, malformed output, and post-run mutation remain unavailable or partial Evidence. No caller command, argument, plugin, implicit network update, authority, or Result is admitted.
...
`commitNativeDecisionOperationSequence()` now synchronizes the exact team snapshot, requires the active canonical attempt operation, rejects stale team/WorkState bindings, derives the continuation base from the verified remote/source/Knowledge/config/policy snapshot, admits Candidate through attempt-end operations in one expected-head Git state batch, refuses blind stale retry, resynchronizes, and verifies every accepted continuation identity.

`createDecisionGitAdmission()` supplies the selection-side production Git boundary. Every load first synchronizes remote state, reads protected config from the exact team source commit, checks its digest, compiles Standard-derived triage policy, projects the Alignment Graph, and retains one short-lived projection identity across pre/post-authorization loads. Attempt append re-fetches state, verifies expected WorkState and every base snapshot field, performs one expected-head state push without blind retry, and accepts success only after exact operation re-observation. Repository identity, project authority snapshot, and replay admission policy remain trusted host inputs.

`createNativeDecisionAttemptExecutor()` connects the selected attempt to that path. It reloads fresh synchronized state, validates exact authenticated attempt/revision identity, and requires an Exit Runtime binding whose protected source head and project-config digest match the current team snapshot before any producer call. It then emits one bounded `codewiki.decision-candidate-production@1.0.0` request containing revision/relationship facts but no authority or Evidence, parses only strict disposition/rationale output, runs independent evaluation, persists supplied, Runtime-collected, and Check-produced Evidence, and commits under the pre-evaluation team/WorkState CAS. Canonical terminal attempt state drives recovery, so a crash after accepted commit cannot reinvoke the producer or evaluator. `createPiSdkNativeDecisionCandidateProducer()` validates the exact closed request before opening one isolated read-only Pi SDK session, admits exactly one strict proposal, applies byte/time limits, and propagates cancellation through abort and disposal; unsupported or authority-bearing request fields never reach the model. `codewiki.pi-native-decision-host@2.0.0` now assembles Git admission, native Pi production, exact protected-config Exit Runtime construction, canonical continuation, and restart recovery from mandatory trusted repository/project/replay/Runtime-authority inputs. Its default authority resolver accepts only approved project-local Pi coordinator connections, derives stable principal and connection-specific authentication Evidence ids without persisting private connection metadata, and applies an optional additional project authorization callback. Pi daemon `nativeDecision` installs this bundle and rejects simultaneous raw `decisionStart` configuration. Disposable two-clone service proof covers authorization denial, one exact producer run, complete canonical exit, restart, and no-reinvoke recovery. External identity/provider proof, production `runWikiDecide()` replacement, and legacy count-path deletion remain part of the clean Decision cut. Exact persisted citations and claim Assessments replay without collector or provider reinvocation; caller-owned freshness is rejected without a compatibility path.
