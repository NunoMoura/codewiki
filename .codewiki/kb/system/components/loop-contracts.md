...
For the CodeWiki source checkout itself, fast edit feedback is never enough to make a change registered. The self-dogfood re-enable gate remains closed: repo-local Pi-tool dogfood is currently disabled while the extension contract establishes a clean pinned baseline. Normal Pi file tools, tests, and Git remain the active workflow. After explicit re-enable, every repo source/test/README/KB mutation must be covered by a durable decision/planning/implementation trace record or an explicit direct-implementation decision record, with current content proof, expected-byte/sequence guards, and guarded append evidence. Changes to quality evaluation must be governed by the pinned baseline while the candidate evaluator remains non-authoritative.
