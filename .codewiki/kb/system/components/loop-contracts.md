...
For the CodeWiki source checkout itself, fast edit feedback is never enough to make a change registered. The pinned-baseline and disposable shadow gates have passed, but the self-dogfood re-enable gate remains closed: repo-local Pi-tool autoload is disabled until the reviewed controller has a reproducible project-local deployment. Normal Pi file tools, tests, and Git remain the active workflow. After explicit re-enable, every repo source/test/README/KB mutation must be covered by a durable decision/planning/implementation trace record or an explicit direct-implementation decision record, with current content proof, expected-byte/sequence guards, and guarded append evidence. Changes to quality evaluation must be governed by the pinned baseline while the candidate evaluator remains non-authoritative.
