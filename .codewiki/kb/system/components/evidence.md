...
Current `src/loop-exit/**` references are migration paths only. The clean move to `src/verification/**` carries every caller and test without an old-path re-export.
