...
`codewiki.decision.research-collection@1.0.0` now drives that boundary from production native Decision. One explicitly configured trusted-host connector receives the exact Candidate plus immutable collector id/version/configuration, sensitivity, citation-count, receipt-byte, and timeout limits. Runtime validates request correlation and closed availability state, derives observation time and freshness, fixes external-service producer and observed authority, and admits all receipt citations atomically. Connector failure, cancellation, timeout, malformed or contradictory receipt, partial coverage, wrong Change revision, or mixed replay boundaries yields no fabricated citation and cannot pass provenance. Raw pages, credentials, authentication, and network access remain outside core Runtime.
...
Production Decision still uses broad `sourceRefs` and `proofRefs`. Native Candidate/policy/report persistence now includes Runtime-collected citations and claim Assessments, and Pi Native Decision Host `2.0.0` schedules the trusted collector plus isolated claim transport. Replacement of production `runWikiDecide()` and deletion of ref-count sufficiency remain pending until the clean Decision cut.
...
Loop-owned packages declare admissible domain payload/obligation semantics. Target `src/verification/**` imports Evidence contracts to bind Check inputs and Results. Runtime supplies generic materialization, artifact/provider adapter ports, approval correlation, Change operation writes, and retention; Loop packages compose their own semantic attempts. Dependency direction stays one-way: Evidence code cannot import Decision, Planning, Implementation, Verification, Runtime, Pi, or an outer adapter.

Shared Verification now lives under `src/verification/**`. The clean move carried every caller and test from former `src/loop-exit/**` without an old-path re-export.
...
Canonical JSON/digest primitives live in `src/utils/canonical-json.ts` so Evidence and Verification identity share exact serialization without reversing the dependency from Verification into Evidence.
