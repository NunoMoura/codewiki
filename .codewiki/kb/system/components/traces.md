...
Every payload is a closed object with no extra fields. `?` marks the only optional fields. IDs for accepted Claims, attempts, Assignments, Integration attempts, review projections, publications, releases, deliveries, and outcomes are the creating operation ID; later payloads reference that ID rather than introducing a mutable secondary identity. Authenticated Decision attention stores a principal-scoped idempotency digest in `privateAttemptDigest`; the resulting `loop.attempt_started` operation ID is both durable selection identity and coordinator job key, so no separate selection receipt exists.
