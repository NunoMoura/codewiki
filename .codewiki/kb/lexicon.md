...
| Check | One project-owned requirement mapping one exact bounded input to one pass/fail boundary and one failure-feedback contract. | [Checks](system/components/checks.md) |
...
| Check Pack | Stage-local directory grouping project Checks for one Gate. | [Checks](system/components/checks.md) |
