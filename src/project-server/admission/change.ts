...
export interface ChangeIntakeProjectServer {
	readonly execute: (
...
export function createChangeIntakeProjectServer(options: {
...
async function executeIntake(
	options: Parameters<typeof createChangeIntakeProjectServer>[0],
	command: ChangeIntakeCommand,
	signal?: AbortSignal,
): Promise<ChangeIntakeReceipt> {
...
async function commitIntakeSequence(input: {
	readonly options: Parameters<typeof createChangeIntakeProjectServer>[0];
	readonly state: ProjectWorkState;
	readonly observation: SynchronizationObservation;
	...
	if (pushResult.status === "stale") {
		...
		throw new Error(
			"Change intake push became stale; Project Server must refetch and semantically reevaluate the material.",
		);
	}
...
function runtimeTimestamp(date: Date): string {
	if (!Number.isFinite(date.getTime())) {
		throw new Error("Change intake Project Server clock returned an invalid time.");
	}
