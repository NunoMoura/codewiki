...
export interface ProjectServerWorkUnitClaimCandidate {
	workUnitId: string;
	traceId: string;
	title: string;
	planningRefs: string[];
...
export interface ProjectServerHeldWorkUnitClaim
	extends ProjectServerWorkUnitClaimCandidate {
	reason: ProjectServerWorkUnitClaimHoldReason;
	conflictsWith?: string;
}

export interface ProjectServerWorkUnitClaimSelection {
	maxWorkers: number;
	activeClaims: ProjectServerWorkUnitClaimCandidate[];
	availableSlots: number;
	selected: ProjectServerWorkUnitClaimCandidate[];
	held: ProjectServerHeldWorkUnitClaim[];
}

export function selectProjectServerWorkUnitClaims(
	queue: WorkQueueView,
	...
	const selection: ProjectServerWorkUnitClaimSelection = {
		maxWorkers,
		activeClaims,
		availableSlots,
		selected: [],
...
function claimCandidate(item: WorkQueueItem): ProjectServerWorkUnitClaimCandidate {
	return {
		workUnitId: item.id,
...
function firstPathConflict(
	candidate: ProjectServerWorkUnitClaimCandidate,
	occupied: ProjectServerWorkUnitClaimCandidate[],
): ProjectServerWorkUnitClaimCandidate | undefined {
	return occupied.find((item) => pathScopesConflict(candidate, item));
}

function pathScopesConflict(
	left: ProjectServerWorkUnitClaimCandidate,
	right: ProjectServerWorkUnitClaimCandidate,
): boolean {
