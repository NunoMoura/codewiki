...
export interface AppServerSessionAuthorization {
	session: ProjectServerSessionRecord;
...
export function appEndpointRequest(
	method: string,
	pathname: string,
	repositoryIdentity: ProjectServerSessionRecord["project"]["repositoryIdentity"],
): ProjectServerEndpointRequest | undefined {
	const endpointId = APP_ENDPOINTS[`${method} ${pathname}` as keyof typeof APP_ENDPOINTS];
	if (!endpointId) return undefined;
	return Object.freeze({
		endpointId,
		method: method as ProjectServerEndpointRequest["method"],
		repositoryIdentity,
	});
}
...
export function revokeAppServerSession(
	...
	authorization.session = revokeProjectServerSession({
...
const appEndpointAuthorizationAdapter: ProjectServerEndpointAuthorizationAdapter = Object.freeze({
	adapterId: "codewiki.project-server-app-policy@1.0.0",
	authorize(input: ProjectServerEndpointAuthorizationContext) {
		return (
