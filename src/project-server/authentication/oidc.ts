...
import {markVerifiedProjectServerAuthenticationAssertion,
...
export const PROJECT_SERVER_OIDC_AUTHENTICATION_PROTOCOL = Object.freeze({
	id: "codewiki.project-server-oidc-authentication",
	version: "1.0.0",
} as const);

export interface ProjectServerOidcIdentity {
	readonly kind: "oidc";
...
export interface ProjectServerOidcClaims {
...
export interface ProjectServerOidcAuthenticationAdapter {
	readonly adapterId: string;
	verify(input: {
		readonly protocolId: typeof PROJECT_SERVER_OIDC_AUTHENTICATION_PROTOCOL.id;
		readonly protocolVersion: typeof PROJECT_SERVER_OIDC_AUTHENTICATION_PROTOCOL.version;
...
export interface VerifiedProjectServerOidcAuthentication {
	readonly assertion: ProjectServerAuthenticationAssertion;
	readonly identity: ProjectServerOidcIdentity;
	readonly adapterId: string;
}

export async function verifyProjectServerOidcAuthentication(input: {
	readonly adapter: ProjectServerOidcAuthenticationAdapter;
...
}): Promise<VerifiedProjectServerOidcAuthentication> {
	...
	try {
		untrusted = await input.adapter.verify(Object.freeze({
			protocolId: PROJECT_SERVER_OIDC_AUTHENTICATION_PROTOCOL.id,
			protocolVersion: PROJECT_SERVER_OIDC_AUTHENTICATION_PROTOCOL.version,
			proof: request.proof,
			expected,
		}));
	} catch {
	...
	const claims = normalizeProjectServerOidcClaims(untrusted);
	...
	const identity = projectServerOidcIdentity(claims.issuer, claims.subject);
	const assertion = normalizeProjectServerAuthenticationAssertion({
	...
	markVerifiedProjectServerAuthenticationAssertion(assertion); const verified = Object.freeze({assertion, identity, adapterId});
	verifiedOidcAuthentications.add(verified);
	return verified;
}

export function assertVerifiedProjectServerOidcAuthentication(
	value: VerifiedProjectServerOidcAuthentication,
): void {
	if (
		typeof value !== "object" ||
		value === null ||
...
export function projectServerOidcIdentity(
	issuerValue: string,
	subjectValue: string,
...
function normalizeProjectServerOidcClaims(value: unknown): ProjectServerOidcClaims {
...
function assertExpectedClaims(
	claims: ProjectServerOidcClaims,
	expected: {
		readonly clientKind: ClientKind;
		readonly clientInstanceId: string;
		readonly issuer: string;
...
function authenticationRequest(
	value: unknown,
): ProjectServerAuthenticationProof {
