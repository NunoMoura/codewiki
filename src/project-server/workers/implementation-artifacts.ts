...
export interface ImplementationWorkerDispatchPacket {
	...
	worktreePlan: ProjectServerWorktreePlan;
}
...
async function cleanupPacketWorktree(
	...
	try {
		...
		await executeProjectServerWorktreeCommands(prunePlan(packet.worktreePlan), {
			dryRun: false,
...
function prunePlan(plan: ProjectServerWorktreePlan): ProjectServerWorktreePlan {
	return {
		...plan,
		commands: {
			worktreePrepare: [],
