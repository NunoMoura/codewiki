...
import { appendProjectServerWorkUnitClaims } from "../claims/work-unit-events.ts";
import type { ProjectServerReactor, ProjectServerTrigger } from "../coordinator/reactor.ts";
...
export interface ImplementationWorkerProjectServerReconciliation {
...
export class ImplementationWorkerDispatcher {
	...
	reconcileProjectServer(
		trigger: ProjectServerTrigger,
	): Promise<ImplementationWorkerProjectServerReconciliation> {
	...
	private async reconcileOnce(
		...
		const complete = (
			...
			jobIds: string[],
			blockers: string[],
		): ImplementationWorkerProjectServerReconciliation =>
		...
		const runtimeInput: RunWikiProjectServerInput = {
		...
		const preview = await runProjectServer(runtimeInput);
		...
		await this.options.beforeAppend?.();
		const appended = await appendProjectServerWorkUnitClaims(claimBatch, {
			repoRoot: this.options.repoRoot,
			expectedBytesByTrace: observation.expectedBytesByTrace,
		});
...
function createDispatchPackets(input: {
	repoRoot: string;
	workState: WorkState;
	runtime: Awaited<ReturnType<typeof runProjectServer>>;
	isolationKind: ImplementationWorkerAssignment["isolation"]["kind"];
...
function preparedAdapter(
	...
	return {
		...
		async execute(assignment, signal) {
			...
			await executeProjectServerWorktreeCommands(plan, {
				dryRun: false,
				runner,
