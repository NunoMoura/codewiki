...
export interface ProjectServerDevLogItem {
	id: string;
	timestamp: string;
	workUnitId?: string;
	workerId?: string;
...
export interface ProjectServerDevLogProjection {
	available: boolean;
	entryCount: number;
	items: ProjectServerDevLogItem[];
}

export function projectServerDevLog(
	entries: DevLogEntry[] | undefined,
	limit = 200,
): ProjectServerDevLogProjection {
	if (!Number.isInteger(limit) || limit < 1 || limit > 1_000)
		throw new Error("Project Server Dev Log limit must be an integer from 1 to 1000.");
