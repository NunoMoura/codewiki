...
import { ProjectServerReactor, type ProjectServerObservation } from "../coordinator/reactor.ts";
import { assertProjectServerSemanticJobId } from "../../changes/trace/schema.ts";
import {
...
/** Project Server-only entry using the exact observation that selected this iteration. */
export async function runProjectServerSelectedWikiImplement(
	input: RunWikiImplementInput,
...
async function runWikiImplementFromObservation(
	...
	assertProjectServerSemanticJobId(input.runtimeJobId, "wiki_implement");
...
async function runtimeImplementationContext(
	input: RunWikiImplementInput,
	selectedObservation?: ProjectServerObservation,
): Promise<{
	input: PreparedWikiImplementInput;
	...
	const observation =
		selectedObservation ||
		(await new ProjectServerReactor(input.repoRoot).observe({
			kind: "manual_resume",
		}));
	...
	if (selection?.loop !== "implementation") {
		throw new Error(
			"Project Server did not select Implementation for current WorkState.",
		);
	}
...
function assertEvidenceDoesNotClaimProjectServerAuthority(
