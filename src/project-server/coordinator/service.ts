...
import {
	createDecisionStartProjectServer,
	type DecisionStartResult,
	type DecisionStartProjectServer,
	type DecisionStartProjectServerOptions,
...
import {
	...
	scheduleProjectServerReactions,
	type ProjectServerReactionJobReceipt,
} from "./reactions.ts";
...
export interface ProjectCoordinatorServiceOptions
	...
	now?: () => string;
	clock?: () => number;
	semanticAdapters?: ProjectServerSemanticAdapters;
...
export interface ProjectCoordinatorDecisionStartOptions
	extends Omit<DecisionStartProjectServerOptions, "coordinator" | "now"> {
	resolveAuthority(
...
export interface ProjectCoordinatorServiceHandle {
	...
	reconcileWorkers(trigger: ProjectServerTrigger): Promise<ImplementationWorkerDispatchResult>;
	close(): Promise<void>;
}
...
type ProjectCoordinatorRemoteTrigger = Omit<
	...
	"occurredAt"
>;
export type ProjectServerCandidateLoop = "decision" | "planning" | "implementation";
export type ProjectCoordinatorSemanticExecution =
	| "service"
	| "client_candidate";

export interface ProjectCoordinatorCandidateResult {
	receipt: ProjectServerReactionJobReceipt;
	execution?: RunProjectServerSelectedSemanticReactionResult;
}
...
export interface ProjectCoordinatorRemoteClient {
	...
	inspect(trigger: ProjectCoordinatorRemoteTrigger): Promise<ProjectServerReaction>;
	decisionAttention(
	...
	submitCandidate(
		...
		loop: ProjectServerCandidateLoop,
...
export async function startProjectCoordinatorService(
	...
	try {
		...
		const reactor = new ProjectServerReactor(canonicalRoot);
		...
		const decisionStart = decisionStartOptions
			? {
					...
					runtime: createDecisionStartProjectServer({
						coordinator,
						loadCurrentContext: decisionStartOptions.loadCurrentContext,
...
export async function connectProjectCoordinatorClient(
	...
	return {
		...
		appState(context) {
			...
			return requestCoordinatorJson<CodewikiAppState>(
				...
				"/v1/runtime/app-state",
				{
					...
					body: {
						connectionId: response.connectionId,
						context: normalizeClientProjectServerRequestContext(context),
					},
					timeoutMs: options.timeoutMs,
				},
			);
		...
		changes(context) {
			assertRemoteClientConnected(disconnected, response.clientId);
			return requestCoordinatorJson<ProjectServerChangesState>(
				endpoint,
				"/v1/runtime/changes",
				{
					method: "POST",
		...
		configuration(context) {
			assertRemoteClientConnected(disconnected, response.clientId);
			return requestCoordinatorJson<ProjectServerConfigurationState>(
				endpoint,
		...
		inspect(trigger) {
			assertRemoteClientConnected(disconnected, response.clientId);
			return requestCoordinatorJson<ProjectServerReaction>(
				endpoint,
				"/v1/runtime/inspect",
				{
					method: "POST",
		...
		react(trigger, mode = "append") {
			assertRemoteClientConnected(disconnected, response.clientId);
			return requestCoordinatorJson<ProjectServerReactionJobReceipt[]>(
				endpoint,
				"/v1/runtime/react",
				{
					method: "POST",
...
async function routeServiceRequest(
	...
	if (method === "POST" && url.pathname === "/v1/runtime/app-state") {
		await handleProjectServerProjection(runtime, request, response, loadProjectServerAppState);
		return;
	}
	if (method === "POST" && url.pathname === "/v1/runtime/changes") {
		await handleProjectServerProjection(
			runtime,
			request,
			response,
			loadProjectServerChangesState,
	...
	if (method === "POST" && url.pathname === "/v1/runtime/inspect") {
		await handleProjectServerInspection(runtime, request, response);
		return;
	}
	...
	if (method === "POST" && url.pathname === "/v1/runtime/candidate") {
		await handleProjectServerCandidate(runtime, request, response);
		return;
	}
	if (method === "POST" && url.pathname === "/v1/runtime/react") {
		await handleProjectServerReaction(runtime, request, response);
		return;
	}
...
async function handleProjectServerProjection<T>(
	runtime: ProjectServerService,
	request: IncomingMessage,
	response: ServerResponse,
	...
	const lease = requiredLease(runtime, text(body.connectionId));
	normalizeClientProjectServerRequestContext(body.context);
	lease.activeRequests += 1;
	try {
		await assertCurrentGeneration(runtime);
...
async function handleProjectServerInspection(
	runtime: ProjectServerService,
	request: IncomingMessage,
	response: ServerResponse,
): Promise<void> {
...
async function handleProjectServerCandidate(
	runtime: ProjectServerService,
	request: IncomingMessage,
	response: ServerResponse,
): Promise<void> {
	...
	try {
		...
		let execution: RunProjectServerSelectedSemanticReactionResult | undefined;
		const receipt = await scheduleProjectServerReactionJob({
...
async function handleProjectServerReaction(
	runtime: ProjectServerService,
	request: IncomingMessage,
	response: ServerResponse,
): Promise<void> {
	...
	try {
		const workerReconciliation = await runtime.workerDispatcher
			?.reconcileProjectServer(trigger)
			.catch(() => undefined);
		const receipts = await scheduleProjectServerReactions({
			repoRoot: runtime.endpoint.repoRoot,
			coordinator: runtime.coordinator,
			reactor: runtime.reactor,
			trigger,
...
async function reconcileWorkersAfterSemantic(
	...
	try {
		await assertCurrentGeneration(runtime);
		await runtime.workerDispatcher.reconcileProjectServer({
...
function extendLease(runtime: ProjectServerService, lease: RemoteClientLease): void {
	lease.activeRequests -= 1;
	lease.expiresAt = runtime.clock() + runtime.clientLeaseMs;
}
...
function sweepExpiredClients(runtime: ProjectServerService): void {
...
function runtimeTrigger(value: unknown, observedAt: number): ProjectServerTrigger {
	...
	return {
		kind: kind as ProjectServerTrigger["kind"],
		occurredAt: new Date(observedAt).toISOString(),
		...(refs.length > 0 ? { refs: [...new Set(refs)].sort(compareText) } : {}),
	};
}

function runtimeCandidateLoop(value: unknown): ProjectServerCandidateLoop {
	if (
		value === "decision" ||
		value === "planning" ||
...
function runtimeSemanticMode(value: unknown): ProjectServerSemanticMode {
