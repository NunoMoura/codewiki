...
import {
	...
	type ResolvedProjectServerConnection,
...
export async function resolveLocalAppServerConnection(input: {
	...
	const local = Object.freeze({
		...
		projectServerRouteRef: `project-server:project:${projectKey}`,
	});
	const proof = randomBytes(32).toString("base64url");
	const authentication = await verifyProjectServerAuthentication({
		request: {
			clientKind: "app",
			...
			projects: project
				...
				: [
						...(current?.projects ?? []),
						{
							...
							projectServerRouteRef: input.local.projectServerRouteRef,
							status: "active",
