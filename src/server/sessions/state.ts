...
import {
	...
	type OpenedServerSession,
	type ServerEndpointAuthorization,
	type ServerEndpointAuthorizationAdapter,
	type ServerEndpointAuthorizationContext,
	type ServerEndpointRequest,
	type ServerSessionBinding,
	type ServerSessionRecord,
} from "./contracts.ts";

export function openServerSession(input: {
	...
	return Object.freeze({
		credential,
		session: normalizeServerSessionRecord({
			protocolId: SERVER_SESSION_PROTOCOL.id,
			protocolVersion: SERVER_SESSION_PROTOCOL.version,
...
export function rotateServerSession(input: {
...
export function revokeServerSession(input: {
	readonly session: ServerSessionRecord;
	readonly credential: string;
	readonly expectedSessionGeneration: number;
	readonly now?: Date;
	...
	return normalizeServerSessionRecord({
		...session,
		generation: session.generation + 1,
		status: "revoked",
		updatedAt: now.toISOString(),
...
export async function authorizeServerEndpoint(input: {
	readonly session: ServerSessionRecord;
	readonly credential: string;
	readonly expectedSessionGeneration: number;
	...
	const context: ServerEndpointAuthorizationContext = Object.freeze({
...
export function normalizeServerSessionRecord(value: unknown): ServerSessionRecord {
	...
	if (
		input.protocolId !== SERVER_SESSION_PROTOCOL.id ||
		input.protocolVersion !== SERVER_SESSION_PROTOCOL.version
	) {
...
function normalizeBinding(value: unknown): ServerSessionBinding {
	...
	const project = exactObject(input.project, ["projectId", "repositoryIdentity", "runtimeRouteRef"], "session project");
	return Object.freeze({
		actor: Object.freeze({
		...
		project: Object.freeze({
			...
			runtimeRouteRef: text(project.runtimeRouteRef, "session.project.runtimeRouteRef", 4_096),
		}),
	});
}

function normalizeEndpoint(value: unknown): ServerEndpointRequest {
