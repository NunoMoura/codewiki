...
export const SERVER_REPOSITORY_ACCESS_PROTOCOL = Object.freeze({
	id: "codewiki.server-repository-access",
	version: "1.0.0",
	maximumEvidenceAgeSeconds: 900,
});

export type ServerRepositoryAccess = "accessible" | "inaccessible";

export interface ServerRepositoryAccessAdapterRequest {
	readonly protocolId: typeof SERVER_REPOSITORY_ACCESS_PROTOCOL.id;
	readonly protocolVersion: typeof SERVER_REPOSITORY_ACCESS_PROTOCOL.version;
	readonly identity: ServerOidcIdentity;
	readonly repositoryIdentity: Sha256Digest;
	readonly providerRepositoryRef: string;
}

export interface ServerRepositoryAccessAdapter {
	readonly adapterId: string;
	readonly providerId: string;
	readonly issuer: string;
	readonly check: (request: ServerRepositoryAccessAdapterRequest) => Promise<unknown>;
}

export interface ServerRepositoryAccessObservation {
	readonly authenticatedIdentityRef: string;
	readonly repositoryIdentity: Sha256Digest;
	readonly providerRepositoryRef: string;
	readonly access: ServerRepositoryAccess;
	readonly checkedAt: string;
	readonly expiresAt: string;
}

export interface VerifiedServerRepositoryAccess {
	readonly protocolId: typeof SERVER_REPOSITORY_ACCESS_PROTOCOL.id;
	readonly protocolVersion: typeof SERVER_REPOSITORY_ACCESS_PROTOCOL.version;
...
export async function checkServerProviderRepositoryAccess(input: {
	readonly adapter: ServerRepositoryAccessAdapter;
	readonly authentication: VerifiedServerOidcAuthentication;
	readonly repositoryIdentity: Sha256Digest;
	readonly providerRepositoryRef: string;
	readonly now?: Date;
	readonly maximumEvidenceAgeSeconds?: number;
}): Promise<VerifiedServerRepositoryAccess> {
	...
	try {
		untrusted = await input.adapter.check(Object.freeze({
			protocolId: SERVER_REPOSITORY_ACCESS_PROTOCOL.id,
			protocolVersion: SERVER_REPOSITORY_ACCESS_PROTOCOL.version,
			identity,
			repositoryIdentity,
...
export function assertVerifiedServerRepositoryAccess(
	value: VerifiedServerRepositoryAccess,
): void {
	if (
		typeof value !== "object" ||
		value === null ||
...
function normalizeObservation(value: unknown): ServerRepositoryAccessObservation {
...
function assertObservationTime(
	observation: ServerRepositoryAccessObservation,
	now: Date,
...
function access(value: unknown): ServerRepositoryAccess {
...
function evidenceAge(value: number): number {
	if (
		...
		value < 1 ||
		value > SERVER_REPOSITORY_ACCESS_PROTOCOL.maximumEvidenceAgeSeconds
	) {
