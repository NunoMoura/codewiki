...
} from "../../project-server/coordinator/daemon.ts";
import type { ProjectCoordinatorDecisionStartOptions } from "../../project-server/coordinator/service.ts";
import type { ImplementationWorkerAdapter } from "../../project-server/workers/implementation-adapter.ts";
import type { ProjectBranchMergeAuthority } from "../../project-server/effects/project-branch-merge.ts";
import type { ProjectBranchPushAuthority } from "../../project-server/effects/project-branch-push.ts";
import type {
	ProductPublicationAdapter,
	ProductPublicationPlan,
} from "../../project-server/effects/product-publication-contract.ts";
import type {
	ProductReleaseAdapter,
	ProductReleasePlan,
} from "../../project-server/effects/product-release-contract.ts";
import {
...
} from "../../project-server/coordinator/executor.ts";
import {
...
export type PiSemanticAdapterLoader = (
	repoRoot: string,
) => Promise<ProjectServerSemanticAdapters | undefined>;
...
export async function loadPiSemanticAdapters(
	...
	try {
		...
		const { createPiSdkProjectServerSemanticAdapters } = await import(
			"./sdk-semantic-session.ts"
		);
		return createPiSdkProjectServerSemanticAdapters({ repoRoot, piSdk });
	} catch (error) {
