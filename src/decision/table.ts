...
import {
	normalizeDecisionTableUserAction,
	type CodewikiDecisionTableRowInput,
	type CodewikiDecisionTableUserAction,
} from "./types.ts";
import type { WikiProject } from "../project/types.ts";
import { nowIso } from "../shared/utils.ts";

export type DecisionTableRowAction = CodewikiDecisionTableUserAction;

export type DecisionTableBatchRowAction =
	| "accept"
	| "reject"
	| "defer"
	| "alternative"
...
export interface CodewikiDecisionTableRowActionInput {
	row_id: string;
	action: DecisionTableBatchRowAction;
	row?: CodewikiDecisionTableRowInput;
	alternative?: string;
}

export interface CodewikiDecisionTableRowActionFailure {
	row_id: string;
	action: string;
	error: string;
}

export interface RuntimeDecisionTableRow extends CodewikiDecisionTableRowInput {
	id: string;
	user_action: DecisionTableRowAction | string;
	alternatives: string[];
}

export interface RuntimeDecisionTable {
	id: string;
	summary: string;
	source: string;
	status: "pending" | "compiled" | "archived";
	scope?: { kind: "roadmap" | "sprint" | "task"; id?: string };
	rows: RuntimeDecisionTableRow[];
	created_at: string;
	updated_at: string;
}

export interface RuntimeDecisionTablesFile {
	version: number;
	updated_at: string;
	tables: RuntimeDecisionTable[];
}

export interface CodewikiDecisionTableToolInput {
	repoPath?: string;
	action:
		| "propose"
		| "revise"
	...
	source?: string;
	scope?: { kind: "roadmap" | "sprint" | "task"; id?: string };
	rows?: CodewikiDecisionTableRowInput[];
	alternative?: string;
}

export function decisionTableStorePath(project: WikiProject): string {
	return resolve(project.root, ".codewiki/runtime/decision-tables.json");
}

export function normalizeDecisionTableRows(
	rows: CodewikiDecisionTableRowInput[] = [],
): RuntimeDecisionTableRow[] {
	return rows
		.map((row, index) => {
			...
			return {
				...
				status: normalizeDecisionTableUserAction(row.status, userAction),
...
export async function readRuntimeDecisionTables(
	project: WikiProject,
): Promise<RuntimeDecisionTablesFile> {
	try {
		const raw = JSON.parse(
			await readFile(decisionTableStorePath(project), "utf8"),
		);
		return {
			...
			tables: Array.isArray(raw.tables)
				? (raw.tables
						.map(normalizeRuntimeTable)
						.filter(Boolean) as RuntimeDecisionTable[])
				: [],
		};
	} catch {
...
export async function writeRuntimeDecisionTables(
	project: WikiProject,
	file: RuntimeDecisionTablesFile,
): Promise<void> {
	const path = decisionTableStorePath(project);
	await mkdir(dirname(path), { recursive: true });
	await writeFile(
		path,
		JSON.stringify({ ...file, updated_at: nowIso() }, null, 2) + "\n",
		"utf8",
	);
}

export async function executeDecisionTableAction(
	project: WikiProject,
	input: CodewikiDecisionTableToolInput,
) {
	const file = await readRuntimeDecisionTables(project);
	if (input.action === "list") return { changed: false, tables: file.tables };
	const now = nowIso();
	if (input.action === "propose") {
		const id = String(
			input.table_id || `DT-${now.slice(0, 10)}-${file.tables.length + 1}`,
		).trim();
		const table: RuntimeDecisionTable = {
			id,
			summary: String(input.summary || "Pending Decision Table").trim(),
			source: String(input.source || "wiki_decision_table tool").trim(),
			status: "pending",
			...(input.scope ? { scope: input.scope } : {}),
			rows: normalizeDecisionTableRows(input.rows || []),
			created_at: now,
			updated_at: now,
		};
		if (!table.rows.length)
			throw new Error("decision_table propose requires rows.");
		file.tables = [
			table,
			...file.tables.filter((existing) => existing.id !== id),
		];
		await writeRuntimeDecisionTables(project, file);
	...
	if (batchActions.length) {
		...
		return {
			changed: true,
			table,
			changed_refs: [decisionTableStorePath(project)],
			changed_row_ids: Array.from(
				new Set(batchActions.map((action) => action.row_id)),
			),
			failed_row_ids: [] as string[],
			failures: [] as CodewikiDecisionTableRowActionFailure[],
		};
	}
	if (input.action === "revise") {
		table.rows = normalizeDecisionTableRows(input.rows || table.rows);
	} else if (input.action === "archive") {
		table.status = "archived";
	} else {
		const action = singleRowAction(input);
		const row = table.rows.find((item) => item.id === action.row_id);
		if (!row) throw new Error(`Decision row not found: ${action.row_id}`);
...
function singleRowAction(
	input: CodewikiDecisionTableToolInput,
): CodewikiDecisionTableRowActionInput {
	const rowId = String(input.row_id || "").trim();
	if (!rowId) throw new Error("decision_table row action requires row_id.");
	...
	throw new Error(`Unsupported decision_table row action: ${input.action}`);
}
...
function validateBatchActions(
	table: RuntimeDecisionTable,
	actions: CodewikiDecisionTableRowActionInput[],
): CodewikiDecisionTableRowActionFailure[] {
	return actions
		.map((action) => validateRowAction(table, action))
		.filter((failure): failure is CodewikiDecisionTableRowActionFailure =>
			Boolean(failure),
		);
}
...
function validateRowAction(
	table: RuntimeDecisionTable,
	action: CodewikiDecisionTableRowActionInput,
): CodewikiDecisionTableRowActionFailure | null {
	...
	if (
		!["accept", "reject", "defer", "alternative", "edit"].includes(
			action.action,
		)
	) {
		return {
	...
	if (
		...
		!String(action.alternative || "").trim()
	) {
		return {
			row_id: rowId,
			action: action.action,
			error: "decision_table alternative requires alternative text.",
		};
	}
	if (action.action === "edit") {
		const edited = normalizeDecisionTableRows([
		...
		if (!edited.length) {
			return {
				row_id: rowId,
				action: action.action,
				error: "decision_table edit produced an invalid row.",
			};
		}
	}
	return null;
...
function applyRowAction(
	...
	if (action.action === "alternative") {
		...
		row.alternatives = normalizeStringList([
			...(row.alternatives || []),
			alternative,
		]);
...
function normalizeRuntimeTable(raw: any): RuntimeDecisionTable | null {
	if (!raw || typeof raw !== "object") return null;
	const id = String(raw.id || "").trim();
	if (!id) return null;
	const status = ["pending", "compiled", "archived"].includes(
		String(raw.status),
	)
		? raw.status
		: "pending";
	return {
		...
		rows: normalizeDecisionTableRows(raw.rows || []),
