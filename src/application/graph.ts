...
type GraphLensFamilyId = "decision" | "knowledge" | "work" | "execution" | "proof";

const DEFAULT_GRAPH_LENS_FAMILIES: Array<{ id: GraphLensFamilyId; label: string; summary: string }> = [
	{ id: "decision", label: "Decision", summary: "Approved intent, requirement rows, risk state, and semantic direction." },
	{ id: "knowledge", label: "Knowledge", summary: "Product/system docs, diagram refs, and source-backed knowledge context." },
	{ id: "work", label: "Work", summary: "Planning, roadmap, sprint, task, and coordination state." },
	{ id: "execution", label: "Execution", summary: "Code, tests, checks, implementation builds, and dirty working-set evidence." },
	{ id: "proof", label: "Proof", summary: "Validation, audit evidence, commits, publication, archive, and content proof." },
];
...
function graphLensFamilyForNode(node: any): GraphLensFamilyId | null {
	...
	if (kind === "decision_build" || kind === "research_collection" || kind === "research_entry" || layer === "intent") return "decision";
	if (kind === "doc" || kind === "system_diagram" || kind === "system_diagram_ref" || kind === "missing_system_diagram_ref" || layer === "knowledge") return "knowledge";
	if (kind === "roadmap_task" || kind === "roadmap_sprint" || kind === "planning_build" || kind === "change_claim" || kind === "change_claim_waiter" || kind === "change_claim_scope" || layer === "roadmap") return "work";
	if (kind === "code_path" || kind === "test_file" || kind === "implementation_build" || layer === "code" || id.startsWith("code:") || id.startsWith("test:") || path.startsWith("src/") || path.startsWith("tests/")) return "execution";
	if (kind === "validation_report" || kind === "content_proof" || kind === "audit_evidence" || kind === "canonical_source_ref" || kind === "git_archive_ref" || kind === "lint_issue" || layer === "validation" || layer === "content_proof" || layer === "audit" || layer === "archive") return "proof";
...
function graphLensFamilyForReconciliation(item: any): GraphLensFamilyId {
	...
	const layers = [item?.from_layer, item?.to_layer].map((value) => String(value || "").trim());
	if (loop === "decision" || layers.some((layer) => ["intent", "decision"].includes(layer))) return "decision";
	if (layers.includes("knowledge")) return "knowledge";
	if (loop === "planning" || layers.includes("roadmap")) return "work";
	if (loop === "implementation" || layers.some((layer) => ["code", "build"].includes(layer))) return "execution";
	return "proof";
}

function graphLensBadge(id: string, label: string, count: number, refs: string[] = []) {
	return {
		id,
		label,
		count,
		...(refs.length > 0 ? { refs: refs.slice(0, 6), ref_count: refs.length } : {}),
	};
}
...
function extractNextActionSourceId(items: any[], action: any): string {
	...
	const item = items.find((candidate) => String(candidate?.id || "").trim() === actionItemId);
	return String(item?.source_id || "").trim();
}

function applyDefaultLensCompaction(nodes: any[], nextActionSourceId: string): void {
...
function buildGraphLensViews(input: {
	...
	const nextActionSourceId = extractNextActionSourceId(input.reconciliationItems, input.reconciliationAction);
	...
	const familyDrift: Record<GraphLensFamilyId, { drift: number; blocked: number }> = {
	...
	for (const node of input.nodes) {
		const family = graphLensFamilyForNode(node);
		if (!family) continue;
		const hiddenByDefault = node.default_hidden === true || node.compacted === true || (isBuildOrValidationNode(node) && String(node.id || "") !== nextActionSourceId);
	...
	for (const item of input.reconciliationItems) {
		...
		if (String(item?.state || "").trim() === "blocked") familyDrift[family].blocked += 1;
		else familyDrift[family].drift += 1;
	}
	const buildPaths = input.builds.map((build) => normalizeCodewikiRef(build.path)).filter(Boolean);
	const validationPaths = input.validations.map((validation) => normalizeCodewikiRef(validation.path)).filter(Boolean);
	const buildCounts = {
		total: input.builds.length,
		decision: input.builds.filter((build) => build.kind === "decision_build").length,
		planning: input.builds.filter((build) => build.kind === "planning_build").length,
		implementation: input.builds.filter((build) => build.kind === "implementation_build").length,
		hot: input.gc?.classes?.hot?.build_paths?.length || 0,
		collapsed: buildPaths.filter((path) => `build:${path}` !== nextActionSourceId).length,
	};
	const validationCounts = {
		total: input.validations.length,
		pass: input.validations.filter((validation) => String(validation.verdict || "") === "pass").length,
		fail_or_block: input.validations.filter((validation) => ["fail", "block"].includes(String(validation.verdict || ""))).length,
		hot: input.gc?.classes?.hot?.validation_paths?.length || 0,
		collapsed: validationPaths.filter((path) => `validation:${path}` !== nextActionSourceId).length,
	};
	const openTasks = input.roadmapEntries.filter((task) => isOpenTaskStatus(String(task.status || "todo")));
	...
	const badgesByFamily: Record<GraphLensFamilyId, any[]> = {
		...
		work: [
			...
			graphLensBadge("active_sprints", "active sprints", input.activeSprintIds.length, input.activeSprintIds),
			graphLensBadge("active_claims", "active claims", input.claimState.active_claim_count),
		],
		execution: [
			graphLensBadge("code_paths", "code paths", input.nodes.filter((node) => node.kind === "code_path").length),
			graphLensBadge("test_files", "test files", input.nodes.filter((node) => node.kind === "test_file").length),
			graphLensBadge("implementation_builds", "implementation builds", buildCounts.implementation),
			graphLensBadge("dirty_paths", "dirty paths", input.dirtyPaths.length, input.dirtyPaths),
		],
		proof: [
			graphLensBadge("validation_reports", "validation reports", validationCounts.total),
			graphLensBadge("fail_or_block_validation", "fail/block validation", validationCounts.fail_or_block),
			graphLensBadge("audit_refs", "audit refs", input.auditEvidenceRefs.length),
			graphLensBadge("content_proofs", "content proofs", input.contentProofRefs.length),
			graphLensBadge("traceability_gaps", "trace gaps", traceabilityGaps.length),
		],
	};
	const families = DEFAULT_GRAPH_LENS_FAMILIES.map((family) => {
		const drift = familyDrift[family.id];
		const state = drift.blocked > 0 ? "blocked" : drift.drift > 0 ? "drift" : "aligned";
	...
	return {
		...
		trace: {
			...
			build_refs: input.builds.map((build) => ({
				...
				kind: build.kind,
				status: build.status || build.data?.status || build.data?.lifecycle?.state || "unknown",
...
export function buildGraph(inputs: GraphBuildInputs): GraphFile {
	...
	applyDefaultLensCompaction(nodes, graphLensViews.default.next_action.source_id || "");
