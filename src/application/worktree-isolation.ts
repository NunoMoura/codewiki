...
function repoLabel(project: WikiProject): string {
	return safeSegment(project.config?.project_name || project.label || basename(project.root), "repo");
}
...
export function createRoleWorktreePlan(project: WikiProject, input: RoleWorktreePlanInput): RoleWorktreePlan {
	...
	const root = input.worktree_root || resolve(project.root, "..", ".codewiki-worktrees", repoLabel(project));
	const branch = input.branch || `codewiki/${taskId}/${role}/${sessionId}`;
	const worktreePath = input.worktree_path || resolve(root, taskId, role, sessionId);
