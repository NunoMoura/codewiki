...
export function generatedStatePaths(project: WikiProject): string[] {
	...
	return [
		...
		...(Array.isArray(config.codewiki?.gateway?.generated_readonly_paths)
			? config.codewiki.gateway.generated_readonly_paths
			: []),
	];
}
...
export function lintHealth(lintReport: LintReport) {
	...
	const errors = issues.filter(
		(i: any) => String(i.severity) === "error",
	).length;
	const warnings = issues.filter(
		(i: any) => String(i.severity) === "warning",
	).length;
...
export function buildRoadmapState(
	...
	const ordered = entries
		...
		.filter(Boolean);
	const graphSprints = Array.isArray((graphRoadmap as any).sprints)
		? (graphRoadmap as any).sprints
		: sprints;
	...
	for (const item of entries) {
		...
		tasks[taskId] = {
			...
			change_type:
				String(item.change_type || item.change_class || "").trim() || undefined,
	...
	const openTaskIds = sortedEntries
		.filter((t) => isOpenTaskStatus(t.status))
		.map((t) => t.id);
	const inProgressIds = sortedEntries
		.filter((t) => isActiveTaskStatus(t.status))
		.map((t) => t.id);
	const todoIds = sortedEntries
	...
	return {
		...
		summary: {
			...
			active_sprint_count: ((graphRoadmap as any).active_sprint_ids || [])
				.length,
		...
		views: {
			...
			sprint_ids:
				(graphRoadmap as any).sprint_ids ||
				graphSprints.map((sprint: any) => sprint.id),
			active_sprint_ids:
				(graphRoadmap as any).active_sprint_ids ||
				graphSprints
					.filter(
						(sprint: any) =>
							!["closed", "cancelled"].includes(String(sprint.status)),
					)
			...
			done_task_ids: sortedEntries
				.filter((t) => String(t.status) === "done")
				.map((t) => t.id),
			cancelled_task_ids:
				(graphRoadmap as any).cancelled_task_ids ||
				sortedEntries
					.filter((t) => String(t.status) === "cancelled")
					.map((t) => t.id),
			recent_task_ids:
				(graphRoadmap as any).recent_task_ids || recentEntries.map((t) => t.id),
		},
		tasks,
	};
}
...
export function compactCodeArea(codePaths: string[]): string {
	...
	const suffix =
		areas.length > visible.length
			? ` +${areas.length - visible.length} more`
			: "";
...
export function laneRevisionAnchor(
	...
	for (const path of rowPaths) {
		specDigests[path] = String(
			specRowsByPath[path]?.revision?.digest || "",
		).trim();
	}
...
export function buildAgencyLane(
	...
	for (const task of roadmapEntries) {
		...
		const taskSpecPaths = Array.isArray(task.spec_paths)
			? task.spec_paths.map((v: any) => String(v).trim())
			: [];
		const taskCodePaths = Array.isArray(task.code_paths)
			? task.code_paths.map((v: any) => String(v).trim())
			: [];
...
export function buildResumeState(
	...
	if (task) {
		...
		const checksRun = Array.isArray(evidence.checks_run)
			? evidence.checks_run
			: [];
		...
		const reasonPrefix =
			openTaskId === persistedFocusTaskId
...
export function buildParallelSessionState(
	...
	const agentNames = assignAgentNames(
		sessions.map((s: any) => String(s.session_id).trim()).filter(Boolean),
	);
...
export function buildStatusState(
	...
	for (const node of graph.nodes || []) {
		if (
			...
			String((node as any).doc_type || "").trim() !== "spec"
		)
			continue;
		...
		const mergedCodePaths =
			graphDocCodePaths[path] && graphDocCodePaths[path].length > 0
	...
	for (const task of roadmapEntries) {
		const specPaths = Array.isArray(task.spec_paths)
			? task.spec_paths.map((v) => String(v).trim()).filter(Boolean)
			: [];
	...
	for (const doc of specDocs) {
		...
		const codePaths = Array.isArray(doc.code_paths)
			? doc.code_paths.map((v: unknown) => String(v).trim()).filter(Boolean)
			: [];
		...
		specRows.push({
			...
			related_task_ids: [...openTasks, ...blockedTasks, ...doneTasks]
	...
	const productSpecPaths = specRows
		.filter((r) => r.path.startsWith(PRODUCT_SPEC_PREFIX))
		.map((r) => r.path);
	const systemSpecPaths = specRows
		.filter((r) => r.path.startsWith(SYSTEM_SPEC_PREFIX))
		.map((r) => r.path);
	const uxSpecPaths = specRows
		.filter((r) => pathStartsWithAny(r.path, CLIENTS_SPEC_PREFIXES))
		.map((r) => r.path);
	...
	const reconciliationAction =
		(graph.views as any)?.reconciliation?.next_action || null;
	let nextStep: any;
	if (
		...
		(
			(roadmapState.views as any)?.executable_in_progress_task_ids ||
			roadmapState.views?.in_progress_task_ids ||
			[]
		).length
	) {
		const taskId = ((roadmapState.views as any)
			?.executable_in_progress_task_ids ||
		...
		(
			(roadmapState.views as any)?.executable_todo_task_ids ||
			roadmapState.views?.todo_task_ids ||
			[]
		).length
	) {
		const taskId = ((roadmapState.views as any)?.executable_todo_task_ids ||
	...
	const agencySummary = {
		...
		high_cadence_lane_ids: agencyLanes
			.filter((l) => l.cadence === "high")
			.map((l) => l.id),
		medium_cadence_lane_ids: agencyLanes
			.filter((l) => l.cadence === "medium")
			.map((l) => l.id),
		low_cadence_lane_ids: agencyLanes
			.filter((l) => l.cadence === "low")
			.map((l) => l.id),
	};
	...
	const fileStructureActionableDrift = Array.isArray(
		fileStructure?.actionable_entries,
	)
		? fileStructure.actionable_entries.length
		: 0;
	const fileStructureStatus = fileStructure
		? {
				...
				path_rule_counts: {
					intended: Array.isArray(fileStructure.intended_paths)
						? fileStructure.intended_paths.length
						: 0,
					current: Array.isArray(fileStructure.current_paths)
						? fileStructure.current_paths.length
						: 0,
					target: Array.isArray(fileStructure.target_paths)
						? fileStructure.target_paths.length
						: 0,
				},
				approved_migration_delta_paths: Array.isArray(
					fileStructure.approved_migration_deltas,
				)
					? fileStructure.approved_migration_deltas
	...
	const decisionPropagationStatus = decisionPropagation
		? {
				...
				residuals: Array.isArray(decisionPropagation.residuals)
					? decisionPropagation.residuals.slice(0, 20)
					: [],
			}
		: null;
	const workflowCursor = graphViews.workflow_cursor || {
		active_loop:
			String(nextStep.kind || "observe") === "code"
				? "implementation"
				: "observe",
		reason: String(nextStep.reason || ""),
		expected_output:
			String(nextStep.kind || "") === "code"
	...
	const orderedTaskIds = Array.isArray(roadmapState.views?.ordered_task_ids)
		? roadmapState.views.ordered_task_ids
		: [];
	...
	const direction = [
		...
		`Agency policy: level=${agencyPolicy.level}, approval=${agencyPolicy.approval_cadence}, reset_auto_pickup=${agencyPolicy.context_reset.enabled && agencyPolicy.context_reset.auto_pickup ? "on" : "off"}.`,
