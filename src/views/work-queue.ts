...
export function buildWorkQueueView(input: TraceViewInput): WorkQueueView {
	...
	const blockers = blockersFromTrace(input.records).filter(
		(blocker) => blocker.kind !== "conflict",
	);
	const decisions = decisionQueueItems(input.records, blockers);
...
function workUnitStatus(input: {
	...
	if (input.itemBlockers.length > 0) return "blocked";
...
function plannedDecisionRefs(records: TraceRecord[]): Set<string> {
	return new Set([
		...planningWorkUnits(records).flatMap((item) => item.decisionRefs),
		...planningDecisionResolutions(records).map(
			(resolution) => resolution.decisionRef,
		),
	]);
}
...
function planningDecisionResolutions(
	records: TraceRecord[],
): Array<{ decisionRef: string }> {
	return eventsByName(records, "planning.iteration").flatMap((event) =>
