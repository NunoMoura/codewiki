...
const CANONICAL_ROADMAP_STATUSES = [
	"todo",
	"in_progress",
	"blocked",
	"done",
...
const LEGACY_ROADMAP_WORKFLOW_VALUES = new Set([
	"research",
	"implement",
	"verify",
]);
...
export function normalizeAuditProfiles(
	input: CodewikiAuditInput,
): AuditProfile[] {
	const requested: AuditProfile[] = [
		...new Set((input.profiles || []).filter(isAuditProfile)),
	];
	if (input.full || requested.length === 0) {
		if (input.changed && !requested.includes("changed"))
			requested.push("changed");
		if (input.task_id && !requested.includes("task")) requested.push("task");
		if (input.layers?.length && requested.length === 0)
			requested.push("alignment");
...
function exportedStringArray(
	...
	const match = source.match(
		new RegExp(
			`export\\s+const\\s+${safeExportName}\\s*=\\s*\\[([\\s\\S]*?)\\]\\s*as\\s+const`,
		),
	);
...
async function auditFileStructure(
	...
	if (tsFiles.length === 0) {
		issues.push(
			createIssue(
				...
				"warning",
				"missing-source-tree",
				"No TypeScript source files found under src/.",
				"src",
			),
		);
	}

	const roadmapTypesPath = resolve(project.root, "src/roadmap/types.ts");
	const typesText = await maybeReadText(roadmapTypesPath);
	if (typesText) {
		pushArrayEqualsIssue(
			issues,
			exportedStringArray(typesText, "ROADMAP_STATUS_VALUES"),
		...
		if (/TASK_PHASE_VALUES|TaskPhase|phase\?:/.test(typesText)) {
			issues.push(
				createIssue(
					...
					"error",
					"workflow-drift",
					"Task phases must not be canonical types; use roadmap status plus build/validation gates.",
					"src/roadmap/types.ts",
				),
			);
		}
		...
		issues.push(
			createIssue(
				...
				"warning",
				"missing-types",
				"Unable to read shared domain types.",
				"src/roadmap/types.ts",
			),
		);
	}

	const schemaText = await maybeReadText(
		resolve(project.root, "src/adapters/pi/schemas.ts"),
	);
	if (
		schemaText &&
		/taskLoopPhaseSchema|phase:\s*Type\.Optional|Type\.Literal\(["']verify["']\)/.test(
			schemaText,
		)
	) {
		issues.push(
			createIssue(
				...
				"error",
				"workflow-drift",
				"Pi schema must not expose task phase fields or deprecated 'verify' literal.",
				"src/adapters/pi/schemas.ts",
			),
		);
	}

	const graphText = await maybeReadText(
		resolve(project.root, ".codewiki/index_graph.json"),
	);
	if (graphText && /"phase"\s*:/.test(graphText)) {
		issues.push(
			createIssue(
				...
				"error",
				"workflow-drift",
				"Generated index_graph.json must not expose task phase fields.",
			...
			if (LEGACY_ROADMAP_WORKFLOW_VALUES.has(status)) {
				issues.push(
					createIssue(
						...
						"error",
						"workflow-drift",
						`${taskId} uses deprecated roadmap status '${status}'.`,
						project.roadmapPath,
					),
				);
			}
	...
	for (const file of tsFiles) {
		...
		if (
			...
			issues.push(
				createIssue(
					profile,
					"error",
					"transitional-layer-no-new-files",
					`${rel} is in a removed source layer.`,
					`src/${rel}`,
				),
			);
		}
		for (const rule of ARCHITECTURE_RULES) {
			...
			if (
				rule.forbiddenOutside &&
				(rel.startsWith(rule.forbiddenOutside) ||
					PI_SDK_ENTRYPOINT_ALLOWLIST.has(rel))
			)
				continue;
	...
	const architectureScript = await maybeReadText(
		resolve(project.root, "scripts/check-architecture.mjs"),
	);
	if (!architectureScript) {
		issues.push(
			createIssue(
				profile,
				"warning",
				"missing-script-wrapper",
				"Optional architecture script wrapper is absent.",
		...
		if (!architectureScript.includes("executeCodewikiAudit")) {
			issues.push(
				createIssue(
					profile,
					"error",
					"script-owned-audit-semantics",
					"Architecture check script must delegate to the source-owned audit engine.",
					"scripts/check-architecture.mjs",
				),
			);
		}
		if (
			/const\s+checks\s*=|ARCHITECTURE_RULES|domain-is-pure/.test(
				architectureScript,
			)
		) {
			issues.push(
				createIssue(
					...
					"error",
					"script-owned-audit-semantics",
					"Architecture check script must not define authoritative audit rules.",
					"scripts/check-architecture.mjs",
				),
			);
		}
	...
	for (const scriptFile of scriptFiles) {
		...
		if (!text) continue;
		if (
			/const\s+checks\s*=|ARCHITECTURE_RULES|domain-is-pure|\.codewiki\/roadmap\/queue\.json|writeFileSync\([\s\S]{0,160}\.codewiki\//.test(
				text,
			) &&
			!text.includes("../src/")
		) {
			issues.push(
				createIssue(
					profile,
					"error",
					"script-owned-product-logic",
					"Script appears to own CodeWiki product/audit semantics instead of delegating to source-owned code.",
					rel,
				),
			);
		}
	...
	const docFiles = await walkFiles(
		resolve(project.root, project.docsRoot),
	...
	const diagramValidation = validateSystemDiagramRefs(
		project.root,
		project,
		docs,
	);
	...
	const fileStructureDrift = buildFileStructureDriftReport(
		project.root,
		project,
	);
	if (
	...
	const blockingDriftEntries = fileStructureDrift.entries.filter(
	...
	return {
		...
		checked_scopes: {
			...
			files: [
				...
				"scripts/**/*.mjs",
				"scripts/check-architecture.mjs",
				".codewiki/kb/system/diagrams/**/*.yaml",
			],
		},
		issues,
...
async function auditAlignment(
	...
	if (!graph) {
		issues.push(
			createIssue(
				...
				"error",
				"missing-graph",
				"Generated graph index is missing or unreadable.",
				graphPath,
			),
		);
	} else {
		...
		if (health?.errors > 0) {
			issues.push(
				createIssue(
					...
					"error",
					"graph-health",
					`Graph health reports ${health.errors} errors.`,
					graphPath,
				),
			);
		}
		if (health?.warnings > 0) {
			issues.push(
				createIssue(
					...
					"warning",
					"graph-health",
					`Graph health reports ${health.warnings} warnings.`,
					graphPath,
				),
			);
		}
		...
		if (residualCount > 0) {
			issues.push(
				createIssue(
					profile,
					"error",
					"decision-propagation-unmapped",
					`Accepted decision propagation has ${residualCount} unresolved row/question mappings.`,
					graphPath,
				),
			);
		}
	...
	if (!queue?.tasks) {
		issues.push(
			createIssue(
				...
				"error",
				"missing-roadmap",
				"Roadmap queue is missing or unreadable.",
				queuePath,
			),
		);
	}
	const fingerprints = await fingerprintFiles(
		project,
		[graphPath, queuePath, ...(input.paths || [])],
		input.include_fingerprints !== false,
	);
	return {
		profile,
...
async function auditStaleReference(
	project: WikiProject,
	input: CodewikiAuditInput,
): Promise<AuditProfileResult> {
	...
	const docs = await walkFiles(
		docsRoot,
		(path) => path.endsWith(".md") || path.endsWith(".mdx"),
	);
	const activeTextFiles = [
		...docs,
		...(await walkFiles(resolve(project.root, "skills"), (path) =>
			path.endsWith(".md"),
		)),
	...
	const stalePatterns: Array<{ pattern: RegExp; message: string }> = [
		{
			pattern: /extensions\/codewiki\/src/g,
			message:
				"Legacy package-source path extensions/codewiki/src appears in active docs/source.",
		},
		{
			pattern: /\.codewiki\/roadmap\.json/g,
			message: "Legacy root roadmap path appears in active docs/source.",
		},
		{
			pattern: /scripts\/codewiki-transaction\.mjs/g,
			message:
				"Deprecated script-owned CodeWiki transaction path appears in active docs/source.",
		},
		{
			pattern: /\/wiki-verify\b/g,
			message: "Deprecated wiki-verify command appears in active docs/source.",
		},
	];
	const semanticMisusePatterns: Array<{
		kind: string;
		pattern: RegExp;
		allowed: RegExp;
		message: string;
	}> = [
		{
			...
			pattern:
				/(?:\.codewiki\/`?|\.codewiki\/\*\*).{0,100}(?:is|are|as|contains|holds|stores).{0,100}package source/gi,
			allowed:
				/\b(?:not|never|do not|must not)\b.{0,80}package source|package source.{0,80}\b(?:not|never)\b/i,
			message: ".codewiki dogfood state is described as package source.",
		},
		{
			kind: "generated-task-view-as-truth",
			pattern:
				/(?:\.codewiki\/roadmap\/tasks\/\*\*|generated task (?:views|shards|context)).{0,120}(?:canonical|source[- ]of[- ]truth|truth)/gi,
			allowed:
				/\b(?:not|never|do not|must not|read-only)\b.{0,100}(?:canonical|truth|hand-edit|source)/i,
	...
	for (const file of activeTextFiles) {
		...
		for (const item of stalePatterns) {
			...
			if (item.pattern.test(text)) {
				issues.push(
					createIssue(profile, "error", "stale-reference", item.message, rel),
				);
			}
		}
		for (const item of semanticMisusePatterns) {
			...
			for (const match of text.matchAll(item.pattern)) {
				...
				issues.push(
					createIssue(profile, "error", item.kind, item.message, rel),
				);
				break;
			}
		}
	...
	return {
		...
		checked_scopes: {
			root: project.root,
			files: [project.docsRoot, "skills/**/*.md", "README.md"],
		},
		issues,
...
async function discoverSkillAssetFiles(
	project: WikiProject,
	skillEntries: string[],
): Promise<string[]> {
	const assets: string[] = [];
	for (const rawEntry of skillEntries) {
		if (isPackageManifestExclusion(rawEntry) || hasPackageGlobPattern(rawEntry))
			continue;
		...
		if (await pathExists(resolve(absolute, "SKILL.md")))
			skillDirs.add(absolute);
		for (const manifest of await walkFiles(absolute, (path) =>
			normalizeRel(path).endsWith("/SKILL.md"),
		)) {
	...
	return unique(
		assets.map((asset) => normalizeRel(relative(project.root, asset))).sort(),
	);
}
...
async function npmPackDryRunFiles(
	...
	try {
		...
		return files
			.map((item: JsonObject) => String(item.path || ""))
			.filter(Boolean)
			.sort();
	} catch {
		return null;
...
async function auditPackage(
	...
	const packageJson = await maybeReadJson(
		resolve(project.root, packageJsonPath),
	);
	...
	if (!packageJson) {
		issues.push(
			createIssue(
				...
				"No package.json found; package audit has no package surface to inspect.",
				packageJsonPath,
			),
		);
	} else {
		const files = Array.isArray(packageJson.files)
			? packageJson.files.map(String)
			: [];
			...
			if (!files.includes(required)) {
				issues.push(
					createIssue(
						...
						"error",
						"package-files",
						`package.json files must include ${required}.`,
						packageJsonPath,
					),
				);
			}
		...
		for (const entry of files) {
			if (entry.startsWith("extensions")) {
				issues.push(
					createIssue(
						...
						"error",
						"package-files",
						"package.json files must not include deprecated extensions/ source paths.",
						packageJsonPath,
					),
				);
			}
			if (
				!entry.includes("*") &&
				!(await pathExists(resolve(project.root, entry)))
			) {
				issues.push(
					createIssue(
						profile,
						"error",
						"package-files-unreachable",
						`package.json files entry is unreachable: ${entry}.`,
						packageJsonPath,
					),
				);
			}
		}
		const extensions = Array.isArray(packageJson.pi?.extensions)
			? packageJson.pi.extensions.map(String)
			: [];
		if (!extensions.includes("./src/index.ts")) {
			issues.push(
				createIssue(
					...
					"error",
					"pi-extension-entry",
					"Pi extension entry must resolve from ./src/index.ts.",
					packageJsonPath,
				),
			);
		}
		for (const entry of extensions) {
			if (
				!(await pathExists(resolve(project.root, entry.replace(/^\.\//, ""))))
			) {
				issues.push(
					createIssue(
						profile,
						"error",
						"pi-extension-unreachable",
						`Pi extension entry is unreachable: ${entry}.`,
						packageJsonPath,
					),
				);
			}
		}
		const skills = Array.isArray(packageJson.pi?.skills)
			? packageJson.pi.skills.map(String)
			: [];
		if (
			skills.length === 0 &&
			files.map(normalizePackageEntry).includes("skills")
		) {
			issues.push(
				createIssue(
					profile,
					"error",
					"pi-skill-entry",
					"package.json files include skills but pi.skills declares no skill entries.",
					packageJsonPath,
				),
			);
		}
		for (const entry of skills) {
			if (isPackageManifestExclusion(entry) || hasPackageGlobPattern(entry))
				continue;
			const normalizedEntry = normalizePackageEntry(entry);
			if (
				normalizedEntry &&
				!(await pathExists(resolve(project.root, normalizedEntry)))
			) {
				issues.push(
					createIssue(
						profile,
						"error",
						"pi-skill-unreachable",
						`Pi skill entry is unreachable: ${entry}.`,
						packageJsonPath,
					),
				);
			}
			...
			if (!(await pathExists(resolve(project.root, asset)))) {
				issues.push(
					createIssue(
						profile,
						"error",
						"skill-asset-unreachable",
						`Skill asset is unreachable: ${asset}.`,
						asset,
					),
				);
			}
		...
		if (!packageJson.scripts?.["check:architecture"]) {
			issues.push(
				createIssue(
					...
					"warning",
					"package-checks",
					"package.json should expose check:architecture wrapper for CI/package smoke.",
					packageJsonPath,
				),
			);
		}
		...
		if (packedFiles) {
				...
				if (!packedFiles.includes(required)) {
					issues.push(
						createIssue(
							profile,
							"error",
							"package-dry-run-missing",
							`npm pack --dry-run omits required artifact ${required}.`,
							packageJsonPath,
						),
					);
				}
			...
			for (const packed of packedFiles) {
				if (
					/^(?:\.codewiki|\.pi|tests|node_modules|extensions)\//.test(packed)
				) {
					issues.push(
						createIssue(
							profile,
							"error",
							"package-dry-run-unexpected",
							`npm pack --dry-run includes unexpected artifact ${packed}.`,
							packageJsonPath,
						),
					);
				}
			...
			issues.push(
				createIssue(
					profile,
					"warning",
					"package-dry-run-unavailable",
					"Unable to inspect npm pack --dry-run --json output.",
					packageJsonPath,
				),
			);
		}
	...
	if (
		packageJson &&
		!(await pathExists(resolve(project.root, "package-lock.json")))
	) {
		issues.push(
			createIssue(
				profile,
				"error",
				"missing-lockfile",
				"package-lock.json is absent; publication reproducibility is not anchored.",
				"package-lock.json",
			),
		);
	}
	...
	return {
		...
		status: statusForIssues(issues),
		summary:
			"Checked package manifest reachability, Pi package entries, and lockfile presence.",
		checked_scopes: {
			root: project.root,
			files: ["package.json", "package-lock.json", "src/index.ts", "skills"],
		},
		issues,
...
async function auditSecurity(
	...
	const riskyScriptPattern =
		/\b(curl|wget|sudo|chmod\s+777|rm\s+-rf\s+\/|npm\s+publish)\b/;
			...
			if (riskyScriptPattern.test(String(script))) {
				issues.push(
					createIssue(
						profile,
						"warning",
						"risky-package-script",
						`package script ${name} contains a risky shell fragment.`,
						"package.json",
					),
				);
			}
	...
	if (tracked) {
			...
			if (/(^|\/)(\.env|id_rsa|id_dsa|.*\.pem|.*\.p12)$/.test(rel)) {
				issues.push(
					createIssue(
						...
						"error",
						"secret-risk-path",
						`Tracked path looks like a secret-bearing file: ${rel}.`,
						rel,
					),
				);
			}
		...
		issues.push(
			createIssue(
				...
				"warning",
				"git-unavailable",
				"Unable to inspect git tracked files for secret-risk paths.",
			),
		);
	}
	const fingerprints = await fingerprintFiles(
		project,
		["package.json", "package-lock.json", ...(input.paths || [])],
		input.include_fingerprints !== false,
	);
	return {
		profile,
		...
		checked_scopes: {
			root: project.root,
			files: ["package.json", "git ls-files"],
		},
		issues,
...
async function auditRoadmapTaskViewParity(
	project: WikiProject,
	issues: AuditIssue[],
): Promise<number> {
	...
	for (const [taskId, rawTask] of Object.entries(queue.tasks)) {
		...
		const contextView = await maybeReadJson(
			resolve(project.root, contextViewPath),
		);
		if (!taskView) {
			issues.push(
				createIssue(
					profile,
					"error",
					"roadmap-task-view-missing",
					`Generated task view missing for active ${taskId}.`,
					taskViewPath,
				),
			);
		} else if (
			JSON.stringify(comparableTaskFields(taskView)) !==
			JSON.stringify(comparableTaskFields(task))
		) {
			issues.push(
				createIssue(
					profile,
					"error",
					"roadmap-task-view-mismatch",
					`Generated task view does not match canonical roadmap queue for ${taskId}.`,
					taskViewPath,
				),
			);
		}
		...
		if (!contextTask) {
			issues.push(
				createIssue(
					profile,
					"error",
					"roadmap-task-context-missing",
					`Generated task context missing for active ${taskId}.`,
					contextViewPath,
				),
			);
		} else if (
			JSON.stringify(comparableTaskFields(contextTask)) !==
			JSON.stringify(comparableTaskFields(task))
		) {
			issues.push(
				createIssue(
					profile,
					"error",
					"roadmap-task-context-mismatch",
					`Generated task context does not match canonical roadmap queue for ${taskId}.`,
					contextViewPath,
				),
			);
		}
	}
	const generatedTaskDirs = await walkFiles(
		resolve(project.root, ".codewiki/roadmap/tasks"),
		(path) => path.endsWith("task.json"),
	);
	for (const taskView of generatedTaskDirs) {
		const taskId = normalizeRel(
			relative(resolve(project.root, ".codewiki/roadmap/tasks"), taskView),
		).split("/")[0];
		if (taskId && !activeIds.has(taskId) && !allIds.has(taskId)) {
			issues.push(
				createIssue(
					profile,
					"error",
					"roadmap-task-view-orphan",
					`Generated task view exists for missing task ${taskId}.`,
					`.codewiki/roadmap/tasks/${taskId}/task.json`,
				),
			);
		}
	}
...
async function auditGeneratedParity(
	project: WikiProject,
	input: CodewikiAuditInput,
): Promise<AuditProfileResult> {
	const profile: AuditProfile = "generated-parity";
	...
	const canonicalPaths = [project.roadmapPath, project.docsRoot].map((path) =>
	...
	try {
		...
		issues.push(
			createIssue(
				...
				"Generated index_graph.json is missing.",
				".codewiki/index_graph.json",
			),
		);
	}
			...
			try {
				...
				issues.push(
					createIssue(
						...
						`Unable to stat ${normalizeRel(relative(project.root, canonicalPath))}.`,
						normalizeRel(relative(project.root, canonicalPath)),
					),
				);
			}
		}
...
function classifyLayer(path: string): string {
	...
	if (
		path.startsWith(".codewiki/roadmap/tasks/") ||
...
async function auditChanged(
	...
	const raw = await gitOutput(project, [
		"status",
		"--porcelain=v1",
		"-z",
		"--untracked-files=all",
	...
	if (raw === null) {
		issues.push(
			createIssue(
				...
				"warning",
				"git-unavailable",
				"Unable to read git status for changed-file audit.",
			),
		);
	} else {
		...
		for (const file of changedFiles) {
			if (
				file.startsWith(".codewiki/roadmap/tasks/") ||
				file === ".codewiki/index_graph.json"
			) {
				issues.push(
					createIssue(
						profile,
						...
						"Generated state/view file is changed; refresh or revert generated output before publication.",
						file,
					),
				);
			}
...
async function auditTask(
	...
	const task =
		taskId && queue?.tasks
			? (queue.tasks[taskId] as JsonObject | undefined)
			: undefined;
	if (!taskId) {
		issues.push(
			createIssue(
		...
		issues.push(
			createIssue(
				...
				"error",
				"task-not-found",
				`${taskId} was not found in roadmap queue.`,
				queuePath,
			),
		);
	} else {
		...
		if (!boundary.executable) {
			issues.push(
				createIssue(
					...
					`${taskId} is not self-contained executable work; use a sprint for grouping. ${boundary.reasons.join("; ")}`,
					queuePath,
				),
			);
		}
		for (const rel of [...(task.spec_paths || []), ...(task.code_paths || [])]
			...
			if (!(await pathExists(resolve(project.root, rel)))) {
				issues.push(
					createIssue(
						...
						"warning",
						"task-path-missing",
						`${taskId} references a missing path: ${rel}.`,
						rel,
					),
				);
			}
		...
		if (task.status === "done") {
			...
			const validations = await walkFiles(validationDir, (path) =>
				path.endsWith(".json"),
			);
			const hasClosePass = validations.some(
				(path) =>
					path.includes("task-close-pass") &&
					path.includes(taskId.toLowerCase()),
			);
			if (!hasClosePass) {
				issues.push(
					createIssue(
						profile,
						"warning",
						"missing-task-close-validation",
						`${taskId} is done but no matching task-close pass report was found.`,
						".codewiki/validation",
					),
				);
			}
	...
	return {
		...
		evidence_refs: [
			queuePath,
			...(taskId ? [`.codewiki/roadmap/tasks/${taskId}/task.json`] : []),
		],
		fingerprints,
	};
}
...
async function runProfile(
	...
	switch (profile) {
		...
		case "source-contract":
			return auditSourceContract(project, input);
		case "file-structure":
			return auditFileStructure(project, input);
		case "stale-reference":
			return auditStaleReference(project, input);
		case "package":
			return auditPackage(project, input);
		case "security":
		...
		case "task":
			return auditTask(project, input);
	}
}
...
function mergeScope(
	project: WikiProject,
	input: CodewikiAuditInput,
	profileResults: AuditProfileResult[],
): AuditScope {
	return {
		root: project.root,
		files: unique(
			profileResults.flatMap((result) => result.checked_scopes.files || []),
		).sort(),
		layers: unique([
			...(input.layers || []),
			...profileResults.flatMap((result) => result.checked_scopes.layers || []),
		]).sort(),
		task_id: input.task_id,
		changed:
			input.changed ||
			profileResults.some((result) => result.checked_scopes.changed),
	};
}

export async function executeCodewikiAudit(
	project: WikiProject,
	input: CodewikiAuditInput = {},
): Promise<AuditReport> {
	...
	return {
		...
		evidence_refs: unique(
			profileResults.flatMap((result) => result.evidence_refs),
		).sort(),
		fingerprints,
...
export function formatAuditReport(report: AuditReport): string {
	...
	for (const issue of report.issues.slice(0, 30)) {
		...
		lines.push(
			`  [${issue.severity}] ${issue.profile}/${issue.kind}${path}: ${issue.message}`,
		);
	}
	if (report.issues.length > 30)
		lines.push(`  … ${report.issues.length - 30} more issues`);
	return lines.join("\n");
}
