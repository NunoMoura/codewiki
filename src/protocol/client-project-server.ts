...
export const CLIENT_PROJECT_SERVER_PROTOCOL = Object.freeze({
	id: "codewiki.client-project-server",
	version: "1.0.0",
...
export type ClientProjectServerOperationStatus =
	| "accepted"
	| "running"
	| "succeeded"
	| "failed"
	| "cancelled";
export type ClientProjectServerCoverage = "complete" | "partial" | "unavailable";

export interface ClientProjectServerActorContext {
	readonly actorId: string;
	readonly authenticatedIdentityRef: string;
}

export interface ClientProjectServerTransportContext {
	readonly clientKind: ClientKind;
	readonly clientInstanceId: string;
	readonly authenticationRef: string;
}

export interface ClientProjectServerRequestContext {
...
export interface ClientProjectServerSnapshotContext {
	readonly snapshotDigest: Sha256Digest;
	readonly provenanceRefs: readonly string[];
	readonly coverage: ClientProjectServerCoverage;
	readonly truncated: boolean;
	readonly stale: boolean;
	readonly redacted: boolean;
}

export interface ClientProjectServerCommandEnvelope extends ClientProjectServerRequestContext {
	readonly protocolId: typeof CLIENT_PROJECT_SERVER_PROTOCOL.id;
	readonly protocolVersion: typeof CLIENT_PROJECT_SERVER_PROTOCOL.version;
...
export interface ClientProjectServerQueryEnvelope extends ClientProjectServerRequestContext {
	readonly protocolId: typeof CLIENT_PROJECT_SERVER_PROTOCOL.id;
	readonly protocolVersion: typeof CLIENT_PROJECT_SERVER_PROTOCOL.version;
	readonly kind: "query";
	readonly transportRequestId: string;
...
export interface ClientProjectServerQueryResultEnvelope {
	readonly protocolId: typeof CLIENT_PROJECT_SERVER_PROTOCOL.id;
	readonly protocolVersion: typeof CLIENT_PROJECT_SERVER_PROTOCOL.version;
	...
	readonly repositoryIdentity: Sha256Digest;
	readonly queryName: string;
	readonly snapshot: ClientProjectServerSnapshotContext;
	readonly payload: Readonly<Record<string, CanonicalJsonValue>>;
}

export interface ClientProjectServerOperationEnvelope {
	readonly protocolId: typeof CLIENT_PROJECT_SERVER_PROTOCOL.id;
	readonly protocolVersion: typeof CLIENT_PROJECT_SERVER_PROTOCOL.version;
	...
	readonly status: ClientProjectServerOperationStatus;
...
export interface ClientProjectServerEventEnvelope {
	readonly protocolId: typeof CLIENT_PROJECT_SERVER_PROTOCOL.id;
	readonly protocolVersion: typeof CLIENT_PROJECT_SERVER_PROTOCOL.version;
	readonly kind: "event";
...
export function normalizeClientProjectServerCommand(
	value: unknown,
	now: Date = new Date(),
	...
	return envelope<ClientProjectServerCommandEnvelope>({
...
export function normalizeClientProjectServerRequestContext(
	value: unknown,
...
export function normalizeClientProjectServerQuery(value: unknown): ClientProjectServerQueryEnvelope {
	const input = exactObject(value, QUERY_FIELDS, "Client/Server query");
	assertProtocol(input, "query");
	const maxItems = integer(input.maxItems, "maxItems", 1, CLIENT_PROJECT_SERVER_PROTOCOL.maxQueryItems);
	return envelope<ClientProjectServerQueryEnvelope>({
		protocolId: CLIENT_PROJECT_SERVER_PROTOCOL.id,
		protocolVersion: CLIENT_PROJECT_SERVER_PROTOCOL.version,
		kind: "query",
		transportRequestId: text(input.transportRequestId, "transportRequestId"),
...
export function normalizeClientProjectServerQueryResult(
	value: unknown,
	...
	return envelope<ClientProjectServerQueryResultEnvelope>({
		protocolId: CLIENT_PROJECT_SERVER_PROTOCOL.id,
		protocolVersion: CLIENT_PROJECT_SERVER_PROTOCOL.version,
		kind: "query_result",
		transportRequestId: text(input.transportRequestId, "transportRequestId"),
...
export function normalizeClientProjectServerOperation(
	value: unknown,
): ClientProjectServerOperationEnvelope {
	...
	if (!(["accepted", "running", "succeeded", "failed", "cancelled"] as const).includes(status as ClientProjectServerOperationStatus)) {
		throw new Error("Client/Server operation status is unsupported.");
	}
	return envelope<ClientProjectServerOperationEnvelope>({
		protocolId: CLIENT_PROJECT_SERVER_PROTOCOL.id,
		protocolVersion: CLIENT_PROJECT_SERVER_PROTOCOL.version,
		kind: "operation",
		...
		status: status as ClientProjectServerOperationStatus,
		acceptedAt,
		updatedAt,
...
export function normalizeClientProjectServerEvent(value: unknown): ClientProjectServerEventEnvelope {
	const input = exactObject(value, EVENT_FIELDS, "Client/Server event");
	assertProtocol(input, "event");
	return envelope<ClientProjectServerEventEnvelope>({
		protocolId: CLIENT_PROJECT_SERVER_PROTOCOL.id,
		protocolVersion: CLIENT_PROJECT_SERVER_PROTOCOL.version,
		kind: "event",
...
export function runtimeSemanticIdempotencyDigest(
	command: ClientProjectServerCommandEnvelope,
): Sha256Digest {
	return canonicalJsonDigest({
...
function requestContext(input: Record<string, unknown>): ClientProjectServerRequestContext {
...
function snapshotContext(value: unknown): ClientProjectServerSnapshotContext {
	...
	if (!(["complete", "partial", "unavailable"] as const).includes(coverage as ClientProjectServerCoverage)) {
		throw new Error("Client/Server snapshot coverage is unsupported.");
	}
	if (!Array.isArray(input.provenanceRefs) || input.provenanceRefs.length > CLIENT_PROJECT_SERVER_PROTOCOL.maxProvenanceRefs) {
	...
	return {
		...
		coverage: coverage as ClientProjectServerCoverage,
...
function assertProtocol(input: Record<string, unknown>, kind: string): void {
	if (
		input.protocolId !== CLIENT_PROJECT_SERVER_PROTOCOL.id ||
		input.protocolVersion !== CLIENT_PROJECT_SERVER_PROTOCOL.version ||
		input.kind !== kind
	) {
...
function payloadObject(
	...
	if (Buffer.byteLength(canonicalJson(payload), "utf8") > CLIENT_PROJECT_SERVER_PROTOCOL.maxPayloadBytes) {
		throw new Error(`${label} exceeds ${CLIENT_PROJECT_SERVER_PROTOCOL.maxPayloadBytes} canonical UTF-8 bytes.`);
	}
