...
async function cachedProjectionMessage(
	...
	const resume = await buildCodewikiResumeContext(project, {
...
function isDurableCheckpointMessage(
	...
	if (isCodewikiProjectionOrKickoff(message)) return true;
...
function isCodewikiProjectionOrKickoff(message: AgentMessageLike): boolean {
	if (message?.customType === CODEWIKI_CONTEXT_PROJECTION_CUSTOM_TYPE) {
		return true;
	}
	...
	const text = messageText(message);
	return (
		text.includes(CODEWIKI_CONTEXT_PROJECTION_HEADER) ||
