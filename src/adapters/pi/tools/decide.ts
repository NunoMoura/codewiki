...
export function registerCodewikiDecideTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		description:
			"Normal CodeWiki decision workflow tool for diff rows, approvals, KB mappings, and decision-build creation.",
		promptSnippet:
		...
		promptGuidelines: [
			...
			"Use row_actions to approve/reject/defer/edit pending diff-table rows, then create a decision_build with row_to_kb_mappings and propagation evidence.",
			"Do not call low-level wiki_diff_table or wiki_build for normal decision work unless expert compatibility behavior is explicitly required.",
		],
