...
import {
	...
	type ControlRoomServerHandle,
} from "../../../ui/web/control-room.ts";
...
export function registerUiCommand(pi: ExtensionAPI): void {
	pi.registerCommand("wiki-ui", {
		description:
			"Start the standalone local CodeWiki UI. Usage: /wiki-ui [repo-path] [port]",
			...
			await withUiErrorHandling(ctx, async () => {
				...
				if (existing) {
					...
					ctx.ui.notify(
						...
						opened.opened ? "info" : "warning",
					);
					return;
				}
				const server = await startControlRoomServer(project, {
					port: parsed.port,
				});
				activeControlRooms.set(project.root, server);
				const opened = await tryOpenUrlInBrowser(server.url);
				ctx.ui.notify(
					formatControlRoomLaunchMessage(project.label, server.url, opened, {
...
export function parseUiArgs(args: string): {
	pathArg: string | null;
	port?: number;
} {
...
export function tryOpenUrlInBrowser(url: string): Promise<BrowserOpenResult> {
	const browserCommand = buildBrowserOpenCommand(url);
	if (!browserCommand) {
		return Promise.resolve({
			opened: false,
			error: `No browser opener configured for ${process.platform}.`,
		});
	}
	return new Promise<BrowserOpenResult>((resolve) => {
		...
		try {
			...
			child.once("error", (error) =>
				settle({ opened: false, command: commandText, error: error.message }),
			);
			child.once("spawn", () => {
				child.unref();
...
export function formatControlRoomLaunchMessage(
	...
	const prefix = options.alreadyRunning
		? `${projectLabel} CodeWiki UI already running`
		: `${projectLabel} CodeWiki UI started`;
	if (result.opened) {
