...
- Added digest-bound Live Preview profiles with exact package-script content guards, accepted Sprint preview bindings with mandatory DESIGN.md scope, structured package-script supervision, loopback readiness and attach behavior, automatic Implementation-stage startup, system/Playwright browser adapters, Pi-session cleanup, bounded redacted logs, source-harness reuse, and guarded dashboard Open/Capture/Restart/Stop controls without adding a `/wiki-preview` command. Side-effect-free Playwright CLI preflight and verified browser-session state disable unsupported actions and provide explicit install/restart guidance without silently installing software. Explicit Playwright Capture writes accepted desktop/mobile screenshots plus bounded redacted console/network observations to a digest-correlated operational manifest tied to trace state, latest available Implementation iteration, Git head, profile digest, URL, and viewport.
