...
- Added the executable Custom Check contract/config/catalog/policy foundation. Ten closed Check Types now constrain bounded project requirements; Runtime materializes stable id/revision/content digests, immutable lifecycle and observe/warn/approved-require rollout, normalization and tamper checks; project config persists only complete definitions; Check Catalog `2.0.0` emits active definitions only as project-authority Model Checks; policy applies exact loop/Change-kind/layer/path applicability and binds type/evaluator/Knowledge metadata; and Decision Model Check protocol `1.2.0` transports exact Custom Check context through focused calls. Removed broad `ProjectCheckRegistration` without alias or dual path. Dedicated Dashboard commands/UI, protected-base loading, type-route binding, Knowledge-content resolution, batching calibration, and production cutover remain pending.
- Ratified project-bound Custom Checks under closed CodeWiki-owned Check Types. Target design uses bounded declarative requirement text and applicability, protected Git-backed config, Dashboard-first draft/observe/warn/approved-require rollout, type-specific Check Evaluators, separate model Assessments and Runtime-owned Results per Custom Check, protected-base anti-self-disable policy, and calibration-gated focused or type-batched model calls while preserving Code Check and Model Check execution kinds. Implementation remains pending; no production Custom Check path is claimed.
