...
- Added idempotent implementation-worker artifact sanitation. Reconciliation now matches private Assignment packets and Worker reports against canonical Claims, preserves active-Claim and completed evidence, removes pre-Claim or terminal unsuccessful packet/report/output scratch, deletes runtime-local partial worktrees, and runs structured `git worktree prune`. Unknown, completed, external-path, or cleanup-failed artifacts remain fail-safe instead of being deleted.
