...
- `wiki_audit`
  ...
  - Common profiles: `task`, `alignment`, `horizontal-alignment`, `source-contract`, `generated-parity`, `changed`.
...
Before `wiki_roadmap create` or `update`, verify:
...
- Do not create tasks that only group, coordinate, sequence, or close other tasks; use sprint metadata through `wiki_roadmap action="sprint"` after acceptance.
