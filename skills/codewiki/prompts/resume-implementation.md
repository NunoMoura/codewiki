...
- Use `wiki_state` as the map and `/wiki resume`, CodeWiki-owned compaction, or source refs from `wiki_state` as the high-signal restart packet when context is noisy, stale, or token-heavy; then read linked source-of-truth docs/builds/roadmap/validation/code directly before semantic edits.
- Use `wiki_runtime` for focus and narrow write scopes when overlap risk exists.
