...
- Use `wiki_state` as the map and `wiki_resume_context` as the high-signal restart packet when context is noisy, stale, or token-heavy; then read linked source-of-truth docs/builds/roadmap/validation/code directly before semantic edits.
- Use `wiki_session` for focus and `wiki_artifact_status` for narrow write scopes when overlap risk exists.
...
- Compile an `implementation_build` with `wiki_build kind="implementation"` after edits/checks and before requesting implementation validation.
...
- Do not close the task from builder context when policy requires fresh validation/content proof. Use `wiki_roadmap action="update"` for builder evidence; use `action="close"` only after required pass proof exists.
