...
- [x] Resolve sparse Repair Profiles by Check outcome and finding code from project, Pack, and per-Check configuration; ship curated Default Check profiles and strong Custom Check fallbacks with exact variant digests.
- [x] Derive bounded snapshot-bound Repair Frontiers from Change, Candidate, Knowledge, Alignment, Evidence, and Check relationships; report coverage, truncation, staleness, and provenance without turning suggested scope into execution authority.
- [x] Compile matched profiles, structured Result signals, and one exact frontier into an immutable report-bound Repair Brief and Repair Bundle. Keep them separate from Exit Report readiness truth, expose them together as Exit Outcome, and route only bounded matched guidance to Harnesses.
