...
- [x] Clean-cut broad `ProjectCheckRegistration` into bounded `CustomCheckProposal` and Runtime-materialized stable-id/definition-digest contracts under `src/loop-exit/custom-checks/**`; persist complete definitions in project config, materialize active definitions as required through Check Catalog `3.0.0`, and add no alias, dual registration path, caller-authored executor, or migration shim.
