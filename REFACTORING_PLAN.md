...
- [ ] Persist bounded Custom Checks in protected Git-backed project config; implement expected-config-digest CAS, Runtime-owned stable identity and definition/config digests, deterministic applicability, guarded `draft | active | disabled` lifecycle, policy-change review, next-snapshot authority, and protected-base anti-self-disable behavior.
- [ ] Add Dashboard Custom Check library/editor grouped by Check Type, activation/Evidence/agent-feedback/cost preview, generated config diff, calibration view, exact Assessment/Result history, and guarded lifecycle commands shared with CLI/API.
