...
- [x] Add authenticated exact-revision user selection that binds current WorkState, triage projection, and protected policy before creating one Decision job; remove automatic pending-Change Decision selection without an alias or second queue.
...
Current checklist inventory after the authenticated exact-revision Decision selection cut: **69 complete, 48 remaining**.
