...
- [ ] Implement the concrete isolated Node process launcher with bootstrap-key transfer through a private inherited file descriptor rather than argv, environment, logs, or canonical state; bind it to the exact Runner Bundle artifact and fail closed on spawn, handshake, stream, exit, and cleanup faults.
