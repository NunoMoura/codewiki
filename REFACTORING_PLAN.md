...
Stage Producers | Implementation Workers | Checks
        |
Agent Run Supervisor -> isolated DSH Agent Runners -> optional Claude/Codex/ACP delegates
...
src/
  ...
  execution/
    ports.ts
    supervisor/          # Agent Run Supervisor and Runner lifecycle
    dsh/
...
- [x] Define User, Actor, User Interface, Client, CodeWiki Backend, CodeWiki Server, Project Runtime, Agent Run Supervisor, CodeWiki Agent Runner, Backend Plugin, Worker, Assignment, Workbench, and Model Provider in Knowledge.
...
- [ ] Add primary `src/main.ts` standalone CodeWiki Backend composition over Server, per-project Runtime, App, CLI, Agent Run Supervisor, isolated DSH Agent Runners, and Check Runners without moving policy into bootstrap.
...
- [ ] Add Backend status for Server, Runtime, Agent Run Supervisor, Agent and Check Runners, Workbenches, exact DSH/plugin closure, upgrade qualification, cancellation, and recovery without presenting process health as project acceptance.
...
- [x] Persist the canonical qualified-bundle registry and active pointer under Backend custody; copy and reverify exact content-addressed Runner artifact bytes, serialize qualification and activation with expected-generation CAS, retain prior artifacts for same-bundle resume and rollback, and resolve only the exact challenged bundle into the isolated Node process launcher.
...
- [x] Implement the Agent Run Supervisor core and authenticated process protocol: exact expiring challenge/handshake, one-time bootstrap-key MACs, direction/channel/sequence-bound envelopes, immutable Run Specification start, expected-event cancellation CAS, append-only events, quiescence plus process-exit proof, forced termination, and key erasure. The launcher port carries no Runtime persistence, canonical-state, protected-ref, or credential-store handle.
- [x] Implement the concrete isolated Node process launcher with bootstrap-key transfer through a private inherited file descriptor rather than argv, environment, logs, or canonical state; bind it to the exact Runner Bundle artifact, use separate bounded command/event pipes and a closed environment, and fail closed on spawn, framing, stream, non-zero or signal exit, cancellation, and cleanup faults.
...
- [ ] Verify standalone Backend/Server/App/CLI, Agent Run Supervisor, DSH Agent and Check Runners, optional Pi Client, External Agent Client MCP lifecycle, Backend-delegated Claude Code/Codex/ACP lifecycle, Stage Context, state-aware compaction, Check Packs, optional Pack Skills, npm/Git/local installation, manual and App editing, Code/Model execution, Gate outcomes, Backend-owned Ledgers, custody-limited receipts, external capture, guarded writes, failure paths, and cleanup.
...
- standalone CodeWiki Backend, Server, Clients, per-project Runtime, Agent Run Supervisor, isolated DSH Agent and Check Runners, Backend Agent and Delegated Agent Runs, Stage Producers, Implementation Workers, Stage Context, exact Backend-owned Ledgers, custody-limited receipts, state-aware compaction, provenance, MCP, Checks, optional Pack Skills, Check SDK, four Stage Loops, and Gate contracts are executable;
