...
- [ ] Clean-cut public/dashboard use of broad `ProjectCheckRegistration` into narrow `CustomCheckProposal` and Runtime-materialized definition/revision contracts under `src/loop-exit/custom-checks/**`; add no alias, dual registration path, caller-authored executor, or migration shim.
