...
Current `src/verification/**`, `src/change-trace/**`, Loop-named Runtime modules, and legacy trace/work-state packages are migration paths, not target names or permission for duplicate ownership. Every rehome is one clean move with its imports, tests, ownership metadata, and package exports; old-path re-exports, compatibility aliases, dual contracts, and transitional writes are forbidden.
...
- source structure still contains parallel legacy and replacement stacks: `src/loops/**` beside shared `src/verification/**` and Loop-local `exit/**`; `src/traces/**` and legacy Change-record files beside `src/change-trace/**`; and legacy `src/work-state/**` beside canonical `ProjectWorkState` under `src/change-trace/**`;
...
```text
former src/loop-exit/**          → src/verification/**                    # complete
...
- [x] Add CodeWiki-owned `codewiki.production-security-collector@2.0.0` profiles for fixed Semgrep SARIF static analysis, Gitleaks directory SARIF secret detection, and offline Trivy filesystem/advisory SARIF. Bind exact executable bytes/version, Semgrep configuration, Gitleaks rules plus ignore policy, or Trivy database, source snapshot/tree/environment, fixed invocation, bounded output/time, pre/post-run identity checks, credential-free environment, structured no-shell arguments, sanitized finding intake, and explicit unavailable/partial behavior without exposing an arbitrary command or plugin boundary.
- [x] Fan one exact `codewiki.security-scanner-suite@3.0.0` Evidence substrate into independent protected `static_analysis_findings_absent`, `dependency_advisories_absent`, `credential_exposure_absent`, `infrastructure_configuration_verified`, `authorization_controls_verified`, and `persistence_safety_verified` Decision Code Checks through `codewiki.atomic-security-scanner-check@2.0.0`. Each Check depends on `security_scanners_valid`, filters only its exact scanner-family/request Evidence, resolves separate command/source obligations, and receives its own Runtime-created Result; complete families may pass while another fails or remains `indeterminate`, and scanner adapters still grant no Result.
- [x] Close Scanner Suite `3.0.0` at six distinct measured families: static analysis, dependency advisory, secret detection, infrastructure configuration, authorization testing, and migration testing. Clean-cut generic `configuration` to `infrastructure_configuration`, add its independent protected Check, and route parsing/deserialization, process execution, filesystem, cryptography, public-interface, and browser-trust concerns through applicable static-analysis, standard contract/test, or independent Model Checks instead of inventing scanner families without distinct measurements. Advance scanner, atomic-evaluator, collector, and Catalog identities whenever their closed vocabularies change.
- [x] Add off-repository `codewiki.security-route-calibration@1.0.0` admission and scoring for exact Scanner Suite `3.0.0` plus atomic evaluator `2.0.0` route receipts. Require every route to cover human-labeled pass, defect, and unavailable cases for all six families with at least one critical trap; bind source/report/request/environment/configuration/scanner/evaluator/Evidence identity; report false pass/failure, escaped critical defects, `indeterminate`, latency, and cost separately; and block incomplete matrices, protocol drift, any false pass, escaped critical defect, or below-threshold score without granting Result or promotion authority.
- [ ] Run sealed scanner/evaluator route calibration with real off-repository human-labeled cases and production scanner receipts; measure false pass/failure, escaped critical defects, `indeterminate`, latency, and cost.
- [x] Add stronger independent Evidence and separately authenticated authority for high/critical residual risk. Exact high/critical Decision Candidates activate `security_independent_challenge_reviewed` on a distinct no-tool provider/model route plus `security_residual_risk_authorized`; Evidence schema `1.3.0` scopes approval receipts to one exact Check/version/purpose; residual-risk receipts require a qualified security/risk role, an authenticated identity distinct from general Candidate approval, both complete supported Candidate-bound assessments from distinct routes/configurations/producers, exact risk, and digest-only rationale/finding bindings. Missing/disagreeing/unavailable assessments, route aliasing, generic approval reuse, same-identity approval, or wrong-Candidate Evidence remains `indeterminate`.
...
- [x] Clean-cut broad `ProjectCheckRegistration` into bounded `CustomCheckProposal` and Runtime-materialized stable-id/definition-digest contracts under `src/verification/custom-checks/**`; persist complete definitions in project config, materialize active definitions as required through Check Catalog `4.0.0`, and add no alias, dual registration path, caller-authored executor, or migration shim.
...
Current checklist inventory after the Verification package move: **89 complete, 45 remaining**. This is literal checkbox inventory, not effort or readiness percentage.
...
- [x] Replace exported stale `sourceLayout` with one complete internal architecture manifest; validate every active root and target/deprecated path explicitly.
- [x] Add import-boundary and circular-dependency gates. Core/domain packages cannot import outer adapters; Runtime cannot import Pi; Verification cannot import Runtime or Loop implementations; new files/imports in named legacy stacks fail.
- [x] Move former `src/loop-exit/**` to `src/verification/**` with all callers, tests, ownership metadata, scripts, and exports; retain no old-path re-export.
- [ ] Remove `src/runtime/loop-exit-runtime.ts` by moving Loop-specific composition into its Loop package and exposing only generic Runtime ports; preserve Loop-local `exit/**` only for Loop-specific bindings.
