...
try {
	const decision = await writeDecisionBuild(project, {
		...
		decision_table: [
			{
				...
				agreed_change:
					"Require typed planning decision coverage and roadmap reconciliation.",
				expected_final_state:
					"Planning gate blocks prose-only or unresolved residuals.",
		...
		propagation: {
			...
			downstream_planning_questions: [
				"How should planning coverage be represented?",
			],
		},
		...
		risks: ["Fixture risk: planning coverage handoff can be under-specified."],
	});
		...
		roadmap_reconciliation: [
			{
				...
				rationale:
					"Queue order keeps the existing TASK-111 owner and avoids duplicate planning work.",
				source_refs: [decision.path],
			},
		],
	});
	...
	assert.ok(
		partialPreflight.missing.roadmap_reconciliation.some(
			(entry) => entry.includes("partial") && entry.includes("missing_owner"),
		),
	);
} finally {
