...
try {
	...
	const planning = await writePlanningBuild(project, {
		...
		roadmap_reconciliation: [
			{
				id: "task-900-roadmap-review",
				state: "mapped",
				task_ids: [task.id],
				rationale:
					"TASK-900 remains the only active roadmap owner for source-root resume guard work.",
				evidence: "Reviewed fixture queue before implementation handoff.",
				source_refs: [decision.path, task.id],
			},
		],
	});
