...
try {
	const decision = await writeDecisionBuild(project, {
		...
		risks: ["Fixture risk: isolation policy handoff can be under-specified."],
	});
	...
	const planning = await writePlanningBuild(project, {
		...
		roadmap_reconciliation: [
			{
				id: "isolation-roadmap-review",
				state: "mapped",
				task_ids: ["TASK-123"],
				rationale:
					"TASK-123 remains the durable roadmap owner for isolated implementation policy work.",
				evidence: "Reviewed fixture roadmap before implementation handoff.",
				source_refs: [decision.path, "TASK-123"],
			},
		],
	});
	...
	const taskCloseWithoutPublisherBlocked = await writeGatewayReport(project, {
		profile: "task-close",
		task_id: "TASK-123",
		verdict: "pass",
		rationale:
	...
	assert.ok(
		dirtyPublicationBlocked.data.failed_criteria.includes("ship_ready") ||
			dirtyPublicationBlocked.data.failed_criteria.includes(
				"publisher_result_proof",
			),
	);
