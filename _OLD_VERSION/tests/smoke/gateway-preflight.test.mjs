...
assert.ok(scratchRoot, "tests/setup-env must set repo-local TMPDIR");
const root = await mkdtemp(join(scratchRoot, "codewiki-gateway-preflight-"));
...
const decisionRisks = [
	"Fixture risk: shallow decision handoffs can strand accepted work before planning.",
];
const decisionNonGoals = [
	"Do not implement source changes during decision gate fixture setup.",
];
...
try {
		...
		roadmap_reconciliation: [
			{
				...
				task_ids: ["TASK-777"],
				rationale:
					"Queue order keeps TASK-777 as the sole owner of the accepted gateway work.",
			},
		],
		...
		acceptance_mapping: [
			{
				...
				evidence:
					"Smoke assertions cover missing linter/content/source evidence.",
			},
		],
		closure_brief: {
		...
		missingMetadata.diagnostics.some(
			(diagnostic) =>
				diagnostic.remediation
					.join("\n")
					.includes(
						"Return gate feedback to the originating implementation loop",
					) &&
				diagnostic.remediation
					.join("\n")
					.includes("without asking the end user to supply refs"),
		),
	);
	assert.ok(
	...
	const shipReadyCandidate = buildGatewayPreflight(project, {
		...
		verdict: "pass",
		rationale:
			"Ship-ready gate evidence covers quality, not publication approval.",
		source: semanticImplementation.path,
		audit_refs: [
			"audit:alignment",
			"audit:package",
						...
						issues: [
							{
								...
								message:
									"Gateway tool behavior changed without residual coverage.",
							},
						],
					},
				},
	...
	const shallowDecisionHandoff = await writeDecisionBuild(project, {
		kind: "decision",
		summary: "Accept shallow handoff fixture.",
		decision_table: decisionTableFixture([
			{
				id: "SHALLOW-HANDOFF",
				current_state: "Decision pass can omit roadmap-ready handoff evidence.",
				desired_state:
					"Decision pass captures risks, non-goals, and planning handoff semantics.",
				rationale: "Regression fixture.",
				affected_layers: ["system", "roadmap"],
				user_action: "approved",
			},
		]),
		row_to_kb_mappings: [
			{
				row_id: "SHALLOW-HANDOFF",
				knowledge_refs: [".codewiki/kb/system/validation-gateway.md"],
				evidence: "Validation gateway docs capture the decision handoff rule.",
			},
		],
		propagation: {
			direction: "system-first",
			product_impact: ["Agents receive roadmap-ready decision handoffs."],
		},
		knowledge_changes: [".codewiki/kb/system/validation-gateway.md"],
	});
	const shallowDecisionHandoffPreflight = buildGatewayPreflight(project, {
		profile: "decision",
		verdict: "pass",
		rationale: "Decision handoff must be roadmap-ready.",
		source: shallowDecisionHandoff.path,
		audit_refs: decisionAuditRefs,
	});
	assert.equal(shallowDecisionHandoffPreflight.status, "blocked");
	assert.ok(
		shallowDecisionHandoffPreflight.missing.decision_handoff.includes(
			"decision_build:missing_risks",
		),
	);
	assert.ok(
		shallowDecisionHandoffPreflight.missing.decision_handoff.includes(
			"decision_build:missing_non_goals",
		),
	);
	assert.ok(
		shallowDecisionHandoffPreflight.missing.decision_handoff.includes(
	...
	assert.equal(
		shallowDecisionHandoffPreflight.routing.recommended_next_loop,
		"decision",
	);
	const shallowDecisionHandoffReport = await writeGatewayReport(project, {
		profile: "decision",
		verdict: "pass",
		rationale: "Decision handoff missing roadmap-ready metadata.",
		source: shallowDecisionHandoff.path,
		audit_refs: decisionAuditRefs,
	});
	assert.equal(shallowDecisionHandoffReport.data.verdict, "block");
	assert.ok(
		shallowDecisionHandoffReport.data.failed_criteria.includes(
			"decision_handoff",
		),
	);
		...
		roadmap_reconciliation: [
			{
				status: "reviewed",
				evidence:
					"Existing roadmap reviewed; TASK-900 and SPRINT-777 need no new active task for knowledge-only and non-executable deferred rows.",
				task_ids: ["TASK-900"],
				sprint_ids: ["SPRINT-777"],
				rationale:
		...
		decision_row_resolutions: [
			{
				...
				rationale: "Wait for TASK-070 evidence before follow-up.",
				evidence:
		...
		roadmap_reconciliation: [
			{
				status: "reviewed",
				evidence:
					"Existing roadmap reviewed; TASK-070 remains the durable owner, TASK-900 stays the active residual owner, and SPRINT-777 needs no duplicate task.",
				task_ids: ["TASK-070", "TASK-900"],
				sprint_ids: ["SPRINT-777"],
				rationale:
					"Queue order keeps TASK-070 as the durable follow-up owner without duplicate work.",
			},
		],
	});
	...
	const mechanicalImplementation = await writeImplementationBuild(project, {
		...
		test_design_evidence: [
			"Generated refresh reviewed by graph parity linter.",
		],
	...
	const publicationPassed = await writeGatewayReport(project, {
		...
		task_id: "TASK-777",
		verdict: "pass",
		rationale: "Publication approval and evidence present.",
		source: semanticImplementation.path,
