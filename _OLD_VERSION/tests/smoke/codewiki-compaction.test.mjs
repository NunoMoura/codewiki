...
const preservedAfterCompaction = contextRefreshObserverPercentAfterCompaction(
	83,
	null,
);
assert.equal(
	shouldTriggerCodewikiThresholdRefresh(
		{ percent: 85 },
		preservedAfterCompaction,
		80,
	),
	false,
	"post-compaction observer state must not reset to first-observation mode while still above threshold",
);
assert.equal(
	shouldTriggerCodewikiThresholdRefresh(
		{ percent: 85 },
		contextRefreshObserverPercentAfterCompaction(83, 40),
		80,
	),
	true,
	"post-compaction observer should allow a later low-to-high threshold crossing",
);
...
const gatewayBlockRequest = buildPostGatewayContextRefreshRequest({
	profile: "implementation",
	verdict: "block",
	taskId: "TASK-001",
	source: ".codewiki/builds/implementation/impl.json",
	validationRef: ".codewiki/validation/impl-block.json",
});
assert.equal(
	gatewayBlockRequest.reason,
	"implementation-gateway-block-feedback-boundary",
	"blocked gateway verdicts should return bounded feedback to the originating compiler loop",
);
assert.match(
	gatewayBlockRequest.followUpIntent,
	/Resume originating compiler loop after implementation gateway block/,
);
assert.match(gatewayBlockRequest.followUpIntent, /Gate feedback source refs/);
...
async function createCompactionFixture({ openTask = true } = {}) {
	...
	if (openTask) {
		const decision = await writeDecisionBuild(project, {
			kind: "decision",
			summary: "Compaction resume evidence decision.",
			decision_table: decisionTableFixture([
				{
					id: "COMPACTION-RESUME",
					current_state: "TASK-001 lacks planning evidence.",
					desired_state:
						"TASK-001 has validated planning evidence for compaction resume.",
					rationale:
						"Resume context enforcement requires planning-gateway evidence.",
			...
			row_to_kb_mappings: [
				{
					...
					evidence: "Fixture graph doc captures compaction resume evidence.",
				},
			],
			propagation: {
			...
			non_goals: [
				"Do not implement source changes in compaction fixture setup.",
			],
			risks: [
				"Fixture risk: missing decision handoff evidence blocks planning.",
			],
		});
		await writeGatewayReport(project, {
			profile: "decision",
			verdict: "pass",
			rationale: "Decision pass for compaction resume evidence.",
		...
		const planning = await writePlanningBuild(project, {
			kind: "planning",
			summary: "Compaction resume evidence planning.",
			...
			decision_row_resolutions: [
				{
					...
					task_ids: ["TASK-001"],
					evidence: "TASK-001 carries compaction resume evidence.",
					source_refs: [decision.path, "TASK-001"],
				},
			],
			roadmap_reconciliation: [
				{
					id: "COMPACTION-RESUME-ROADMAP",
					row_id: "COMPACTION-RESUME",
					resolution: "roadmap-task",
					task_ids: ["TASK-001"],
					evidence:
						"Existing roadmap task TASK-001 carries compaction resume work.",
					rationale:
						"TASK-001 is the only open executable fixture task, so queue order remains valid after planning consumes the decision.",
					source_refs: [decision.path, "TASK-001"],
				},
			],
			...
			tdd_plan: ["Compaction smoke uses planning evidence."],
		...
		await writeGatewayReport(project, {
			profile: "planning",
			verdict: "pass",
			rationale: "Planning pass for compaction resume evidence.",
