...

{
	...
	assert.ok(
		result.fresh_worker.request.context_boundary.blockers.includes(
			"content_evidence:immutable_content_ref",
		),
	);
	assert.equal(
		result.fresh_worker.blockers[0].recommended_next_loop,
		"implementation",
	);
	assert.match(
		result.fresh_worker.blockers[0].remediation.join("\n"),
		/originating implementation\/compiler loop/,
	);
	assert.match(
		result.fresh_worker.blockers[0].remediation.join("\n"),
		/without asking the end user to supply refs/i,
	);
	assert.doesNotMatch(
		result.fresh_worker.blockers[0].remediation.join("\n"),
		/^Provide /i,
	);
	assert.equal(result.fresh_worker.handoff.next_loop, "implementation");
	assert.equal(result.workflow_efficiency.user_interruptions_required, 0);
	assert.equal(result.workflow_efficiency.manual_commands_required, 0);
}
...

{
	...
	await writeJson(buildPath, {
		...
		task_id: task.id,
		summary: "Fixture implementation build missing validation evidence.",
