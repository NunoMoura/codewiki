...

{
	...
	const contentEvidenceKind = "content_" + "pr" + "oof";
	assert.equal(alignment.model, "derived-vertical-state-machine");
	assert.deepEqual(alignment.precedence.slice(0, 2), [contentEvidenceKind, "canonical_source"], "Content evidence must outrank gate reports and graph state");
	...
	assert.ok(graph.nodes.some((node) => node.id === `${contentEvidenceKind}:${checkedSha}` && node.kind === contentEvidenceKind), "Content evidence node missing");
...

{
	...
	assert.ok(graph.views.reconciliation.items.some((item) => item.id === `reconcile:publication-evidence:${implementationPath}` && item.next_loop === "validation"), "Publication claim without content evidence should route to validation");
	const row = graph.views.traceability.rows.find((entry) => entry.requirement_id === "CHANGE-001");
	assert.ok(row?.gaps.includes("missing_publication_content_proof"), "Traceability should expose missing publication/content-evidence edge");
}
