...
function assertIssue(report, kind) {
	assert.ok(
		...
		`expected ${kind}, got ${report.issues.map((issue) => issue.kind).join(", ")}`,
	);
}
...
function createThinkCodeStylePackageFixture({
	includeSkillsInFiles = true,
} = {}) {
	...
	writeJson(resolve(root, ".codewiki", "config.json"), {
		project_name: "package-neutral-fixture",
	});
	...
	write(
		resolve(root, "skills", "think-code", "SKILL.md"),
		"---\nname: think-code\ndescription: Fixture skill for package-neutral linter tests.\n---\n# Think Code\n",
	);
	write(
		resolve(root, "skills", "think-code", "references", "usage.md"),
		"# Usage\n",
	);
	write(resolve(root, "README.md"), "# Think Code Fixture\n");
	write(resolve(root, "LICENSE"), "MIT\n");
	writeJson(resolve(root, "package-lock.json"), {
		name: "think-code-linter-fixture",
		version: "0.0.0",
		lockfileVersion: 3,
		packages: { "": { name: "think-code-linter-fixture", version: "0.0.0" } },
	});
...
function createFixture() {
	const root = mkdtempSync(resolve(tmpdir(), "codewiki-linter-drift-"));
	...
	writeJson(resolve(root, ".codewiki", "config.json"), {
		project_name: "linter-drift-fixture",
	});
	write(
	...
	writeJson(
		resolve(root, ".codewiki", "roadmap", "tasks", "TASK-001", "task.json"),
		{ ...task, title: "Stale generated task" },
	);
	writeJson(
		resolve(root, ".codewiki", "roadmap", "tasks", "TASK-001", "context.json"),
		{ version: 1, task: { ...task, summary: "Stale context summary" } },
	);
	writeJson(resolve(root, ".codewiki", "index_graph.json"), {
		...
		views: {
			decision_propagation: {
				...
				residuals: [
					{
						decision_build: ".codewiki/builds/decision/file-structure.json",
						kind: "row",
						id: "FS-HUMAN-DRIVEN-SURFACE",
						gaps: ["row:FS-HUMAN-DRIVEN-SURFACE:missing_resolution"],
					},
				],
			},
		},
	...
	write(
		...
		"This stale fixture still points at extensions/codewiki/src. It also says .codewiki/ stores package source and generated task views are canonical truth.\n",
	);
	write(resolve(root, "src", "index.ts"), "export const ok = true;\n");
	write(resolve(root, "src", "core", "bad.ts"), "export const bad = true;\n");
	write(
		resolve(root, "src", "domain", "bad", "imports-application.ts"),
		"import { bad } from '../../application/bad/imports-adapter.ts';\nexport const domainBad = bad;\n",
	);
	write(
		resolve(root, "src", "application", "bad", "imports-adapter.ts"),
		"import { adapterBad } from '../../adapters/pi/nope.ts';\nexport const bad = adapterBad;\n",
	);
	write(
		resolve(root, "scripts", "check-architecture.mjs"),
	...
	write(
		resolve(root, "scripts", "rogue.mjs"),
		"const lintEvidence = ['linter'];\nconsole.log('.codewiki/roadmap/queue.json', lintEvidence);\n",
	);
	write(
		resolve(root, "skills", "codewiki", "SKILL.md"),
		"---\nname: codewiki\ndescription: fixture\n---\n# Skill\n",
	);
	writeJson(resolve(root, "package.json"), {
		name: "linter-drift-fixture",
		version: "0.0.0",
		type: "module",
		files: [
			"src",
...
async function main() {
	...
	try {
		...
		const alignment = await executeCodewikiAudit(project, {
			profiles: ["alignment"],
		...
		const fileStructure = await executeCodewikiAudit(project, {
		...
		const staleReference = await executeCodewikiAudit(project, {
			profiles: ["stale-reference"],
			include_fingerprints: false,
		});
		...
		const generatedParity = await executeCodewikiAudit(project, {
			profiles: ["generated-parity"],
			include_fingerprints: false,
		});
		...
		const packageAudit = await executeCodewikiAudit(project, {
			profiles: ["package"],
			include_fingerprints: false,
		});
	...
	try {
		...
		const neutralAudit = await executeCodewikiAudit(neutralProject, {
			profiles: ["package"],
		...
		assert.equal(
			neutralAudit.status,
			"pass",
			`package-neutral fixture should pass: ${neutralAudit.issues.map((issue) => `${issue.kind}:${issue.message}`).join(" | ")}`,
		);
		assertNoIssue(neutralAudit, "skill-asset-unreachable");
		assertNoIssue(neutralAudit, "package-dry-run-missing");
		assert.ok(
			neutralAudit.fingerprints.some(
				(item) => item.path === "skills/think-code/SKILL.md",
			),
			"package linter should fingerprint discovered skill manifest",
		);
		assert.ok(
			neutralAudit.fingerprints.some(
				(item) => item.path === "skills/think-code/references/usage.md",
			),
			"package linter should fingerprint discovered skill asset files",
		);
		assert.ok(
			!neutralAudit.issues.some((issue) =>
				`${issue.path ?? ""} ${issue.message}`.includes("skills/codewiki"),
			),
			"package linter must not require CodeWiki skill paths for other packages",
		);
	} finally {
		rmSync(neutralRoot, { recursive: true, force: true });
	}

	const omittedSkillRoot = createThinkCodeStylePackageFixture({
		includeSkillsInFiles: false,
	});
	try {
		const omittedProject = await loadProject(omittedSkillRoot);
		const omittedAudit = await executeCodewikiAudit(omittedProject, {
			profiles: ["package"],
			include_fingerprints: false,
		});
		assert.ok(
			omittedAudit.issues.some(
				(issue) =>
					issue.kind === "package-dry-run-missing" &&
					issue.message.includes("skills/think-code/SKILL.md"),
			),
			"package linter should require discovered package skill assets in npm pack output",
		);
	} finally {
...
main().then(() => console.log("✓ linter drift fixture passed"));
