...
const isolatedScratchRoot = resolve(
...
function withTempDir(prefix, fn) {
	mkdirSync(isolatedScratchRoot, { recursive: true });
	const dir = mkdtempSync(path.join(isolatedScratchRoot, prefix));
...
async function main() {
	...
	const verifierSmokeDir = mkdtempSync(
		resolve(isolatedScratchRoot, "codewiki-verifier-parser-"),
	);
	...
	await withTempDir("codewiki-bootstrap-", async (projectDir) => {
		...
		const resumeDecision = await preResumeBuildTool.definition.execute(
			"resume-decision-proof-smoke",
			{
				...
				risks: ["Fixture risk: resume proof handoff can be under-specified."],
			},
			undefined,
			undefined,
			outsideToolCtx,
				...
				roadmap_reconciliation: [
					{
						status: "reviewed",
						evidence:
							"Existing roadmap reviewed; TASK-001 owns the resume proof while TASK-002, TASK-003, and SPRINT-001 need no duplicate work.",
						task_ids: ["TASK-001", "TASK-002", "TASK-003"],
						sprint_ids: ["SPRINT-001"],
						rationale:
							"Queue and sprint order stay unchanged because TASK-001 is the implementation owner for the resume proof fixture.",
					},
				],
		...
		const buildResult = await buildTool.definition.execute(
			"build-tool-smoke",
			{
				...
				risks: [
					"Fixture risk: decision handoff semantics can be under-specified.",
				],
				...
				roadmap_reconciliation: [
					{
						status: "reviewed",
						evidence:
							"Existing roadmap reviewed; TASK-001 owns the implementation fixture while TASK-002, TASK-003, and SPRINT-001 need no duplicate work.",
						task_ids: ["TASK-001", "TASK-002", "TASK-003"],
						sprint_ids: ["SPRINT-001"],
						rationale:
							"Queue and sprint order stay unchanged because TASK-001 is the implementation owner for the smoke fixture.",
					},
				],
				...
				roadmap_reconciliation: [
					{
						status: "reviewed",
						evidence:
							"Existing roadmap reviewed; TASK-002 owns the explicit resume proof while active smoke tasks and sprints need no duplicate work.",
						task_ids: [
							"TASK-001",
							"TASK-002",
							"TASK-003",
						...
						sprint_ids: ["SPRINT-001", "SPRINT-002"],
						rationale:
							"Queue and sprint order stay unchanged because TASK-002 is the implementation owner for the explicit resume fixture.",
					},
				],
		...
		assert.equal(
			...
			26,
			`Expected 26 created starter files including lexicon, product users/stories/uis, flat system docs, roadmap queue, and inferred boundary specs, got ${first.created.length}`,
		);
		assert.equal(
