...
function normalizeLintOwnershipPath(value: string) {
	return value.trim().replace(/\\/g, "/").replace(/^\.\//, "");
}

function lintOwnershipPathMatches(pattern: string, path: string) {
	const normalizedPattern = normalizeLintOwnershipPath(pattern);
	const normalizedPath = normalizeLintOwnershipPath(path);
...
function lintIssueKey(issue: LintIssue) {
	return [
		"lint",
		issue.kind || "unknown",
		issue.path || "",
...
function lintIssueText(issue: LintIssue) {
	return [issue.kind, issue.path, issue.message].join("\n").toLowerCase();
}

function activeRoadmapEntries(entries: RoadmapTaskRecord[]) {
	return entries.filter((entry) =>
		OPEN_ROADMAP_STATUSES.has(String(entry.status || "todo").trim()),
	);
}

function lintIssueOwnerTaskIds(
	issue: LintIssue,
	roadmapEntries: RoadmapTaskRecord[],
): string[] {
	const text = lintIssueText(issue);
	const issuePath = String(issue.path || "").trim();
	return activeRoadmapEntries(roadmapEntries)
		.filter((task) => {
			const taskId = String(task.id || "").trim();
			if (taskId && text.includes(taskId.toLowerCase())) return true;
			const paths = [...(task.spec_paths || []), ...(task.code_paths || [])];
			return paths.some((pattern) =>
				lintOwnershipPathMatches(pattern, issuePath),
			);
		})
...
function annotateLintIssueOwnership(
	issue: LintIssue,
	roadmapEntries: RoadmapTaskRecord[],
): LintIssue {
	const actionable = ["warning", "error"].includes(
		String(issue.severity || ""),
	);
	const ownerTaskIds = actionable
		? lintIssueOwnerTaskIds(issue, roadmapEntries)
		: [];
	const ownershipState: LintOwnershipState = !actionable
		? "non_actionable"
		: ownerTaskIds.length > 0
			? "covered_by_task"
			: "unowned_actionable";
	return {
		...issue,
		actionable,
		residual_issue_key: lintIssueKey(issue),
		ownership_state: ownershipState,
		owner_refs: ownerTaskIds,
	};
}
...
function lintImplementationBuildV2(
	...
	["implemented_changes", "acceptance_evidence", `che${"cks"}`].forEach(
		(field) => {
...
export function buildLintReport(
	...
	const ownedIssues = issues.map((issue) =>
		annotateLintIssueOwnership(issue, roadmapEntries),
	);
	const counts: Record<string, number> = {};
	ownedIssues.forEach((issue) => {
