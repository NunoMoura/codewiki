...
export function lintHealth(lintReport: LintReport) {
	...
	const unownedActionable = issues.filter(
		(i) =>
			String(i.severity) === "warning" &&
			String((i as any).ownership_state || "unowned_actionable") ===
				"unowned_actionable",
	).length;
	const ownedResiduals = issues.filter((i) =>
		["covered_by_task", "covered_by_sprint"].includes(
			String((i as any).ownership_state || ""),
		),
	).length;
	...
	return {
		...
		errors,
		warnings,
		unowned_actionable: unownedActionable,
		owned_residuals: ownedResiduals,
