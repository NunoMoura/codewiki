...
function buildSemanticExecutionClosureView(input: {
	...
	return {
		...
		model:
			"approved-decision-row-through-planning-work-implementation-gate-evidence",
		invariant:
			"generated_view_not_canonical_truth; canonical truth remains accepted builds, roadmap, gate reports, and Git/content evidence",
		summary: {
...
export function buildGraph(inputs: GraphBuildInputs): GraphFile {
	...
	const graphAccumulator = new GraphAccumulator();
	const { nodes, edges } = graphAccumulator;
	const addNode = graphAccumulator.addNode;
	const addEdge = graphAccumulator.addEdge;
	...
	for (const build of builds) {
		...
		if (
			...
			reconciliationItems.push({
				id: `reconcile:publication-evidence:${buildPath}`,
				source_id: buildId,
				state: "drift",
				direction: "gateway",
				from_layer: "publication",
				to_layer: "content_evidence",
				next_loop: "validation",
				task_id: firstTaskId(build),
				reason:
					"Publication claim lacks immutable content evidence such as commit/tree SHA, package digest, archive ledger, or remote ref.",
	...
	for (const issue of lintReport?.issues || []) {
		...
		const ownershipState = String(
			(issue as any).ownership_state || "unowned_actionable",
		);
		const ownerRefs = stringList((issue as any).owner_refs);
		const relatedOpenTaskIds = unique([
			...(openTasksBySpec.get(issuePath) || []),
			...ownerRefs.filter((ref) => /^TASK-/.test(ref)),
		]);
		if (
			ownershipState !== "unowned_actionable" ||
			relatedOpenTaskIds.length > 0
		)
			continue;
		...
		reconciliationItems.push({
			...
			to_layer: "roadmap",
			next_loop: "planning",
			ownership_state: "unowned_actionable",
			owner_refs: ownerRefs,
			reason: `Linter ${issue.severity} (${issue.kind}) is an unowned actionable finding; fix it or record durable residual coverage before promotion.`,
			doc_paths: [issuePath],
		});
	}
	...
	const cursorScope: WorkflowCursor["scope"] = activeSprintIds[0]
		...
		: openTaskIds[0]
			...
			: { kind: "roadmap" };
	const workflowCursor: WorkflowCursor = {
