	...
	try {
		...
		const issueItems = lintIssues
			...
			.map((issue: any) => {
				...
				return {
					key:
						String(issue?.residual_issue_key || "").trim() ||
						residualIssueKey({ kind, path, message }),
					kind,
					path,
					message,
					refs,
					...
					ownership_state:
						String(issue?.ownership_state || "").trim() || undefined,
					owner_refs: trimList(issue?.owner_refs),
				};
			});
		...
		for (const item of reconciliationItems) {
			...
			byKey.set(id, {
				...
				actionable: true,
				ownership_state:
					String(item?.ownership_state || "").trim() || undefined,
				owner_refs: trimList(item?.owner_refs),
			});
		}
		return [...byKey.values()];
	} catch {
...
function residualCoverageOwnerRefs(entry: ResidualIssueCoverageInput) {
	return unique([
		...
		String(entry.decision_build_ref || "").trim(),
		String(entry.restore_ref || "").trim(),
		...trimList(entry.refs),
	]).filter(Boolean);
}

function residualCoverageMatchRefs(entry: ResidualIssueCoverageInput) {
...
function residualCoverageEntryMatchesIssue(
	entry: ResidualIssueCoverageInput,
	item: ResidualIssueItem,
) {
	const { paths, refs } = residualCoverageMatchRefs(entry);
	const keyMatches = refs.includes(item.key);
	const kindMatches =
		String(entry.issue_kind || "").trim() === item.kind ||
		refs.includes(item.kind);
	const pathMatches = paths.some((pattern) =>
		[item.path, ...item.refs].some((path) =>
			globPatternMatchesPath(pattern, path),
		),
	);
	const refMatches = refs.some((ref) => residualItemText(item).includes(ref));
	return keyMatches || kindMatches || pathMatches || refMatches;
}
...
function explicitCoverageCoversIssue(
	...
	if (
		...
		!residualCoverageEntryMatchesIssue(entry, item)
	)
		return false;
	...
	const ownerRefs = residualCoverageOwnerRefs(entry);
	const trigger = String(entry.trigger || "").trim();
	const routeBack = String(entry.route_back || "").trim();
	if (classification === "fixed") return false;
	...
	if (classification === "deferred_by_decision")
		return Boolean(
			trigger && routeBack && String(entry.decision_build_ref || "").trim(),
		);
	if (classification === "accepted_compatibility")
		return Boolean(trigger && ownerRefs.length > 0);
	if (classification === "archive_candidate")
		return Boolean(
			trigger &&
				(String(entry.restore_ref || "").trim() || ownerRefs.length > 0),
		);
	if (classification === "false_positive")
		return Boolean(trigger || routeBack || ownerRefs.length > 0);
	return false;
}

function residualGateScopeRefs(
	project: WikiProject,
	input: CodewikiValidationReportInput,
	build: any,
) {
	...
	return unique([
		...
		...trimList(build?.source_refs),
		...trimList(build?.knowledge_changes),
		...trimList(build?.roadmap_changes),
		...trimList(build?.code_files),
		...trimList(build?.test_files),
		...trimList(build?.candidate_code_paths),
		...trimList(build?.candidate_test_files),
		...trimList(build?.produces?.knowledge),
		...trimList(build?.produces?.roadmap),
		...trimList(build?.produces?.code),
		...trimList(build?.produces?.tests),
		...trimList(build?.consumes?.source),
		...trimList(build?.consumes?.roadmap),
		...trimList(build?.consumes?.decision),
		...trimList(build?.consumes?.planning),
		...trimList(build?.consumes?.implementation),
		...taskPaths,
	]).filter(Boolean);
}

function residualIssueInGateScope(
	item: ResidualIssueItem,
	scopeRefs: string[],
) {
	if (scopeRefs.length === 0) return true;
	const text = residualItemText(item);
	const paths = [item.path, ...item.refs].filter(Boolean);
	return scopeRefs.some((ref) => {
		const normalized = String(ref || "").trim();
		if (!normalized) return false;
		if (text.includes(normalized.toLowerCase())) return true;
		return paths.some(
			(path) =>
				globPatternMatchesPath(normalized, path) ||
				globPatternMatchesPath(path, normalized),
		);
	});
}
...
function validationResidualIssueCoverage(
	...
	const scopeRefs = residualGateScopeRefs(project, input, build);
	const items = readCurrentResidualIssueItems(project).filter((item) =>
		residualIssueInGateScope(item, scopeRefs),
	);
	if (items.length === 0)
		return {
			required,
	...
	const gaps: string[] = [];
	const ownership = new Map<string, { state: string; owner_refs: string[] }>();
	for (const item of items) {
		const roadmapTask = roadmap.tasks.find((task) =>
			issueMatchesRoadmapTask(item, task),
		);
		const roadmapSprint = roadmap.sprints.find((sprint) =>
			issueMatchesRoadmapSprint(item, sprint),
		);
		const explicitEntry = entries.find((entry) =>
			explicitCoverageCoversIssue(entry, item, roadmap),
		);
		if (roadmapTask) {
			covered.push(item.key);
			ownership.set(item.key, {
				state: "covered_by_task",
				owner_refs: [roadmapTask.id],
			});
			continue;
		}
		if (roadmapSprint) {
			covered.push(item.key);
			ownership.set(item.key, {
				state: "covered_by_sprint",
				owner_refs: [roadmapSprint.id],
			});
			continue;
		}
		if (explicitEntry) {
			covered.push(item.key);
			ownership.set(item.key, {
				state: String(explicitEntry.classification || "covered"),
				owner_refs: residualCoverageOwnerRefs(explicitEntry),
			});
			continue;
		}
		ownership.set(item.key, { state: "unowned_actionable", owner_refs: [] });
		gaps.push(`${item.key}${item.path ? ` (${item.path})` : ""}`);
	}
	return {
		required,
		items: items.map((item) => {
			const state = ownership.get(item.key) || {
				state: item.ownership_state || "unowned_actionable",
...
function hasPlanningEvidence(row: any): boolean {
	return Boolean(
		String(row?.evidence || row?.summary || row?.proof || "").trim() ||
			...
			trimList(row?.work_unit_ids).length ||
...
function planningRoadmapReconciliationGaps(
	...
	if (
		!rows.some((row) =>
			String(
				row?.rationale ||
					row?.reorder_rationale ||
					row?.ordering_rationale ||
					"",
			).trim(),
		)
	)
		gaps.push(`${path}:roadmap_reconciliation:missing_reorder_rationale`);
	const reconciledTaskIds = unique(
		rows.flatMap((row) => [
			...trimList(row?.task_ids),
			...trimList(row?.roadmap_task_ids),
		]),
	);
	const reconciledSprintIds = unique(
		rows.flatMap((row) => trimList(row?.sprint_ids)),
	);
	const reconciledWorkUnitIds = unique(
		rows.flatMap((row) => trimList(row?.work_unit_ids)),
	);
	const activeTaskIds = unique(
		roadmap.tasks.map((task) => task.id).filter(Boolean),
	);
	const activeSprintIds = unique(
		roadmap.sprints.map((sprint) => sprint.id).filter(Boolean),
	);
	const executionWorkUnitIds = unique(
		(Array.isArray(planning?.execution_graph?.work_units)
			? planning.execution_graph.work_units
			: []
		)
			.map((unit: any) => String(unit?.id || "").trim())
			.filter(Boolean),
	);
	const missingReviewedTaskIds = activeTaskIds.filter(
		(taskId) => !reconciledTaskIds.includes(taskId),
	);
	if (missingReviewedTaskIds.length) {
		gaps.push(
			`${path}:roadmap_reconciliation:missing_whole_roadmap_task_review:${missingReviewedTaskIds.join(",")}`,
		);
	}
	const missingReviewedSprintIds = activeSprintIds.filter(
		(sprintId) => !reconciledSprintIds.includes(sprintId),
	);
	if (missingReviewedSprintIds.length) {
		gaps.push(
			`${path}:roadmap_reconciliation:missing_whole_roadmap_sprint_review:${missingReviewedSprintIds.join(",")}`,
		);
	}
	const missingReviewedWorkUnitIds = executionWorkUnitIds.filter(
		(workUnitId) => !reconciledWorkUnitIds.includes(workUnitId),
	);
	if (missingReviewedWorkUnitIds.length) {
		gaps.push(
			`${path}:roadmap_reconciliation:missing_execution_graph_work_unit_review:${missingReviewedWorkUnitIds.join(",")}`,
		);
	}
	rows.forEach((row, index) => {
		const id = planningCoverageIdentity(row) || `entry-${index + 1}`;
...
function decisionApprovedRowIds(build: any): string[] {
	const explicit = trimList(build?.approved_decision_rows);
...
function validationDecisionMappingGaps(build: any, profile: string): string[] {
	...
	const approvedIds = new Set(decisionApprovedRowIds(build));
	const rows = Array.isArray(build?.decision_table?.rows)
		? build.decision_table.rows
		: [];
...
function decisionMappings(build: any): any[] {
	return Array.isArray(build?.row_to_kb_mappings)
		? build.row_to_kb_mappings
		: [];
}

function decisionMappingHasNoWorkDisposition(mapping: any): boolean {
	if (mapping?.deferred === true) return true;
	const noWorkEvidence = [
		mapping?.no_kb_impact,
		mapping?.no_knowledge_impact,
		mapping?.no_diagram_impact,
		mapping?.no_impact_rationale,
		mapping?.no_work_rationale,
		mapping?.already_done_evidence,
	];
	return noWorkEvidence.some((value) => String(value || "").trim());
}

function decisionHandoffHasPlanningTarget(build: any): boolean {
	const planningQuestions = unique([
		...trimList(build?.downstream_planning_questions),
		...trimList(build?.propagation?.downstream_planning_questions),
	]);
	const workRoutes = unique([
		...trimList(build?.roadmap_changes),
		...trimList(build?.produces?.roadmap),
	]);
	return (
		...
		workRoutes.length > 0 ||
		decisionMappings(build).some(decisionMappingHasNoWorkDisposition)
	);
}

function validationDecisionHandoffGaps(build: any, profile: string): string[] {
	if (normalizeValidationGate(profile) !== "decision") return [];
	if (String(build?.kind || "").trim() !== "decision_build") return [];
	const gaps: string[] = [];
	if (decisionApprovedRowIds(build).length === 0)
		gaps.push("decision_build:missing_approved_decision_rows");
	if (trimList(build?.risks).length === 0)
		gaps.push("decision_build:missing_risks");
	if (trimList(build?.non_goals).length === 0)
		gaps.push("decision_build:missing_non_goals");
	if (!decisionHandoffHasPlanningTarget(build))
		gaps.push(
			"decision_build:missing_downstream_planning_questions_or_work_route",
		);
	return unique(gaps);
}
...
function gatewayIssueRemediation(
	...
	if (kind.includes("audit"))
		return ["Run or cite the required CodeWiki linter evidence."];
	...
	if (kind.includes("content") || kind.includes("isolation")) {
		return [
			"Return gate feedback to the originating implementation loop; produce or cite clean/immutable content proof from validated content, then rerun fresh validation without asking the end user to supply refs.",
		];
	}
...
function inferValidationRouting(
	...
	const failureClass =
		...
		joined.includes("decision_handoff")
			? "decision_ambiguity"
			: undefined) ||
...
export function buildGatewayPreflight(
	...
	const decisionHandoffGaps = validationDecisionHandoffGaps(
		source.build,
		profile,
	);
	...
	const roadmapReconciliationGaps = planningRoadmapReconciliationGaps(
		...
		profile,
		source.source,
		readRoadmapResidualCoverage(project),
	);
	...
	const missing = {
		...
		decision_handoff: decisionHandoffGaps,
	...
	const issues = [
		...
		...validationPreflightIssue(
			...
			"high",
			decisionHandoffGaps,
			"Decision gate handoff must capture roadmap-ready semantics: approved rows, risks, non-goals, and downstream planning questions or known work routes.",
		),
		...validationPreflightIssue(
			"ambiguity",
			"high",
	...
	const routingSignals = [
		...
		...decisionHandoffGaps.map((gap) => `decision_handoff:${gap}`),
...
export async function writeGatewayReport(
	...
	const decisionHandoffGaps =
		input.verdict === "pass" ? preflight.missing.decision_handoff : [];
	const ambiguityGaps =
	...
	const decisionMappingIssue =
		decisionMappingGaps.length > 0
			...
			: [];
	const decisionHandoffIssue =
		decisionHandoffGaps.length > 0
			? [
					{
						severity: "high",
						summary: `Decision handoff is not roadmap-ready: ${decisionHandoffGaps.join(", ")}.`,
					},
				]
			: [];
	...
	const data = {
		...
		failed_criteria: unique([
			...
			...(decisionHandoffGaps.length > 0 ? ["decision_handoff"] : []),
		...
		blocking_questions: unique([
			...
			...(decisionHandoffGaps.length > 0
				? [
						"Update the decision build with risks, non-goals, and downstream planning questions or explicit roadmap/work-route evidence before planning promotion.",
					]
				: []),
			...(ambiguityGaps.length > 0
				? [
