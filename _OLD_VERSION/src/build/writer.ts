...
function normalizePlanningCoverageEntry(
	...
	const workUnitIds = unique([
		...trimList(entry?.work_unit_ids),
		...trimList(entry?.workUnitIds),
		...trimList(entry?.work_units),
	]);
	...
	return {
		...
		rationale:
			String(
				entry?.rationale ||
					entry?.reorder_rationale ||
					entry?.ordering_rationale ||
					"",
			).trim() || undefined,
	...
	return normalizePlanningCoverageEntry(
		{
			...
			work_unit_ids: (resolution as any)?.work_unit_ids,
