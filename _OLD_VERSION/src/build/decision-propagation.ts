...
function normalizeResolution(
	...
	if (["defer", "deferred", "explicit-deferred"].includes(normalized))
		return "deferred";
	if (["already-implemented", "implemented", "complete"].includes(normalized))
		return "already-implemented";
	if (["route-back", "routeback", "route-back-blocker"].includes(normalized))
		return "route-back";
...
function normalizeResolutionEntry(
	...
	return {
		...
		task_ids: unique([
			...
			...stringList(raw?.roadmap_task_ids),
			...extractTaskIds([raw?.task_id, raw?.roadmap_task_id]),
		]),
		sprint_ids: unique([
			...stringList(raw?.sprint_ids),
			...extractSprintIds([raw?.sprint_id]),
		]),
		work_unit_ids: unique([
			...stringList(raw?.work_unit_ids),
			...stringList(raw?.workUnitIds),
			...stringList(raw?.work_units),
		]),
...
function planningWorkUnitIds(planning: any): string[] {
	const data = planningData(planning);
	const graph = data?.execution_graph || {};
	return unique([
		...list(graph?.work_units).map((unit) => text(unit?.id)),
		...list(graph?.waves).flatMap((wave) => stringList(wave?.work_unit_ids)),
		...list(graph?.lease_plan).flatMap((lease) =>
			stringList([lease?.work_unit_id]),
		),
	]).filter(Boolean);
}
...
function hasDurableRoadmapOwnership(
	entry: NormalizedDecisionPropagationResolution,
): boolean {
	return Boolean(
		entry.task_ids.length ||
			entry.sprint_ids.length ||
			entry.work_unit_ids.length,
	);
}

function appendKnownOwnerGaps(
	entry: NormalizedDecisionPropagationResolution,
	knownTaskIds: Set<string>,
	knownSprintIds: Set<string>,
	knownWorkUnitIds: Set<string>,
	gaps: string[],
) {
	if (
		knownTaskIds.size > 0 &&
	...
	if (
		knownWorkUnitIds.size > 0 &&
		entry.work_unit_ids.length > 0 &&
		!entry.work_unit_ids.some((unitId) => knownWorkUnitIds.has(unitId))
	) {
		gaps.push(`unknown_work_unit:${entry.work_unit_ids.join(",")}`);
	}
}
...
function validateResolution(
	...
	const gaps: string[] = [];
	const hasRoadmapOwnership = hasDurableRoadmapOwnership(entry);
	...
	if (entry.resolution === "roadmap-task") {
		if (entry.task_ids.length === 0 && entry.work_unit_ids.length === 0)
			gaps.push("missing_task_ids");
	}
	if (entry.resolution === "sprint") {
		if (entry.sprint_ids.length === 0 && entry.work_unit_ids.length === 0)
			gaps.push("missing_sprint_ids");
	}
	appendKnownOwnerGaps(
		entry,
		knownTaskIds,
		knownSprintIds,
		knownWorkUnitIds,
	...
	if (entry.resolution === "deferred") {
		...
		if (executable && !hasRoadmapOwnership)
			gaps.push("executable_requires_task_or_sprint");
	} else if (executable && NON_EXECUTABLE_RESOLUTIONS.has(entry.resolution)) {
		if (entry.resolution === "already-implemented") {
			if (entry.source_refs.length === 0 && entry.knowledge_refs.length === 0)
				gaps.push("missing_implementation_refs");
		} else if (["blocked", "route-back"].includes(entry.resolution)) {
			if (!hasRoadmapOwnership) gaps.push("executable_requires_task_or_sprint");
		} else if (!hasRoadmapOwnership) {
			gaps.push("executable_requires_task_or_sprint");
		}
	}
	return gaps;
...
function classifyResolution(
	...
	if (resolution.resolution === "already-implemented")
		return "already-implemented";
	if (resolution.resolution === "route-back") return "route-back";
...
function buildEntry(
	...
	const executable = rowIsExecutable(row, resolution);
	...
	return {
		...
		work_unit_ids: resolution?.work_unit_ids || [],
...
export function assessDecisionPropagation(
	...
	const knownWorkUnitIds = new Set(
		unique([
			...(options.knownWorkUnitIds || []),
			...planningBuilds.flatMap(planningWorkUnitIds),
		]),
	);
		...
		const resolution =
			...
			(allRowsResolved && !questionStillOpen(question, planningBuilds)
				? {
						...
						work_unit_ids: unique(
							rowEntries.flatMap((entry) => entry.work_unit_ids),
						),
						knowledge_refs: unique(
