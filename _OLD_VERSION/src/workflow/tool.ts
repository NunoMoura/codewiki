...
import type { CodewikiRuntimePlan } from "../runtime/types.ts";
import type {
...
import { AUTOMATION_READINESS_CONTRACT_VERSION } from "../state/automation-readiness.ts";
...
const RUNTIME_BOUNDARY_ACTIONS = new Set([
	"context_boundary",
	"context-boundary",
	"fresh_worker",
	"fresh-worker",
...
function materializeRuntimeBoundaryPlan(
	input: JsonRecord,
	action: string,
): CodewikiRuntimePlan | null {
	if (!RUNTIME_BOUNDARY_ACTIONS.has(action)) return null;
	const nested =
		nestedInput(input, [
			"context_boundary",
			"contextBoundary",
	...
	const taskId = String(
		input.taskId || input.task_id || nested.taskId || nested.task_id || "",
	).trim();
	if (!taskId) return null;
	const freshWorker = action === "fresh_worker" || action === "fresh-worker";
	const reason = String(
		nested.reason ||
			input.reason ||
			(freshWorker ? "runtime-fresh-worker" : "runtime-context-boundary"),
	).trim();
	const contextBoundary = {
		reason,
		trace_refs: stringValues(nested.trace_refs, input.trace_refs),
		gate_refs: stringValues(nested.gate_refs, input.gate_refs),
		source_refs: stringValues(nested.source_refs, input.source_refs),
		graph_lens:
			String(nested.graph_lens || input.graph_lens || "").trim() || undefined,
		expected_output:
			String(nested.expected_output || input.expected_output || "").trim() ||
			undefined,
		constraints: isRecord(nested.constraints)
			? nested.constraints
			: isRecord(input.constraints)
				? input.constraints
				: undefined,
		content_evidence_requirements: stringValues(
			nested.content_evidence_requirements,
			input.content_evidence_requirements,
		),
	};
	const cycle: JsonRecord = {
		action: freshWorker ? "fresh_worker" : "context_boundary",
		next_task: taskId,
		summary: reason,
		context_boundary: contextBoundary,
		follow_up_intent:
			String(input.followUpIntent || input.follow_up_intent || "").trim() ||
			undefined,
	};
	if (freshWorker) {
		cycle.fresh_worker = {
	...
	const readiness = {
		...
		state: "ready",
		blockers: [],
		source_refs: stringValues(contextBoundary.source_refs, taskId),
		expires_at: expiresAt,
		next_action: {
			action: freshWorker ? "fresh_worker" : "context_boundary",
			task_id: taskId,
			trace_refs: stringValues(contextBoundary.trace_refs),
			gate_refs: stringValues(contextBoundary.gate_refs),
			git_refs: [],
		},
	};
	return {
		...
		budget: {
			...
			maxWallSeconds: 120,
			maxTokens: 8000,
			...(isRecord(input.budget) ? input.budget : {}),
		},
		cycles: [{ ...cycle, automation_readiness: readiness }],
...
export async function executeCodewikiRuntimeTool(
	...
	const operations: CodewikiWorkflowOperation[] = [];
	const runtimePlan = materializeRuntimeBoundaryPlan(input, action);
	if (runtimePlan) {
		...
		operations.push(operation("wiki_runtime_step", action, result));
	}
