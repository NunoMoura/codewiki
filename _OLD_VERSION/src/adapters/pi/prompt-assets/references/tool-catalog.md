...
| `wiki_decide` | `src/workflow/tool.ts`, `src/decision/tool.ts`, `src/build/tool.ts` | Manage decision rows, approvals, KB mappings, propagation evidence, and decision-build creation. | Pending decision rows plus transient `decision_build` writes; canonical KB edits happen with normal file tools after approved rows. |
