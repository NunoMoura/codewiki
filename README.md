...
`buildVerificationCapabilityMatrix()` exposes executable `codewiki.verification-capability-matrix@1.0.0` truth for every Loop-qualified Check. Rows bind exact catalog/config identity, execution availability, Evidence obligations, collection gaps, and potential standard formats. Canonical Evidence material has native validation. SARIF 2.1 and JUnit XML ingestion are implemented; LCOV, Cobertura, CycloneDX, SPDX, Playwright, axe, Pact, OpenAPI, and provider-check adapters remain explicitly `not_implemented`. Formats are Evidence-only and never grant Results.
