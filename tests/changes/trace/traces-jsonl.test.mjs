...
import { invalidTraceRefs } from "../../../src/changes/trace/refs.ts";
import {
	CodewikiError,
	codewikiErrorData,
	isCodewikiError,
...
import {
	...
	readLastTraceRecord,
	readTrace,
} from "../../../src/changes/trace/reader.ts";
import {replayTrace} from "../../../src/changes/trace/replay.ts";
import {
	buildTraceRetentionStub,
	createTraceCloseRecord,
} from "../../../src/changes/trace/retention.ts";
import {traceHasEvent, traceRefs} from "../../../src/changes/trace/queries.ts";
import {TRACE_LOOP_VALUES, traceFilePath} from "../../../src/changes/trace/schema.ts";
import {
...
function sampleRecords() {
	...
	const event = {
		...
		refs: ["kb:system/components/traces.md", "src/changes/trace/schema.ts"],
...
describe("trace JSONL core", () => {
	...
	it("creates target loop iteration trace events", () => {
		const event = createLoopIterationEvent({
			...
			sequence: 2,
			refs: [
				"src/changes/trace/events.ts",
				"src/changes/trace/events.ts",
				"tests/changes/trace/traces-jsonl.test.mjs",
			],
			createdAt: "2026-06-11T00:00:03.000Z",
			iteration: 1,
			trigger: "worker_results",
			output: { changedPaths: ["src/changes/trace/events.ts"] },
			exit: {
				status: "route_back",
				targetLoop: "planning",
			...
			progress: {
				changedRefs: ["src/changes/trace/events.ts", "src/changes/trace/events.ts"],
		...
		assert.deepEqual(event.data.progress.changedRefs, ["src/changes/trace/events.ts"]);
	});
