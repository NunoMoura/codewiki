...
function record(id, options = {}) {
	const change = acceptedChangeFixture({
		id,
		question:
			options.question || "Should runtime budget feedback become a Change?",
		currentState:
			options.currentState ||
			"Budget feedback is retained only in runtime logs.",
		desiredState:
			options.desiredState ||
			"Budget feedback becomes one reviewable pending Change.",
		targetRefs: options.targetRefs || ["src/runtime/execution-policy.ts"],
		sourceRefs: options.sourceRefs || [
			"trace:TRACE-budget:implementation:iteration:1",
		],
	});
...
describe("feedback Change deduplication", () => {
	it("prefers exact source identity and deterministic semantic matches", () => {
		...
		candidate.evidence.sourceRefs = [
			"trace:TRACE-other:implementation:iteration:1",
		];
