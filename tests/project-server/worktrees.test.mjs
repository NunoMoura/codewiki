...
import {
	executeProjectServerWorktreeCommands,
...
describe("runtime worktree planning", () => {
	it("creates deterministic worktree refs and process commands", () => {
		const [plan] = planProjectServerWorkUnitClaimWorktrees([item("WU-one")], {
			mode: "worktree",
	...
	it("adds optional setup commands to explicit worktree prepare", async () => {
		...
		const dryRun = await executeProjectServerWorktreeCommands(plan, {
	...
	it("dry-runs planned worktree commands by default", async () => {
		...
		const result = await executeProjectServerWorktreeCommands(plan);
	...
	it("executes worktree commands only with an explicit runner", async () => {
		...
		await assert.rejects(
			() => executeProjectServerWorktreeCommands(plan, { dryRun: false }),
			/requires a runner/,
		);
	...
	it("stops explicit command execution on non-zero exit", async () => {
		...
		await assert.rejects(
			() =>
				executeProjectServerWorktreeCommands(plan, {
					dryRun: false,
	...
	it("auto mode isolates parallel or dirty overlapping work only", () => {
		const parallel = planProjectServerWorkUnitClaimWorktrees(
			[item("WU-one"), item("WU-two")],
			{ mode: "auto", repoRoot: "/tmp/repo/codewiki" },
		);
		const cleanSolo = planProjectServerWorkUnitClaimWorktrees([item("WU-one")], {
			mode: "auto",
			repoRoot: "/tmp/repo/codewiki",
		});
		const dirtySolo = planProjectServerWorkUnitClaimWorktrees(
			[item("WU-one", ["src/project-server"])],
			{
				mode: "auto",
				repoRoot: "/tmp/repo/codewiki",
				dirtyPaths: ["src/project-server/claims/policy.ts"],
			},
		);
		const dirtyGlobSolo = planProjectServerWorkUnitClaimWorktrees(
		...
		const unrelatedDirtySolo = planProjectServerWorkUnitClaimWorktrees(
