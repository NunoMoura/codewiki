...
import { runWikiImplement } from "../../../src/project-server/commands/implementation.ts";
import { collectWorkerReports } from "../../../src/project-server/workers/reports.ts";
...
async function seedProjectServerImplementation(root, suffix) {
	const seeded = await seedProjectServerImplementationProject(root, { suffix });
...
describe("wiki_implement core facade", () => {
	...
	it("rejects caller proof and deprecated evidence aliases", async () => {
		...
		try {
			const { expectedWorkStateDigest } = await seedProjectServerImplementation(
				root,
	...
	it("routes implementation uncertainty back to decision", async () => {
		const root = await fixture();
		try {
			const prepared = await seedProjectServerImplementation(
				root,
	...
	it("previews implementation from normalized Pi worker completion", async () => {
		...
		try {
			...
			const release = createProjectServerWorkerCompletionReleaseEvents(
				workerReports,
				[claim],
				{
	...
	it("appends implementation iteration with Project Server-owned CAS authority", async () => {
		const root = await fixture();
		try {
			const prepared = await seedProjectServerImplementation(
				root,
