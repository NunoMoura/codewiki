...
import { runProjectServer } from "../../../src/project-server/commands/work.ts";
...
function expiredClaim(sequence = 8) {
	return createProjectServerClaimEvent({
		...
		pathScopes: ["src/project-server/a.ts"],
...
describe("wiki_runtime core facade", () => {
	it("previews work-unit claim selections and claim events", async () => {
		const result = await runProjectServer({
			mode: "preview",
			queue: queue(),
			maxWorkers: 1,
			createdAt: "2026-06-11T00:00:01.000Z",
	...
	it("ignores raw decision items during work-unit claim selection", async () => {
		...
		mixedQueue.items.unshift({
			...
			planningRefs: [],
			componentRefs: [],
			pathScopes: ["src/project-server/raw-decision.ts"],
			dependsOn: [],
			blockers: [],
			qualityStandards: [],
			qualityBlockers: [],
	...
	it("uses config worker limits and blocks unsafe append attempts", async () => {
		const configured = await runProjectServer({
			mode: "preview",
	...
	it("appends runtime claim events across trace files", async () => {
		...
		try {
			const head = createTraceHead({
				traceId: "TRACE-runtime",
				title: "Project Server work-unit claims",
				createdAt: "2026-06-11T00:00:00.000Z",
			});
			...
			await assert.rejects(
				() => runProjectServer({ mode: "append", queue: queue() }),
				/runtime\.automation is manual/,
			);
			await assert.rejects(
				() =>
