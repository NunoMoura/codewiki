...
import { createCodeWikiLoopExecutionPorts } from "../../../src/project-server/coordinator/executor.ts";
import { writeWikiConfigFile } from "../../../src/project/config-file.ts";
import { resolveWikiConfig } from "../../../src/project/config.ts";
import { readImplementationWorkerDispatchPackets } from "../../../src/project-server/workers/implementation-artifacts.ts";
import { ImplementationWorkerDispatcher } from "../../../src/project-server/workers/dispatch.ts";
import { implementationWorkerClaimReleaseJob } from "../../../src/project-server/claims/release.ts";
...
test("elected runtime derives claims and exact worker Assignments from WorkState", async () => {
	...
	const fixture = await seedProjectServerImplementation(root, {
	...
	const dispatcher = new ImplementationWorkerDispatcher({
		repoRoot: root,
		coordinator,
		reactor: new ProjectServerReactor(root),
		adapter,
	...
	try {
		...
		const observed = await new ProjectServerReactor(root).observe({
			kind: "timer_due",
...
test("worker claims remain held when elected coordinator lacks supervision", async () => {
	...
	await seedProjectServerImplementation(root, { suffix: "worker-held" });
	const coordinator = new ProjectCoordinator(root, {
