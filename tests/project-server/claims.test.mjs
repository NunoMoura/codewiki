...
import { createProjectServerWorkerCompletionReleaseEvents } from "../../src/project-server/claims/work-unit-events.ts";
import {
	appendProjectServerLeaseExpirations,
	planProjectServerLeaseExpirations,
} from "../../src/project-server/claims/leases.ts";
...
describe("runtime claim events", () => {
	it("creates canonical runtime claim trace events", () => {
		...
		const claim = createProjectServerClaimEvent({
			traceId: "TRACE-runtime",
	...
	it("creates worker completion release events with session provenance", () => {
		...
		const batch = createProjectServerWorkerCompletionReleaseEvents(
			[
				{
					workerId: "worker-1",
					workUnitId: "WU-runtime",
					...
					refs: ["tests/project-server/runtime-claims.test.mjs"],
				},
			],
			[claim],
			{
		...
		assert.equal(
			release.refs.includes("tests/project-server/runtime-claims.test.mjs"),
			true,
		);
		...
		const statusBatch = createProjectServerWorkerCompletionReleaseEvents(
			[
				{
					traceId: "TRACE-runtime",
					workerId: "worker-2",
	...
	it("does not release a newer claim with the same work unit", () => {
		const planning = planningEvent();
		const planningRef = planningWorkRef(planning);
		const oldClaim = createProjectServerClaimEvent({
			traceId: "TRACE-runtime",
		...
		const newClaim = createProjectServerClaimEvent({
			traceId: "TRACE-runtime",
	...
	it("plans expired lease releases without noisy heartbeat events", () => {
		const planning = planningEvent();
		const planningRef = planningWorkRef(planning);
		const expired = createProjectServerClaimEvent({
			traceId: "TRACE-runtime",
		...
		const active = createProjectServerClaimEvent({
			traceId: "TRACE-runtime",
		...
		const alreadyReleased = createProjectServerClaimEvent({
			traceId: "TRACE-runtime",
		...
		const batch = planProjectServerLeaseExpirations(
	...
	it("appends expired lease releases with trace byte preflight", async () => {
		...
		try {
			...
			const head = createTraceHead({
				traceId: "TRACE-runtime",
				title: "Project Server lease expiry",
				createdAt: "2026-06-11T00:00:00.000Z",
			});
			const seed = await appendTraceRecords(root, [head, planning, claim], 0);
			const batch = planProjectServerLeaseExpirations([head, planning, claim], {
				generatedAt: "2026-06-11T00:00:05.000Z",
				nextSequenceByTrace: { "TRACE-runtime": 3 },
			});
			const append = await appendProjectServerLeaseExpirations(batch, {
				repoRoot: root,
			...
			await assert.rejects(
				() =>
					appendProjectServerLeaseExpirations(batch, {
						repoRoot: root,
						expectedBytesByTrace: { "TRACE-runtime": seed.nextBytes },
					}),
				/append conflict/,
