...
} from "../../src/project-server/coordinator/index.ts";
...
describe("runtime coordinator heartbeat queue", () => {
	...
	it("keeps higher-priority heartbeats while merging refs and data", () => {
		const queue = createProjectServerHeartbeatQueue({ now: () => 100 });
		queue.request({
			source: "schedule",
			intent: "scheduled",
			triggerId: "TRG-ci",
	...
	it("drains highest priority first without making a daemon", () => {
		let now = 0;
		const queue = createProjectServerHeartbeatQueue({ now: () => now });
		now = 10;
		queue.request({
			source: "schedule",
			intent: "scheduled",
	...
	it("orders equal-priority heartbeats by request time", () => {
		const queue = createProjectServerHeartbeatQueue();
		queue.request({
			source: "webhook",
			intent: "event",
			traceId: "TRACE-late",
	...
	it("returns skipped and blocked trigger run append results without inventing traces", async () => {
		...
		try {
			const duplicateQueue = createProjectServerHeartbeatQueue();
			...
			const skippedPlan = planProjectServerTriggerRuns({
				triggers: triggerView({
					status: "completed",
					runs: [
						{
