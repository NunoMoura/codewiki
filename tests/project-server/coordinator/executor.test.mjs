...
} from "../../../src/project-server/coordinator/executor.ts";
...
describe("runtime semantic executor", () => {
	it("quiesces without invoking an adapter when no semantic work is eligible", async () => {
		...
		roots.push(root);
		let calls = 0;
		const result = await runProjectServerSemanticExecutor({
			repoRoot: root,
	...
	it("adapts non-authoritative command results into the legacy Implementation facade", async () => {
		...
		roots.push(root);
		const seeded = await seedProjectServerImplementation(root, {
		...
		await assert.rejects(
			...
			/Project Server implementation execution port is unavailable\./,
		);
