...
} from "../../../src/project-server/coordinator/reactions.ts";
	...
	try {
		...
		for (const [field, value] of [
			...
			await assert.rejects(
				...
				new RegExp(
					`Project Server decision candidate cannot supply Project Server-owned fields: ${field}`,
				),
			);
		}
		await assert.rejects(
			...
			/Project Server decision candidate received unsupported fields: candidateId/,
		);
		await assert.rejects(
			() =>
				client.submitCandidate(
