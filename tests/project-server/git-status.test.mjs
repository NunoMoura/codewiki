	...
	it("collects base SHA and dirty paths with read-only git commands", async () => {
		...
		const runner = (args, context) => {
			...
			if (command === "status --porcelain=v1 -z") {
				return { stdout: " M src/project-server/claims/policy.ts\0?? tests/new.test.mjs\0" };
			}
		...
		assert.deepEqual(snapshot.dirtyPaths, [
			"src/project-server/claims/policy.ts",
			"tests/new.test.mjs",
		]);
		...
		assert.deepEqual(inputs, {
			baseRef: "abc1234",
			baseSha: "abc1234",
			dirtyPaths: ["src/project-server/claims/policy.ts", "tests/new.test.mjs"],
		});
		assert.deepEqual(
			calls.map(([, args]) => args),
			[
