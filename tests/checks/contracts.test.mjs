...
import {createCheckResult} from "../../src/checks/results.ts";
import {
...
function invocationInput() {
	...
	return {
		...
		check: {
			...
			enforcement: "require",
			required: true,
			parameters: {input: {paths: ["src/checks"]}},
		},
		context: {
			repository: complete(
				["src/checks"],
			...
			findings: [
				{
					...
					location: {ref: "src/checks/protocol.ts", startLine: 10},
					repair: {
