...
import { readRoadmapFile } from "../../src/roadmap/store.ts";
import {
...
import {
	...
	readGraphPanelData,
	updateRuntimeDecisionTableRow,
} from "../../src/adapters/pi/ui/manager.ts";
...
try {
	...
	await writeFile(
		...
		JSON.stringify({ project_name: "smoke", schema_version: 4 }, null, 2),
	);
	await writeFile(
		...
		JSON.stringify(
			{
				...
				tasks: {
					"TASK-001": {
						...
						delta: { desired: "done", current: "not done", closure: "done" },
						created: "2026-05-11",
						updated: "2026-05-11",
					},
				},
				sprints: {
					"SPRINT-001": {
						...
						outcome: "Scope work",
						task_ids: ["TASK-001"],
						scope: { knowledge: [".codewiki/kb/system/**"], code: ["src/**"] },
						budget: { maxTokens: 1000, maxCostUsd: 1, maxSessions: 2 },
						gates: ["validation"],
						created: "2026-05-11",
						updated: "2026-05-11",
					},
	...
	const project = {
		root,
		graphPath: join(root, ".codewiki/index_graph.json"),
		config: {},
		roadmapPath: ".codewiki/roadmap/queue.json",
	};
	const proposed = await executeDecisionTableAction(project, {
		action: "propose",
		table_id: "DT-001",
	...
	assert.equal(
		(await readRuntimeDecisionTables(project)).tables[0].rows[0].user_action,
		"pending",
	);
	assert.equal(
		updateRuntimeDecisionTableRow(project, "DT-001", "DTR-001", "approved"),
		true,
	);
	assert.equal(readDecisionTablePanelData(project).rows[0].status, "approved");
	...
	assert.deepEqual(readDecisionTablePanelData(project).rows[0].alternatives, [
		"Newer",
	]);
		...
		JSON.stringify(
			{
				...
				nodes: [
					{ id: "build:a", kind: "build" },
					{ id: "task:TASK-001", kind: "roadmap_task" },
				],
				edges: [
					{ from: "build:a", to: "task:TASK-001", kind: "build_produces_task" },
				],
				views: {
					scope_views: {
						roadmap: { task_ids: ["TASK-001"], open_task_ids: ["TASK-001"] },
						sprints: {
							"SPRINT-001": {
								id: "SPRINT-001",
								status: "active",
								task_ids: ["TASK-001"],
								open_task_ids: ["TASK-001"],
							},
						},
					},
					reconciliation: {
						next_action: { command: "/wiki-resume TASK-001" },
						items: [],
					},
				},
			},
