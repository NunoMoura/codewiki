...
describe("implementation review evidence protocol", () => {
	it("classifies artifacts without assuming TypeScript", () => {
		const examples = new Map([
			...
			[
				"tests/execution/review/review-evidence.test.mjs",
	...
	it("normalizes common and language-specific evidence into one report", () => {
		const report = createImplementationEvidenceReport({
			...
			changedPaths: ["src/execution/review/evidence-report.ts"],
			checks: [
				{
					command: "npm run typecheck",
					status: "pass",
					outputRef: "tests/execution/review/review-evidence.test.mjs",
				},
			],
			diagnostics: [
				{
					path: "src/execution/review/evidence-report.ts",
					severity: "error",
					message: "Example blocking diagnostic.",
					sourceId: "tsjs.tsc",
					ruleId: "TS0000",
					evidenceRefs: ["src/execution/review/evidence-report.ts"],
				},
			],
			symbols: [
				{
			...
			dependencyEdges: [
				{
					from: "src/execution/review/evidence-report.ts",
					to: "src/execution/review/artifacts.ts",
					kind: "import",
				},
			],
			evidenceLinks: [
				{
					...
					criterionId: "AC-001",
					evidenceRefs: ["tests/execution/review/review-evidence.test.mjs"],
				},
			],
		});
