...
import { runCommonFastFeedback } from "../../../src/execution/review/index.ts";
...
describe("implementation common fast feedback", () => {
	it("blocks source edits outside active path scope", () => {
		const result = runCommonFastFeedback({
			...
			pathScopes: ["src/execution/review/"],
		});
	...
	it("passes clean scoped code-bearing edits", () => {
		const result = runCommonFastFeedback({
			changedPaths: ["src/execution/review/fast-feedback.ts"],
			pathScopes: ["src/execution/review/"],
			contentByPath: {
				"src/execution/review/fast-feedback.ts": "export const ok = true;",
			},
		});
