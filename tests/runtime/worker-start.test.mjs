...
function plannedClaimSelection() {
	...
	const claimBatch = createRuntimeWorkUnitClaimEvents(plan, {
...
describe("worker-start seam", () => {
	it("creates scoped implementation worker prompts", () => {
		...
		assert.deepEqual(report.checksRun, ["node --test tests/example.test.mjs"]);
	...
	it("starts one injected Pi session per selected work unit", async () => {
		const { plan, claimBatch } = plannedClaimSelection();
		const created = [];
		const prompts = [];
	...
	it("appends runtime claims before starting independent sessions", async () => {
		...
		try {
			...
			assert.equal(trace.records.at(-1).event, "runtime.work_unit.claimed");
			assert.deepEqual(
				created.map((input) => [input.workUnitId, input.workerId]),
				[
					["WU-a", "pi-worker-001"],
	...
	it("appends failed-start releases after runtime worker start failures", async () => {
		...
		try {
			...
			assert.equal(events.at(-1).event, "runtime.work_unit.claim.released");
