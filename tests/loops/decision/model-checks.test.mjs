...
} from "../../../src/loops/decision/evidence.ts";
import {
	DECISION_MODEL_CHECK_REQUEST_PROTOCOL,
	createDecisionModelCheckExecutors,
} from "../../../src/loops/decision/model-checks.ts";
import {
	createDecisionGate,
...
describe("native Decision Model Checks", () => {
	...
	it("runs classified security review as an asserted dependency-bound challenge", async () => {
		...
		const requests = [];
		const runtime = createDecisionGate({
		...
		const scannerBlocked = createDecisionGate({
		...
		const incompleteScannerRuntime = createDecisionGate({
		...
		const challenged = createDecisionGate({
			securityScanners: securityScannerConfiguration(),
			modelChecks: {
				route: route(),
				transport: {
	...
	it("wires model Evidence into complete native Decision execution", async () => {
		...
		assert.equal(result.result.report.status, "pass");
		assert.equal(result.transition.route, "planning");
		assert.equal(result.transition.reasonCode, "decision-approved");
		assert.match(result.transition.routeDigest, /^sha256:[0-9a-f]{64}$/);
		...
		assert.equal(replay.transition.routeDigest, result.transition.routeDigest);
		assert.equal(calls, 2);
	...
	it("runs high-risk research Checks and replays their exact Evidence", async () => {
		const setup = fixture({
			risk: "high",
			proofRefs: ["tests/loops/decision/a.test.mjs", "tests/loops/decision/b.test.mjs"],
	...
	it("keeps unsupported, unavailable, and malformed outcomes explicit", async () => {
		for (const scenario of [
			...
			assert.equal(
				deriveDecisionLifecycleTransition(setup.candidate, result.report).route,
