...
} from "../../../src/loops/implementation/evidence.ts";
import { evaluateImplementationExit } from "../../../src/loops/implementation/loop.ts";
import { aggregateImplementationWorkerReports } from "../../../src/loops/implementation/workers.ts";
		...
		workItemInputs: [
			{
				...
				pathScopes: ["src/loops/implementation"],
				verification: [
					"tests/loops/implementation/implementation-iteration.test.mjs",
				],
			},
		],
	});
...
describe("implementation iteration runner", () => {
			...
			changeInputs: [
				{
					...
					codePaths: ["src/loops/implementation/iteration.ts"],
					testPaths: ["tests/loops/implementation/implementation-iteration.test.mjs"],
					checks: ["npm test"],
					checkResults: [
						{
							command: "npm test",
			...
			checkResults: [
				{
					command: "npm test",
					status: "pass",
					outputRef: "tests/loops/implementation/implementation-iteration.test.mjs",
				},
			],
			acceptanceEvidenceItems: [
				{
		...
		const changes = normalizeImplementationChanges([
			{
				id: "IC-uncertain",
				planningRefs: [planningEvent.id],
				codePaths: ["src/loops/implementation/loop.ts"],
					...
					acceptanceEvidenceItems: [
						{
							...
							evidenceRefs: ["src/loops/implementation/loop.ts"],
						},
					],
					contentProof: { workingTreeDigest: "sha256:abc123" },
					...implementationQualityFields(),
			...
			changeInputs: [
				{
					...
					checkResults: [
						{
							command:
								"node --test tests/loops/implementation/implementation-iteration.test.mjs",
							status: "fail",
							phase: "red",
							criterionId: "AC-001",
							outputRef:
			...
			workerReports: [
				{
					...
					changeInputs: [
						{
							id: "IC-worker",
							codePaths: ["src/loops/implementation/workers.ts"],
							testPaths: [
								"tests/loops/implementation/implementation-iteration.test.mjs",
							],
							checkResults: [{ command: "npm test", status: "pass" }],
	...
	it("fills worker change content proof from normalized worker proof", () => {
			...
			workerReports: [
				{
					...
					proof: {
						headSha: "abc1234",
						treeSha: "def5678",
						changedPaths: ["src/loops/implementation/workers.ts"],
						checksRun: ["npm test"],
					},
					changeInputs: [
						{
		...
		assert.deepEqual(result.changes[0].codePaths, [
			"src/loops/implementation/workers.ts",
		]);
	...
	it("blocks changed and evidence paths absent from repo snapshot", () => {
		...
		const result = runImplementationIteration({
			traceId: "TRACE-implementation",
			planningEvents: planning,
			existingPaths: ["tests/loops/implementation/implementation-iteration.test.mjs"],
			changeInputs: [
				{
					id: "IC-missing-paths",
					planningRefs: [planningEvent.id],
					codePaths: ["src/loops/implementation/missing.ts"],
					checkResults: [
						{
							command: "npm test",
							status: "pass",
							outputRef: "tests/loops/implementation/missing.test.mjs",
						},
					],
					acceptanceEvidenceItems: [
						{
