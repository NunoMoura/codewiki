...
assert.ok(existsSync(source("audit", "types.ts")), "Audit types should be owned by src/audit/types.ts");
assert.ok(existsSync(source("audit", "tool.ts")), "Audit tool/use-case execution should be owned by src/audit/tool.ts");

for (const relPath of removedAuditShimPaths) {
	assert.ok(!existsSync(resolve(repoRoot, relPath)), `${relPath} should be removed after TASK-021 migration`);
}

const auditTypes = await import(pathToFileURL(source("audit", "types.ts")).href);
assert.deepEqual(auditTypes.AUDIT_PROFILE_VALUES, ["alignment", "file-structure", "stale-reference", "package", "security", "generated-parity", "changed", "task"], "Audit profiles should remain source-root owned");
assert.deepEqual(auditTypes.AUDIT_STATUS_VALUES, ["pass", "warning", "fail"], "Audit statuses should remain source-root owned");

const auditTool = await import(pathToFileURL(source("audit", "tool.ts")).href);
assert.equal(typeof auditTool.executeCodewikiAudit, "function", "Audit executor should remain exported from source-root owner");
assert.equal(typeof auditTool.formatAuditReport, "function", "Audit formatter should remain exported from source-root owner");
assert.equal(typeof auditTool.normalizeAuditProfiles, "function", "Audit profile normalization should remain exported from source-root owner");
...
function pointsAtRemovedAuditShim(filePath, specifier) {
	if (specifier.startsWith(".")) {
		...
		return removedAuditShimAbsPaths.some((removedPath) =>
			resolved === removedPath || resolved === removedPath.replace(/\.ts$/, ""),
		);
	}
	return removedAuditShimPaths.some((removedPath) =>
		specifier === removedPath || specifier.endsWith(`/${removedPath}`),
	);
}

const importViolations = [];
for (const codeRoot of [resolve(repoRoot, "src"), resolve(repoRoot, "tests"), resolve(repoRoot, "scripts")]) {
...
assert.match(schemaSource, /from "\.\.\/\.\.\/audit\/types\.ts"/, "Pi schemas should consume source-root audit values");
...
assert.doesNotMatch(auditRoot, /\.\.\/application\/tools\/audit|\.\.\/domain\/audit\/types/, "Audit source root should not import old audit shim paths");
